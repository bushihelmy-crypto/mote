"""DurableBackend — the neutral durable-step seam both persistence tiers share.
...
Two implementations hang on the ONE :class:`DurableBackend` protocol so the loop
...
* :class:`JsonlBackend` (Tier 1, always-on, zero-dependency) — records the step's
  lifecycle in the shared :class:`~mote.common.ledger.RunJournal` and replays a
...
* the optional Temporal backend (Tier 2, ``durable_exec/temporal/``) — dispatches
...
Payloads are opaque strings here: the caller (the loop's durable runner, A3+)
...
class DurableBackend(Protocol):
    """The narrow seam the loop drives to make a step durable.
    ...
    the loop's ``turn_index``, which resume does not restore).
    ...
    (:class:`~mote.loop.durable.runner.DurableRunner`) reach the journal through
...
class JsonlBackend:
    ...
    per-session journal; the loop borrows it to build this backend, so
    ``executor`` never depends on ``loop``). Constructing it writes nothing —
