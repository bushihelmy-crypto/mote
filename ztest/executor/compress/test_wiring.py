#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Wiring tests for ``ToolExecutor._compress_result`` / ``_command_for_compression``.

Exercises the executor-side glue: shell-tool routing, the marker line + raw
persistence, and the skip paths (media, already-persisted, config-off, non
shell tools). ``tool_result_limit._persist`` is redirected to ``tmp_path``.
"""
...
def _persist_to_tmp(monkeypatch, tmp_path):
    """Point ``_persist`` at ``tmp_path`` and record what it wrote."""
    written = {}
    ...
    monkeypatch.setattr(tool_result_limit, "_persist", fake_persist)
    return written
...
class TestBashRouting:
    def test_bash_compressed_with_marker_and_raw_persist(self, monkeypatch, tmp_path):
        ...
        out = ex._compress_result(result, "Bash", {"command": "pytest tests/"})
    ...
    def test_bash_success_never_changed_on_failure(self, monkeypatch, tmp_path):
        ...
        out = ex._compress_result(result, "Bash", {"command": "pytest"})
...
class TestTerminalRouting:
    def test_terminal_uses_first_input_line(self, monkeypatch, tmp_path):
        ...
        ex = ToolExecutor("sess")
        result = ToolResult(output=_PYTEST, success=True)
        out = ex._compress_result(result, "Terminal", {"input": "pytest tests/\n"})
        assert out.output.startswith("[compressed: pytest;")

    def test_terminal_empty_input_skipped(self):
        ex = ToolExecutor("sess")
        result = ToolResult(output=_PYTEST, success=True)
        out = ex._compress_result(result, "Terminal", {"input": "   "})
...
class TestJupyterRouting:
    def test_shell_magic_compressed(self, monkeypatch, tmp_path):
        ...
        ex = ToolExecutor("sess")
        result = ToolResult(output=_PYTEST, success=True)
        out = ex._compress_result(result, "Jupyter", {"code": "!pytest tests/\n"})
        assert out.output.startswith("[compressed: pytest;")
    ...
    def test_plain_python_not_compressed(self):
        ...
        out = ex._compress_result(result, "Jupyter", {"code": "print('pytest tests/')\n"})
...
class TestSkipPaths:
    def test_media_result_skipped(self):
        ex = ToolExecutor("sess")
        result = ToolResult(output=_PYTEST, success=True, images=["base64data"])
        out = ex._compress_result(result, "Bash", {"command": "pytest"})
        assert out.output == _PYTEST
    ...
    def test_config_off_skipped(self):
        ...
        ex = ToolExecutor("sess", limit_config=cfg)
        result = ToolResult(output=_PYTEST, success=True)
        out = ex._compress_result(result, "Bash", {"command": "pytest"})
        assert out.output == _PYTEST
    ...
    def test_non_shell_tool_skipped(self):
        ex = ToolExecutor("sess")
        result = ToolResult(output=_PYTEST, success=True)
        out = ex._compress_result(result, "Read", {"file_path": "/x"})
        assert out.output == _PYTEST
...
class TestCommandForCompression:
    def test_bash_command(self):
        ex = ToolExecutor("sess")
        assert ex._command_for_compression("Bash", {"command": "git status"}) == "git status"

    def test_terminal_first_line(self):
        ex = ToolExecutor("sess")
        assert ex._command_for_compression("Terminal", {"input": "git log\nmore"}) == "git log"

    def test_other_tool_none(self):
        ex = ToolExecutor("sess")
        assert ex._command_for_compression("Grep", {"pattern": "x"}) is None

    def test_jupyter_shell_magic_strips_bang(self):
        ex = ToolExecutor("sess")
        assert ex._command_for_compression("Jupyter", {"code": "!git diff\nx=1"}) == "git diff"

    def test_jupyter_plain_python_none(self):
        ex = ToolExecutor("sess")
        assert ex._command_for_compression("Jupyter", {"code": "import os\nos.getcwd()"}) is None

    def test_jupyter_bare_bang_none(self):
        ex = ToolExecutor("sess")
        assert ex._command_for_compression("Jupyter", {"code": "!   "}) is None

    def test_python_alias_routed(self):
        ex = ToolExecutor("sess")
        assert ex._command_for_compression("Python", {"code": "!pytest"}) == "pytest"
