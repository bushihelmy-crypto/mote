#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""End-to-end integration: ``WorkflowBuilder`` compiled task driven by the real pool.
...
from mote.orchestration.workflows import END, START, WorkflowBuilder
...
def pool(msg_buffer, store):
    return AgentBackgroundTasks(BackgroundTaskPool(msg_buffer=msg_buffer, output_store=store), "test")


def _media_graph() -> WorkflowBuilder:
...
class TestPoolIntegration:
    ...
    async def test_failure_notifies_failed(self, pool, store, msg_buffer):
        g = WorkflowBuilder("boomgraph", state_schema=S)
...
def _llm_pause_graph() -> WorkflowBuilder:
    """Graph that pauses on an LLM edge after node 'a'."""
    g = WorkflowBuilder("llmpause", state_schema=S)
...
class TestPoolPauseAndResubmit:
    ...
    async def test_deadlocked_join_sets_stalled(self, pool, store, msg_buffer):
        ...
        g = WorkflowBuilder("stallpool", state_schema=S)
...
def _timeout_graph(gate: "asyncio.Event") -> WorkflowBuilder:
    ...
    """
    g = WorkflowBuilder("hanggraph", state_schema=S)
...
class TestPoolResultLimitPolicy:
    ...
    async def test_large_error_persists_under_config(self, tmp_path, msg_buffer):
        ...
        from mote.orchestration.background_tasks import BackgroundTaskPool, TaskOutputStore

        store = TaskOutputStore(base_dir=tmp_path)
        # Tiny cap so a modest error block trips the persist threshold.
        pool = AgentBackgroundTasks(
            BackgroundTaskPool(
        ...
        g = WorkflowBuilder("bigboom", state_schema=S)
    ...
    async def test_limit_disabled_passes_error_through_whole(self, tmp_path, msg_buffer):
        ...
        g = WorkflowBuilder("nolimit", state_schema=S)
...
def _wire_registry(pool):
    ...
    from mote.orchestration.background_tasks.status import PAUSE_STATUSES, TERMINAL_STATUSES
    ...
    def _on_terminal(meta) -> None:
        ...
        registry.load(
            id=meta.task_id,
            kind="task_result",
            content=render_task_result_pointer(pointer),
            sticky=True,
        )
...
class TestPushOnceResultRegistration:
    ...
    async def test_pause_registers_resume_marker(self, pool, store, msg_buffer):
        ...
        assert tid in registry
        (m,) = registry.project()
        # A pause marker is a typed pointer with a reason and no produced output.
        assert "<status>paused</status>" in m.content
        assert "<pause-reason>waiting_for_route</pause-reason>" in m.content
        assert "<output>" not in m.content
