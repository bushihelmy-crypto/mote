...
from mote.orchestration.workflows.types import GraphRunState, NodeRecord
...
@pytest.fixture
def pool():
    return AgentBackgroundTasks(BackgroundTaskPool(MessageQueue(), max_concurrency=10), "test-state")
...
def _branch_graph(threshold: int):
    ...
    g = WorkflowBuilder("branch", state_schema=SimpleState, recursion_limit=10)
