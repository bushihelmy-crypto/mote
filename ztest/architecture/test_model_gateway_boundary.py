...
def test_semantic_router_core_is_provider_neutral() -> None:
    core = (
        ROOT / "contracts/model/routing.py",
        ROOT / "contracts/ports/model/routing.py",
    ...
    forbidden_prefixes = (
        "mote.contracts.config.model.llm",
        "mote.product",
...
def test_legacy_semantic_router_state_and_strategy_surfaces_are_deleted() -> None:
    ...
    kernel_source = (ROOT / "kernel/inference/routing.py").read_text(encoding="utf-8")
...
def test_squilla_model_lifetime_has_no_process_global_owner() -> None:
    ...
    backend_source = (ROOT / "product/entrypoints/cli/backend.py").read_text(encoding="utf-8")
