...
from metagpt.common.events import EventBus, LLMStreamDeltaEvent, PreAgentSpawnEvent
...
async def test_denies_when_child_depth_exceeds_ceiling():
    gate = SpawnGate()
    out = await gate.handle_control(
        PreAgentSpawnEvent(parent_path="/root/a/b", child_depth=3, max_depth=2)
    )
...
async def test_allows_when_within_ceiling():
    gate = SpawnGate()
    out = await gate.handle_control(
        PreAgentSpawnEvent(parent_path="/root", child_depth=1, max_depth=2)
    )
...
async def test_allows_at_exact_ceiling():
    ...
    gate = SpawnGate()
    out = await gate.handle_control(
        PreAgentSpawnEvent(parent_path="/root/a", child_depth=2, max_depth=2)
    )
    assert out is None
...
async def test_no_ceiling_never_denies():
    gate = SpawnGate()
    out = await gate.handle_control(
        PreAgentSpawnEvent(parent_path="/root", child_depth=99, max_depth=None)
    )
    assert out is None
