#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Tests for ``metagpt.common.config.env`` — env var -> nested layer mapping."""
...
def test_double_underscore_nests_single_underscore_stays_in_segment():
    env = {"AGENTFRAME_LLM__BASE_URL": "https://x/v1", "AGENTFRAME_LLM__MODEL": "claude"}
    assert build_env_layer(env) == {"llm": {"base_url": "https://x/v1", "model": "claude"}}


def test_values_are_yaml_typed():
    env = {"AGENTFRAME_ENABLE_ROUTER": "true", "AGENTFRAME_LLM__MAX_TOKEN": "8000"}
...
def test_both_prefixes_supported():
    out = build_env_layer({"METAGPT_PROXY": "http://p", "AGENTFRAME_LANGUAGE": "zh"})
    assert out["proxy"] == "http://p"
    assert out["language"] == "zh"


def test_non_prefixed_and_bare_prefix_ignored():
    env = {"PATH": "/usr/bin", "HOME": "/home/x", "AGENTFRAME_": "ignored"}
