#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Tests for ``mote.loop.durable.factory`` — backend dispatch (B0).
...
from mote.loop.durable import DurableBackend, JsonlBackend, make_durable_backend
...
def test_temporal_config_has_per_seam_activity_policy():
    from mote.common.schema import ActivityConfig, TemporalConfig
...
def _install_fake_temporal(monkeypatch, make_fn):
    """Inject a fake ``mote.durable_exec.temporal`` so the factory's lazy import
    ...
    fake = types.ModuleType("mote.durable_exec.temporal")
    fake.make_temporal_backend = make_fn  # type: ignore[attr-defined]
    monkeypatch.setitem(sys.modules, "mote.durable_exec.temporal", fake)
...
def test_real_temporal_backend_returned_when_extra_present(tmp_path):
    ...
    from mote.durable_exec.temporal import TemporalBackend
