#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Tests for ``mote.loop.durable.runner`` — the A3 think-seam durable façade.

``DurableRunner`` memoizes each think round's post-dedup :class:`ThinkResult` in
...
module functions are the single matching authority shared by the loop
...
from mote.common.events import EventBus, JournalEvent, set_bus
...
def _runner(tmp_path, session_id="sess") -> DurableRunner:
    return DurableRunner(JsonlBackend(_journal(tmp_path, session_id)))
...
# ----------------------------------------------------------------------
# DurableRunner — begin / complete / reap / reinstate
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# A6 — journal lifecycle events land on the active bus (observability)
# ----------------------------------------------------------------------
...
def test_no_bus_bound_is_silent_noop(tmp_path):
    # Emitting outside any set_bus scope must not raise (observation dropped).
    runner = _runner(tmp_path)
    step_id = runner.begin_think()
    runner.complete_think(step_id, ThinkResult(content="x"))
