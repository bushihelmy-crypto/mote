...
from mote.contracts.schema import AIMessage, ToolMessage
from mote.kernel.diagnostics import logger
from mote.kernel.output_binding import FINAL_OUTPUT_TOOL_NAME
from mote.kernel.parser.channel import CommandChannel, _collect_media, _media_message
...
class NativeToolChannel(CommandChannel):
    ...
    def tool_specs(self, executor, output_contract=None) -> Optional[list[dict]]:
        specs = list(executor.canonical_tool_specs(include_hidden=self._supports_native_tool_search) or [])
        if not self._supports_native_tool_search:
            specs = [{key: value for key, value in spec.items() if key != "defer_loading"} for spec in specs]
...
def make_command_channel(
    ...
    *,
    ...
    return XmlCommandChannel(artifact_resolver=artifact_resolver)
