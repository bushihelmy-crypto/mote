"""Inference-side execution interface.
...
``InferenceResult`` the channels/loop read. Making it an ABC lets the Role assemble
...
class BaseInferenceEngine(ABC):
    ...
    Contract used by the react loop and the command channels:
      ...
      - ``result`` holds the latest ``InferenceResult`` (text + optional tool calls).
    ...
    target: ResolvedInferenceTarget | None
    memory: "MessageStore"
    ...
    def reinstate(self, result: InferenceResult) -> None:
        ...
        completed :class:`InferenceResult` the run journal memoized before the crash
