...
InferenceContext is a pure dataclass holding all data needed for one think() cycle.
PromptBuilder is a stateless assembler that turns InferenceContext into prompts.
...
from mote.kernel.inference.prompts import (
    COMPACTION_SECTION,
    LANGUAGE_SECTION,
    SCRATCHPAD_SECTION,
...
@dataclass
class InferenceSubsystems:
    """The live collaborators a Role hands to PromptBuilder for one think() cycle.

    Counterpart to InferenceInputs: where InferenceInputs is a pure-data snapshot, this
    ...
    as clean as InferenceContext (data) vs the subsystems (behavior).
...
class InferenceContext:
    ...
    # the provider ``tools=`` request field.
    tool_catalog: str = ""
    # Session-stable instructions contributed by configured Toolsets. Dynamic
    # run/step instructions use a request-only turn-context source instead.
...
class PromptBuilder:
    ...
    Takes template strings and a InferenceContext (for data),
    returns (system_prompt, user_prompt) ready for LLM.
    """

    @staticmethod
    def build(system_tpl: str, cmd_tpl: str, ctx: InferenceContext) -> tuple[str, str]:
        """Assemble system_prompt and user_prompt from mote.runtime.context."""
        result = PromptBuilder.assemble(system_tpl, cmd_tpl, ctx)
        return result.system_prompt, result.user_prompt

    @staticmethod
    def assemble(system_tpl: str, cmd_tpl: str, ctx: InferenceContext) -> PromptBuildResult:
        static_template, boundary, dynamic_template = system_tpl.partition(SYSTEM_PROMPT_DYNAMIC_BOUNDARY)
        if not boundary:
            static_template, dynamic_template = "", system_tpl
        if static_template and Template.pattern.search(static_template):
            raise ValueError("static prompt prefix must not contain placeholders")
        static = PromptBuilder._build_system_prompt(static_template, ctx).strip()
        dynamic = PromptBuilder._build_system_prompt(dynamic_template, ctx).strip()
        user_prompt = PromptBuilder._build_user_prompt(cmd_tpl, ctx)
        assembler = PromptAssembler({"inference": frozenset({"base", "dynamic", "turn"})})
        for key, region, content in (
            ("base", PromptRegion.STATIC, static),
            ("dynamic", PromptRegion.SYSTEM_DYNAMIC, dynamic),
            ("turn", PromptRegion.TURN, user_prompt),
        ):
            assembler.register(
                PromptSection(
                    PromptSectionIdentity("inference", key, region, "1"),
                    content,
                )
            )
        assembled = assembler.assemble(ctx.protocol_vocabulary)
        system_prompt = (
            "\n\n".join(part for part in (assembled.static_prefix, assembled.system_dynamic) if part).strip() + "\n"
        )
        user_prompt = assembled.turn_content
        ...
        if ctx.lower is not None:
            ...
            if "⟦" in system_prompt or "⟦" in user_prompt:
                raise ValueError("unresolved command-protocol symbol in assembled prompt")
        return PromptBuildResult(
            system_prompt,
            user_prompt,
            assembled.section_set_fingerprint,
            assembled.vocabulary_fingerprint,
        )
    ...
    @staticmethod
    def _system_substitutions(ctx: InferenceContext) -> dict:
        """Map InferenceContext fields to the system template's $placeholders."""
        return dict(
            ...
            tool_instructions=ctx.tool_instructions,
            # command_guide + tool_usage_guide (+ any future protocol section).
            **ctx.prompt_vars,
        )

    @staticmethod
    def _user_substitutions(ctx: InferenceContext) -> dict:
        """Map InferenceContext fields to the command template's $placeholders.
    ...
    @staticmethod
    def _build_system_prompt(system_tpl: str, ctx: InferenceContext) -> str:
    ...
    @staticmethod
    def _build_user_prompt(cmd_tpl: str, ctx: InferenceContext) -> str:
    ...
    # ------------------------------------------------------------------
    # Context collection — gathers all data from subsystems into InferenceContext
    # ------------------------------------------------------------------

    @staticmethod
    async def collect_context(inputs: InferenceInputs, subsystems: InferenceSubsystems) -> InferenceContext:
        ...
        # XML built-in definitions belong to the system prompt. MCP definitions
        # stay out because they hot-reload and are announced through the per-turn
        # ToolCatalogContextSource under both protocols. XML pipelines share that
        # volatile reminder path; Native callable definitions ride ``tools=``.
        ...
        ctx.language = PromptBuilder._make_language(subsystems.response_language)
        ...
        if subsystems.executor is not None:
            instruction_blocks = subsystems.executor.static_toolset_instructions()
            if instruction_blocks:
                ctx.tool_instructions = PromptBuilder._section(
        ...
        # fields-on-InferenceInputs + collect_context override blocks here. The
        ...
        # InferenceInputs. None channel => the InferenceContext defaults ("").
        ...
        if channel is not None:
            ...
            if channel.wants_tool_catalog() and subsystems.executor is not None:
                schemas = subsystems.executor.xml_tool_schemas() or {}
                if schemas:
                    ctx.tool_catalog = "# Available Commands\n" + json.dumps(
                        schemas,
                        ensure_ascii=False,
                        sort_keys=True,
                    )
        ...
        if channel is not None:
            symbols = tuple(sorted((str(key), str(value)) for key, value in channel.vocabulary().items()))
            ctx.protocol_vocabulary = ProtocolVocabulary(
                type(channel).__name__,
                "1",
                symbols,
                hashlib.sha256(json.dumps(symbols, separators=(",", ":")).encode()).hexdigest(),
            )
        return ctx
    ...
    def _make_compaction_section(config) -> str:
        ...
        that is what actually clears old tool results from mote.runtime.context. It
