"""CodeMap execution facade that turns touched files into a local navigation map.
...
- :class:`~mote.runtime.code_map.extractor.CodeMapExtractor` — ``ast``-derived
  symbols + import/call edges for one Python file, with an mtime freshness cache;
- :class:`~mote.runtime.code_map.store.CodeMapStore` — a tiny SQLite nodes /
...
:class:`~mote.runtime.code_map.providers.base.ModuleResolver` (chosen by
...
from mote.runtime.code_map.languages import all_registered_providers, provider_for
from mote.runtime.code_map.model import CallEdge, FileExtract, Symbol
from mote.runtime.code_map.providers.base import ModuleResolver
from mote.runtime.code_map.scopes import Def
from mote.runtime.code_map.store import CodeMapStore
from mote.runtime.file_paths import uri_to_path
...
class CodeMap:
    ...
    def neighborhood(
        ...
        *,
        ...
        for f in abs_files:
            ...
            unread = self._dangling_imports(
                raw_imports,
                roots,
                resolver,
                within=file_by_candidate,
                exclude_paths=scope,
            )
    ...
    async def _callers_for_symbol(self, path: str, sym, content_hash: str, lsp_query) -> list[str]:
        ...
        self._evict_stale(
            self._refs_cache,
            identity=(path, getattr(sym, "qualified_name", "")),
            keep=key,
        )
    ...
    @staticmethod
    def _group_by_resolver(
        abs_files: list[str],
    ) -> list[tuple[ModuleResolver, list[str]]]:
