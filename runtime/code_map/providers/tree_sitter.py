"""TreeSitterProvider — one :class:`LanguageProvider` serving every
...
thread-local parser from :mod:`~mote.runtime.code_map.ts_runtime`, runs a fresh
:class:`~mote.runtime.code_map.generic_builder.TreeSitterBuilder` over the tree,
...
from mote.runtime.code_map.generic_builder import TreeSitterBuilder, module_summary
from mote.runtime.code_map.providers.base import ModuleResolver, ProviderExtract
...
class TreeSitterProvider:
    ...
    def extract_tree(self, source: str, abspath: str) -> Optional[ProviderExtract]:
        ...
        with ts_runtime.native_tree_guard():
            return self._extract_tree_guarded(source, abspath)

    def _extract_tree_guarded(self, source: str, abspath: str) -> Optional[ProviderExtract]:
        try:
            tree = ts_runtime.parse(self._config.ts_name, source)
            if tree is None:
                return None
            root = tree.root_node
