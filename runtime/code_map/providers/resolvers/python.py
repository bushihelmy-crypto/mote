"""Python module ⇄ file arithmetic for the ``.py`` resolver.
...
(:mod:`mote.runtime.code_map` — ``_import_roots`` / ``_module_to_path`` /
``_module_candidates``) and the extractor (``_package_segments`` /
``_resolve_relative``), lifted behind the :class:`~mote.runtime.code_map.providers.base.ModuleResolver`
