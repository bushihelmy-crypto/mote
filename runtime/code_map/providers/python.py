...
that emits a :class:`~mote.runtime.code_map.scopes.ScopeGraph` + presentation
...
from mote.runtime.code_map.model import SUMMARY_MAX_CHARS, ImportBinding, ImportRef, Symbol
from mote.runtime.code_map.providers.base import LanguageProvider, ModuleResolver, ProviderExtract
from mote.runtime.code_map.providers.resolvers.python import PythonModuleResolver
...
class _ScopeGraphBuilder:
    ...
    def build(self, tree: ast.Module) -> tuple[ScopeGraph, list[Symbol]]:
        ...
        return (
            ScopeGraph(scopes=self.scopes, defs=self.defs, refs=self.refs),
            self.symbols,
        )
