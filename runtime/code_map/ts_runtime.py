"""tree-sitter runtime — the import-guarded loader for native grammars.
...
``tree_sitter.Parser`` is not safe to share across threads, so each execution
thread gets its own parser per grammar. An unknown grammar name (or a grammar
the pack cannot build) yields ``None`` and is remembered as absent so the
failing lookup is not retried every call.
"""
...
_native_tree_lock = threading.RLock()


@contextmanager
def native_tree_guard() -> Iterator[None]:
    """Serialize native-tree access and defer cyclic GC until nodes are released."""
    with _native_tree_lock:
...
def parse(name: str, source: str) -> Optional[Any]:
    """Parse *source* with grammar *name*, returning the owned tree or None."""
...
__all__ = ["available", "has_grammar", "native_tree_guard", "parser_for", "parse"]
