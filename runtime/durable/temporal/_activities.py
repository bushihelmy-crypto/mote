"""Runtime Temporal activity seam — the ONE generic durable step, run as an activity.

The architectural crux (verified against Pydantic AI's Temporal integration):
a Temporal **activity is a registered named function taking SERIALIZABLE args**,
never an arbitrary closure. mote's :class:`~mote.runtime.durable.backend.DurableBackend`
...
* The journal write (disk I/O) lives INSIDE the activity, never in workflow code
  ...
  :func:`~mote.runtime.ledger.run_journaled_step` body :class:`JsonlBackend` also
...
class StepActivities:
    ...
    async def run_step(self, step: StepInput) -> str:
        ...
        :func:`~mote.runtime.ledger.run_journaled_step`, the ONE side-effecting
