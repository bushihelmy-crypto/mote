"""ThinkJournal — recoverable model-think window over the durable backend.
...
:class:`~mote.contracts.schema.ThinkResult` in the shared run journal so a resume
...
from mote.contracts.events.types import JournalEvent
...
# functions above rather than living on ``ThinkJournal`` (whose backend the flow
...
class ThinkJournal:
    ...
    def begin_think(self, model_call_id: str | None = None) -> str:
        ...
        payload = (
            json.dumps({"model_call_id": model_call_id}, separators=(",", ":")) if model_call_id is not None else None
        )
    ...
    def resume_candidate(self) -> Optional[tuple[str, str]]:
        ...
        for rec in self.journal.records():
            if rec.kind != KIND_THINK or rec.status != STARTED:
                continue
            model_call_id = _started_model_call_id(rec.payload)
            if model_call_id is not None:
                return rec.step_id, model_call_id
        return None
...
def _started_model_call_id(payload: str | None) -> str | None:
    ...
    try:
        ...
        return None
    value = data.get("model_call_id") if isinstance(data, dict) else None
