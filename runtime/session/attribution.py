...
Where File Operations review events are the durable record (thin geometry +
attribution, no duplicated text) and :mod:`~mote.runtime.session.hunk_ops` is the
...
**Layering.** This is a pure session-layer reader over a ``ReviewService`` plus
a content-addressed artifact store; it imports neither
...
class HunkAttribution:
    """Read-side projection of one session's durable review events.
    ...
    def __init__(self, review: ReviewService, blobs) -> None:
        self._review = review
        self._blobs = blobs
    ...
    def hunks_by_source(self, source: str) -> list[HunkView]:
        ...
        return [self._view(r) for r in self._review.records() if r.source == source]

    def pending(self) -> list[HunkView]:
        """Every hunk still awaiting a review decision."""
        return [self._view(r) for r in self._review.pending()]
    ...
    def _view(self, rec: HunkRecord) -> HunkView:
        ...
        new = slice_lines(blob_text(self._blobs, rec.post_hash), *rec.new_range)
