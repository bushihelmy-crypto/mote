"""
Tool declaration collection and immutable Application catalogs.
...
The decorator collector is bootstrap-only. Runtime dispatch receives a frozen
``ToolCatalog`` through Product composition and never reads process state.
"""
...
class ToolRegistry:
    """Isolated registry for BaseTool subclasses."""
    ...
    def snapshot(self) -> ToolCatalog:
        """Freeze the current declarations into an isolated Application view."""

        return ToolCatalog.from_types(self.all_tools().values())


# Default catalog targeted by the Product's ``@register_tool`` decorators.
...
def declared_tool_catalog() -> ToolCatalog:
    """Freeze all declarations imported in this process."""
