...
thrashing. ToolResultPolicy owns the wiring and turns a verdict into a safe
in-band nudge, so this class can be unit-tested in complete isolation.
...
(one contrary outcome clears the relevant count), rather than depending on an
artificial turn-boundary reset.
"""
...
class ThrashDetector:
    ...
    Not thread-safe by design: ToolCallPolicy evaluates it inline on the single
    react-loop task, so records arrive serialized per Role.
    """
    ...
    def record(
        ...
        *,
        ...
        Args:
            ...
            success: The immutable tool-body execution outcome.
