...
config form). Legacy migration is fail-closed: a session whose old Artifact
index cannot be migrated is preserved. The current (or a just-resumed) session
is also excluded so a live run is never swept out from under itself.
...
from mote.runtime.artifacts.migration import LegacyArtifactMigrator
...
from mote.runtime.paths import WORKSPACE_CLEANUP_STAMP
...
class CleanupStats:
    ...
    artifact_revisions_migrated: int = 0
    artifact_publications_migrated: int = 0
    artifact_migration_failures: int = 0
...
def _migrate_legacy_artifacts(
    ...
    report = LegacyArtifactMigrator(store.root).migrate()
    stats.artifact_revisions_migrated += report.migrated_revisions
    stats.artifact_publications_migrated += report.migrated_publications
    stats.artifact_migration_failures += len(report.failures)
    for failure in report.failures:
        logger.warning(f"workspace cleanup: Artifact migration preserved source: {failure}")
    return frozenset(report.blocked_session_ids)
...
def _sweep_session(
    ...
    *,
    ...
    now: float,
    stats: CleanupStats,
    blocked_session_ids: frozenset[str],
) -> None:
...
def sweep_workspace(
    ...
    *,
    ...
    stats = CleanupStats()
    blocked_session_ids = _migrate_legacy_artifacts(store, stats)
...
async def sweep_workspace_async(
    ...
    *,
    ...
    now = time.time() if now is None else now
    stats = CleanupStats()
    blocked_session_ids = await run_disk_io(
