"""Runtime retry predicate and provider-error normalization.
...
into a typed :class:`~mote.contracts.model.provider_errors.LLMError` (status-driven, with
...
__all__ = ["is_retryable", "classify_llm_error"]
...
def _classify_api_status_error(exc: BaseException) -> MoteError | None:
    ...
    error so ``AttemptOrchestrator`` can honour the exact cool-off.
    """
