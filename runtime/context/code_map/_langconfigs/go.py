...
from mote.runtime.context.code_map._langconfigs._shared import iter_nodes, string_text
...
def go_imports(root: Any, source: bytes, abspath: str) -> "tuple[list[str], list[ImportRef], list[ImportBinding]]":
    ...
    for node in iter_nodes(root):
        ...
        if local in ("_", "."):
            continue
        refs.append(ImportRef(module=module, name=local, line=_line(pos), col=_col(pos), imported_name=""))
...
GO = LangConfig(
    ...
    field_access=FieldAccess(node_types=("selector_expression",), object_field="operand", member_field="field"),
