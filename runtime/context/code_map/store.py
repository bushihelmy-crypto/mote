...
from mote.runtime.context.code_map.model import CallEdge, FileExtract, ImportBinding, Symbol
...
_MIGRATIONS = (
    ...
    # Multi-language columns: the language a provider identified, and the
    # resolver's class-skip policy for the file (Python semantics = 1). An old
    # DB reads these back as '' / 1 — i.e. Python behavior — so the migration is
    # backwards-compatible.
    ("files", "language", "TEXT NOT NULL DEFAULT ''"),
    ("files", "skip_class_scope", "INTEGER NOT NULL DEFAULT 1"),
)
...
class CodeMapStore:
    ...
    def _migrate(self) -> None:
        """Bring a persisted older-build DB up to the current schema (idempotent).

        Two migrations, both safe to run on every open:

        - **summary columns** — a DB from before the summary columns shipped
          lacks them; ``ADD COLUMN`` adds them (a duplicate-column
          ``OperationalError`` on an already-migrated DB is swallowed).
        - **edges → imports/calls** — the original build stored imports and calls
          together in a single ``edges`` table keyed by a ``kind`` column. If that
          table still exists, fold its rows into the dedicated ``imports`` /
          ``calls`` tables once and drop it, so a warm store keeps its reverse-dep
          data across the upgrade instead of going blind until the next edit.
        """
    ...
    def _migrate_edges(self) -> None:
        """Fold a legacy ``edges`` table into ``imports`` / ``calls``, then drop it."""
        try:
        ...
        try:
            self._conn.execute(
                "INSERT INTO imports (source_file, module) "
                "SELECT source_file, target FROM edges WHERE kind = 'imports' ORDER BY rowid"
            )
            self._conn.execute(
                "INSERT INTO calls (source_file, caller, callee, line) "
                "SELECT source_file, caller, target, line FROM edges WHERE kind = 'calls'"
            )
            self._conn.execute("DROP TABLE edges")
        except sqlite3.OperationalError:
            pass  # partial/odd legacy shape — leave it; a re-index repopulates
    ...
    def upsert_file(self, extract: FileExtract) -> None:
        ...
        cur.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?)",
            (path, extract.content_hash, int(time.time()), extract.module_summary, extract.language, skip_class),
        )
        self._conn.commit()
    ...
    def _write_graph(self, cur: sqlite3.Cursor, path: str, graph: Optional[ScopeGraph]) -> None:
        ...
        for ref in graph.refs:
            ...
            ref_rows.append(
                (path, ref.name, ref.scope, ref.line, ref.col, int(ref.is_call), int(ref.via_self), rsid, rline)
            )
        cur.executemany(
    ...
    def graph_of(self, file_path: str) -> ScopeGraph:
        ...
        refs = [
            Ref(name=r[0], scope=r[1], line=r[2], col=r[3], is_call=bool(r[4]), via_self=bool(r[5])) for r in ref_rows
        ]
