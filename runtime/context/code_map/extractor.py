"""CodeMapExtractor — the language-agnostic dispatch facade over the providers.
...
:class:`~mote.runtime.context.code_map.providers.base.LanguageProvider`; the facade picks
one by extension (:func:`~mote.runtime.context.code_map.languages.provider_for`), hands it
the already-read source, and wraps the neutral
:class:`~mote.runtime.context.code_map.providers.base.ProviderExtract` it returns into a
:class:`~mote.runtime.context.code_map.model.FileExtract` — adding the path, content hash,
language tag, and the one shared call-edge step (derived from the resolved
:class:`~mote.runtime.context.code_map.scopes.ScopeGraph`, identical for every language).
...
The neutral dataclasses are re-exported here so long-standing imports
(``from ...extractor import Symbol``) keep working after the model move.
"""
...
from mote.contracts.text import content_hash as _content_hash
from mote.runtime.context.code_map.languages import provider_for
from mote.runtime.context.code_map.model import CallEdge, FileExtract, ImportBinding, ImportRef, Symbol
from mote.runtime.context.code_map.scopes import ScopeGraph
from mote.runtime.disk import mtime_ns
