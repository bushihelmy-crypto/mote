"""ContextVisibility — the authority on stored-result visibility.

A *reconstructable* tool result (Read, Search, ...) may have its body folded
away in place (:mod:`mote.runtime.context.compaction.reducers.fold` replaces the content
with :data:`TOOL_RESULT_CLEARED_MESSAGE`) or pair-erased outright
(:mod:`~mote.runtime.context.compaction.reducers.erase` removes the message). Both are
...
- **Read-only, no back-reference.** It only reads messages. The ContextManager
  ...
  File Operations snapshot capabilities), so the executor→context dependency direction is
  never inverted.
"""
...
from mote.contracts.conversation.constants import TOOL_RESULT_CLEARED_MESSAGE
from mote.contracts.conversation.fields import TOOL_CALL_ID, TOOL_RESULT_RESOURCE_PATH
