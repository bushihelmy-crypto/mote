...
def _wire_to_message(d: dict) -> Message:
    ...
    if tool_call_id:
        ...
        return _restore_private_metadata(m, d)
...
def _restore_private_metadata(m: Message, d: dict) -> Message:
    intent = d.get("_cache_intent")
    if intent:
        m.metadata[CACHE_INTENT] = intent
    references = d.get("_tool_references")
    if references:
        m.metadata[TOOL_REFERENCES] = references
