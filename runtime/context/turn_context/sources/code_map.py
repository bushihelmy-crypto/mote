...
:class:`~mote.runtime.context.code_map.CodeMap` it drives lives in the *same* layer,
...
from mote.contracts.text import display_path
from mote.runtime.context.code_map import CodeMap, FileNeighborhood
...
class CodeMapContextSource:
    ...
    def _refresh_in_context(self, neighborhoods: list) -> None:
        ...
        A file counts as *in context* while its exact observed version remains
        registered — the body the model saw is still accurate, so
        ...
        its observation was invalidated (edited since last read — Feature 3 wants to
        ...
        # defines once, then let the observation-derived frontier resume next turn.
        ...
        for nb in neighborhoods:
            if read_state.get(nb.path) is None:
                continue  # never read (dependency-only) → show defines
            in_context.add(nb.path)
