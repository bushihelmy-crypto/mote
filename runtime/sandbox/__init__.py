"""mote.runtime.sandbox — OS-level sandbox runtime (the *runtime* layer).
...
``mote.runtime.tools.permission.sandbox``). This package only depends on
lower-layer contracts and kernel APIs (plus the stdlib), mirroring the Runtime
...
from mote.runtime.sandbox.violations import SandboxViolation
