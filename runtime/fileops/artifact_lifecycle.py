...
@dataclass(frozen=True, slots=True)
class ArtifactGarbageCollectionStatus:
    state: ArtifactGarbageCollectionState
...
class ArtifactLifecycleHealth:
    ...
    garbage_collection: ArtifactGarbageCollectionStatus
...
_SCHEMA = (
    ...
    CREATE TABLE garbage_collection_status (
        ...
        state TEXT NOT NULL CHECK (
            state IN ('never_run', 'succeeded', 'failed')
        ),
        completed_at_ns INTEGER CHECK (
            completed_at_ns IS NULL OR completed_at_ns >= 0
        ),
        quarantined_objects INTEGER NOT NULL CHECK (quarantined_objects >= 0),
        restored_objects INTEGER NOT NULL CHECK (restored_objects >= 0),
        deletion_candidates INTEGER NOT NULL CHECK (deletion_candidates >= 0),
        reclaimed_objects INTEGER NOT NULL CHECK (reclaimed_objects >= 0),
        reclaimed_bytes INTEGER NOT NULL CHECK (reclaimed_bytes >= 0),
        failure TEXT NOT NULL,
        CHECK (
            (state = 'never_run' AND completed_at_ns IS NULL AND
             quarantined_objects = 0 AND restored_objects = 0 AND
             deletion_candidates = 0 AND reclaimed_objects = 0 AND
             reclaimed_bytes = 0 AND failure = '') OR
            (state = 'succeeded' AND completed_at_ns IS NOT NULL AND
             failure = '') OR
            (state = 'failed' AND completed_at_ns IS NOT NULL AND
             length(failure) > 0)
        )
    ) STRICT
    """,
...
_EXPECTED_COLUMNS = {
    ...
    "garbage_collection_status": (
        ("singleton", "INTEGER", 0, 1),
        ("state", "TEXT", 1, 0),
        ("completed_at_ns", "INTEGER", 0, 0),
        ("quarantined_objects", "INTEGER", 1, 0),
        ("restored_objects", "INTEGER", 1, 0),
        ("deletion_candidates", "INTEGER", 1, 0),
        ("reclaimed_objects", "INTEGER", 1, 0),
        ("reclaimed_bytes", "INTEGER", 1, 0),
        ("failure", "TEXT", 1, 0),
    ),
    "reservations": (
...
class ArtifactLifecycleCatalog:
    ...
    def health(self) -> ArtifactLifecycleHealth:
        with self._read_connection() as connection:
            ...
            garbage_collection = self._garbage_collection_status(connection)
    ...
    def record_garbage_collection_success(
        self,
        *,
        quarantined_objects: int,
        restored_objects: int,
        ...
        reclaimed_bytes: int,
        now_ns: int | None = None,
    ) -> ArtifactGarbageCollectionStatus:
        counts = (
            quarantined_objects,
            restored_objects,
            deletion_candidates,
        ...
        for value in counts:
            self._validate_bytes(value, "garbage collection count")
        with self._transaction() as connection:
            completed_at = self._advance_clock(connection, now_ns)
            connection.execute(
                """
                UPDATE garbage_collection_status
                SET state = ?, completed_at_ns = ?, quarantined_objects = ?,
                    restored_objects = ?, deletion_candidates = ?,
                    reclaimed_objects = ?, reclaimed_bytes = ?, failure = ''
                WHERE singleton = 1
                """,
                (
                    ArtifactGarbageCollectionState.SUCCEEDED.value,
                    completed_at,
                    *counts,
                ),
            )
            return self._garbage_collection_status(connection)

    def record_garbage_collection_failure(
        self,
        failure: str,
        *,
        now_ns: int | None = None,
        ...
        if type(failure) is not str or not failure:
            raise ValueError("garbage collection failure must be non-empty")
        if len(failure.encode("utf-8", errors="strict")) > 4_096:
            raise ValueError("garbage collection failure exceeds its size limit")
        with self._transaction() as connection:
            completed_at = self._advance_clock(connection, now_ns)
            connection.execute(
                """
                UPDATE garbage_collection_status
                SET state = ?, completed_at_ns = ?, quarantined_objects = 0,
                    restored_objects = 0, deletion_candidates = 0,
                    reclaimed_objects = 0, reclaimed_bytes = 0, failure = ?
                WHERE singleton = 1
                """,
                (
                    ArtifactGarbageCollectionState.FAILED.value,
                    completed_at,
                    failure,
                ),
            )
    ...
    def reservation_objects(
        ...
        with self._read_connection() as connection:
            ...
            return tuple(
                ArtifactObject(
                    BlobRef(digest=row["digest"], size=row["size"]),
    ...
    def reconcile_quarantine(
        self,
        protected_artifacts: Iterable[BlobRef],
        *,
        expected_generation: int,
        minimum_age_ns: int,
        ...
        with self._transaction() as connection:
            ...
            for row in rows:
                artifact = BlobRef(digest=row["digest"], size=row["size"])
    ...
    def complete_deletion(self, artifact: BlobRef) -> None:
        self._validate_ref(artifact)
        with self._transaction() as connection:
            current = self._object(connection, artifact.digest)
        ...
        try:
            with self._connect() as connection:
                ...
                if version == 0 and not tables:
                    ...
                    connection.execute(
                        """
                        INSERT INTO garbage_collection_status (
                            singleton, state, completed_at_ns,
                            quarantined_objects, restored_objects,
                            deletion_candidates, reclaimed_objects,
                            reclaimed_bytes, failure
                        ) VALUES (1, ?, NULL, 0, 0, 0, 0, 0, '')
                        """,
                        (ArtifactGarbageCollectionState.NEVER_RUN.value,),
                    )
    ...
    def _validate_schema(self, connection: sqlite3.Connection) -> None:
        ...
        self._garbage_collection_status(connection)
    ...
    @classmethod
    def _canonical_refs(cls, artifacts: Iterable[BlobRef]) -> tuple[BlobRef, ...]:
    ...
    @staticmethod
    def _garbage_collection_status(
        connection: sqlite3.Connection,
    ) -> ArtifactGarbageCollectionStatus:
        rows = connection.execute(
            """
            SELECT state, completed_at_ns, quarantined_objects,
                   restored_objects, deletion_candidates, reclaimed_objects,
                   reclaimed_bytes, failure
            FROM garbage_collection_status
            """
        ).fetchall()
        if len(rows) != 1:
            raise ArtifactLifecycleConflictError("artifact garbage collection status is not canonical")
        row = rows[0]
        try:
            state = ArtifactGarbageCollectionState(row["state"])
        except ValueError as exc:
            raise ArtifactLifecycleConflictError("artifact garbage collection state is invalid") from exc
        return ArtifactGarbageCollectionStatus(
            state=state,
            completed_at_ns=row["completed_at_ns"],
            quarantined_objects=row["quarantined_objects"],
            restored_objects=row["restored_objects"],
            deletion_candidates=row["deletion_candidates"],
            reclaimed_objects=row["reclaimed_objects"],
    ...
    def _stage(connection: sqlite3.Connection, stage_id: str) -> ArtifactStage:
        ...
        artifact = None if row["digest"] is None else BlobRef(digest=row["digest"], size=row["size"])
        return ArtifactStage(
