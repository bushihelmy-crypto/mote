...
class FileMutationArtifactRepository:
    ...
    def __init__(
        ...
        *,
        ...
        self.incoming_root = Path(lifecycle_root) / ".incoming"
    ...
    def put(self, stage: ArtifactStage, chunks: Iterable[bytes]) -> ContentIdentity:
    ...
    def resolve_live(self, digest: str) -> ContentIdentity:
        if (
    ...
    def open_verified(self, artifact: ContentIdentity) -> "VerifiedArtifactStream":
        self._validate_ref(artifact)
        return VerifiedArtifactStream(self, artifact)

    def read_bytes(self, artifact: ContentIdentity) -> bytes:
        data = bytearray()
    ...
    def read_bounded(self, artifact: ContentIdentity, *, maximum_bytes: int) -> bytes:
    ...
    def iter_lines(self, artifact: ContentIdentity) -> Iterator[bytes]:
    ...
    def verify(self, artifact: ContentIdentity) -> None:
        with self.open_verified(artifact) as stream:
    ...
    def _publish_capture(
        ...
        try:
            with self.locks.acquire_many((self._artifact_lock(artifact, LockMode.EXCLUSIVE),)):
                self.content_repository.adopt_file(temp_path, self._content_ref(artifact))
    ...
    def _record_after_seal(self, stage: ArtifactStage, artifact: ContentIdentity) -> None:
        try:
    ...
    def _require_live(self, artifact: ContentIdentity) -> None:
        self._validate_ref(artifact)
    ...
    def _payload_path(self, artifact: ContentIdentity) -> Path:
        return self.content_repository.payload_path(self._content_ref(artifact))
    ...
    @staticmethod
    def _artifact_lock(artifact: ContentIdentity, mode: LockMode) -> LockSpec:
        return LockSpec(
            ARTIFACT_LOCK_LEVEL,
            artifact.digest,
            mode,
    ...
    @staticmethod
    def _verify_observed(artifact: ContentIdentity, size: int, digest: str) -> None:
    ...
    def _validate_ref(artifact: ContentIdentity) -> None:
        if type(artifact) is not ContentIdentity:
...
class ArtifactWriteScope(AbstractContextManager["ArtifactWriteScope"]):
    ...
    def __init__(
        ...
        *,
        ...
        self._artifacts: list[ContentIdentity] = []
    ...
    @property
    def artifacts(self) -> tuple[ContentIdentity, ...]:
    ...
    def put_bytes(self, data: bytes) -> ContentIdentity:
