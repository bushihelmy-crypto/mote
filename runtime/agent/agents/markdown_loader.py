...
def _build_agent_class(name: str, meta: dict, body: str) -> Optional[type[BaseAgent]]:
    ...
    class _MarkdownAgent(BaseAgent, Role):
        ...
        def __init__(self, *, parent_session_id: Optional[str] = None, wiring=None, config=None, **_ignored):
            schema_kwargs: dict = {
                "name": name,
            ...
            if model and child_config is not None:
                try:
                    ...
                    child_config = config
            Role.__init__(self, role_schema=schema, state=state, wiring=wiring, config=child_config)
