...
class ReportingIdentity(Protocol):
    name: str
...
class _StreamQueueSubscriber:
    """Telemetry handler that forwards streamed tokens into an ``asyncio.Queue``.

    Registered while a reporter's async streaming context is open; ``None`` ends
    the reporter's drain task.
    """
    ...
    def handle_sync(self, event) -> None:
        if isinstance(event, LLMStreamDeltaEvent):
            if event.provisional and event.attempt_id:
                self._provisional.setdefault(event.attempt_id, []).append(event.token)
                return
            self._queue.put_nowait(event.token)
        elif isinstance(event, LLMStreamCommittedEvent):
            for token in self._provisional.pop(event.attempt_id, []):
                self._queue.put_nowait(token)
        elif isinstance(event, (LLMStreamDiscardedEvent, LLMStreamInterruptedEvent)):
            self._provisional.pop(event.attempt_id, None)
...
CURRENT_ROLE: ContextVar[ReportingIdentity] = ContextVar("role")
...
class ResourceReporter(BaseModel):
    ...
    callback_url: str = Field(
        default=MOTE_REPORTER_DEFAULT_URL,
    ...
    _llm_handle: Optional[TelemetryHandle] = PrivateAttr(None)
    ...
    async def __aenter__(self):
        ...
        Subscribes a bounded handler to the active telemetry runtime so streamed
        LLM tokens are drained to this reporter. Standalone use without a bound
        runtime is a no-op.
        """
    ...
    async def __aexit__(self, exc_type, exc_value, exc_tb):
        ...
        if self._llm_handle is not None:
            await self._llm_handle.aclose()
        if self._llm_task is not None and self._llm_queue is not None:
...
class ReporterSubscriber:
    """Telemetry handler that POSTs :class:`ResourceReportEvent`\\s to a UI endpoint.
    ...
    a failed push is swallowed because UI mirroring must never break a turn.
    Standalone use with no telemetry runtime simply never POSTs.
    """
