"""Single-process resource admission planes with exactly-once permit settlement."""
...
from mote.contracts.ports.model_operator import ModelOperatorAuditStore
...
class ResourceAdmissionController:
        ...
        *,
        ...
        if credential_until > now:
            return self._reject(
                ...
                retry_after_seconds=credential_until - now,
            )
        ...
        if quota_until > now:
            return self._reject(
                ...
                FailureDomain.TRANSPORT,
                Retryability.AFTER_CHANGE,
                health_verdict=HealthVerdict.QUOTA_LIMITED,
                retry_after_seconds=quota_until - now,
            )
    ...
    def _settle(
        ...
        *,
        ...
        now = self._clock()
        if verdict is HealthVerdict.AVAILABILITY_FAILURE:
            availability_permit.fail()
        else:
            availability_permit.abandon()
        if verdict is HealthVerdict.CREDENTIAL_REJECTED:
            self._credential_quarantine[credential_key] = now + (
                disposition.retry_after_seconds or self._credential_quarantine_seconds
            )
        elif verdict is HealthVerdict.QUOTA_LIMITED:
            self._quota_cooldown[quota_key] = now + (
                disposition.retry_after_seconds or self._default_quota_cooldown_seconds
            )
    ...
    def _reject_quota_exhausted(
        ...
        retry_after = (
            reset_at - now if reset_at is not None and reset_at > now else self._default_quota_cooldown_seconds
        )
        return self._reject(
            AdmissionGate.QUOTA,
            reason,
