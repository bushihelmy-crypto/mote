"""Runtime token-usage accounting — a synthesis of Codex and Anthropic usage.

``TokenUsage`` is the single normalized record every provider's raw usage is
mapped into before it reaches the Runtime :class:`CostTracker`.
...
class TokenUsage:
    ...
    @classmethod
    def from_input_output(cls, usage: Any) -> "TokenUsage":
        """Map Responses-style ``input_tokens``/``output_tokens`` usage."""

        input_tokens = _as_int(_get(usage, "input_tokens"))
        output_tokens = _as_int(_get(usage, "output_tokens"))
        total = _as_int(_get(usage, "total_tokens")) or (input_tokens + output_tokens)
        cached = _as_int(_get(_get(usage, "input_tokens_details", None), "cached_tokens"))
        reasoning = _as_int(_get(_get(usage, "output_tokens_details", None), "reasoning_tokens"))
        return cls(
    ...
    def from_usage(cls, usage: Union[Mapping, Any, None]) -> "TokenUsage":
        ...
        has_input = _get(usage, "input_tokens", None) is not None
        has_anthropic_cache = (
        ...
        if not has_prompt and has_input:
            return cls.from_input_output(usage)
