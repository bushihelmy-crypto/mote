"""Runtime RateLimitSnapshot — one provider quota as of the last response.
...
def _parse_reset_after_seconds(
    value: Optional[str],
    *,
    observed_at: float,
) -> Optional[float]:
    """Normalize an RFC-3339 reset timestamp to a non-negative duration."""

    if value is None:
        return None
    try:
        reset_at = datetime.fromisoformat(value.strip().replace("Z", "+00:00"))
        return max(reset_at.timestamp() - observed_at, 0.0)
...
class RateLimitSnapshot:
    ...
    @property
    def normalized_reset_requests_seconds(self) -> Optional[float]:
        if self.reset_requests_seconds is not None:
            return self.reset_requests_seconds
        return _parse_reset_after_seconds(
            self.reset_requests_raw,
            observed_at=self.observed_at,
        )

    @property
    def normalized_reset_tokens_seconds(self) -> Optional[float]:
        if self.reset_tokens_seconds is not None:
            return self.reset_tokens_seconds
        return _parse_reset_after_seconds(
            self.reset_tokens_raw,
            observed_at=self.observed_at,
        )
