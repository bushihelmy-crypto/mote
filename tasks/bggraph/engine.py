"""Frontier scheduler for :mod:`metagpt.tasks.bggraph`.
...
from metagpt.tasks.bggraph.report import report_progress
...
async def _run_one_node(node_name: str, state: Any, graph: Any, completed: set) -> None:
    ...
    node_def.status = NodeStatus.RUNNING
    report_progress(node_name, NodeStatus.RUNNING, node_def.description or None)

    max_attempts = 1 + node_def.auto_retries
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if stage.poll is not None:
                result = await _run_poll(stage, submit_result)
            else:
                result = submit_result

            setattr(state, node_name, result)
            completed.add(node_name)
            node_def.status = NodeStatus.COMPLETED
            if attempt > 0:
                detail = _MSG_COMPLETED_WITH_RETRY.format(result=result, auto_retries=attempt)
            else:
                detail = result
            report_progress(node_name, NodeStatus.COMPLETED, detail)
            return

        except asyncio.CancelledError:
            node_def.status = NodeStatus.CANCELLED
            report_progress(node_name, NodeStatus.CANCELLED)
            raise
            ...
            if attempt + 1 < max_attempts:
                report_progress(
                    ...
                    _MSG_RETRYING.format(
                        attempt=attempt + 1,
                        auto_retries=node_def.auto_retries,
                        error_type=type(e).__name__,
                        error=e,
                    ),
                )
                await asyncio.sleep(node_def.retry_wait)
            else:
                detail = (
                ...
                report_progress(node_name, NodeStatus.FAILED, detail)
                node_def.status = NodeStatus.FAILED
                e._auto_retries_attempted = attempt
                e._auto_retries_limit = node_def.auto_retries
                raise
...
def successors(node: str, graph: Any, state: Any, completed: set, trigger_count: dict) -> list[str]:
    ...
    Raises :class:`_LlmPauseSignal` for an LLM edge and :class:`RouterError`
    ...
    for ce in graph._conditional_edges:
        ...
        try:
            ...
            raise RouterError(f"Router on '{node}' raised {type(e).__name__}: {e}") from e
...
async def _run_driver(
    ...
    *,
    ...
    try:
        ...
        while running:
            ...
            for t in done:
                ...
                if exc is not None:
                    ...
                    prior = [(nm, e) for nm, e in all_errors if nm != n]
                    push_node_failure_notification(
                        n,
                        exc,
                        state,
        ...
        return _LLM_ROUTE_SENTINEL
    except (GraphRecursionError, RouterError) as e:
        await _cancel_running(running)
        fatal = e

    if fatal is not None:
        push_terminal_notification(
            graph, state, NodeStatus.FAILED, error=fatal, initial_params=initial_params
        )
        raise fatal

    if all_errors:
        error = BatchFailureError(all_errors)
        push_terminal_notification(
            graph, state, NodeStatus.FAILED, error=error, initial_params=initial_params
        )
        raise error

    result = _collect_finish_result(graph, state)
    push_terminal_notification(
        graph, state, NodeStatus.COMPLETED, result=result, initial_params=initial_params
    )
    return result
...
def resume(graph: Any, state: Any, from_nodes: list[str]) -> BgTaskResult:
    ...
    for node_name in from_nodes:
        ...
        graph._nodes[node_name].status = NodeStatus.PENDING
...
def _apply_skip(graph: Any, state: Any, skip_nodes: list[str]) -> None:
    for sn in skip_nodes:
        ...
        if existing is None:
            setattr(state, sn, {})
        node_def.status = NodeStatus.SKIPPED
        suffix = " with partial results" if existing is not None else ""
        report_progress(
            sn, NodeStatus.SKIPPED, _MSG_SKIPPING.format(skip_nodes=sn, suffix=suffix)
        )
