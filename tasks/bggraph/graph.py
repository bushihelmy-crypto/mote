...
class BgGraph:
    ...
    def node(
        ...
        def decorator(fn):
            self.add_node(name, fn, params=params, auto_retries=auto_retries, retry_wait=retry_wait)
            return fn
    ...
    def compile(self) -> Callable[..., Awaitable[BgTaskResult]]:
        ...
        from metagpt.tasks.bggraph.engine import _build_executor
    ...
    def resume(self, state: GraphState, from_nodes: list[str]) -> BgTaskResult:
        from metagpt.tasks.bggraph.engine import resume as _resume

        return _resume(self, state, from_nodes)

    def resume_skip(self, state: GraphState, skip_nodes: list[str]) -> BgTaskResult:
        from metagpt.tasks.bggraph.engine import resume_skip as _resume_skip
    ...
    def resume_skip_and_from(
        ...
        from metagpt.tasks.bggraph.engine import resume_skip_and_from as _rsaf
