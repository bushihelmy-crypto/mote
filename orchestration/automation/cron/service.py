...
from mote.orchestration.automation import AutomationTrigger, TriggerSink
...
from mote.orchestration.automation.cron.task import CronJitterConfig, CronTask
...
class CronService:
    ...
    def _on_fire(self, task: CronTask) -> None:
        ...
        trigger = AutomationTrigger(
            trigger_id=f"cron:{task.id}:{fired_at}",
            source_id=task.id,
            target=target,
            content=task.prompt,
            scheduled_at_ms=task.last_fired_at or task.created_at,
            fired_at_ms=fired_at,
        )
        try:
            self._trigger_sink.dispatch(trigger)
