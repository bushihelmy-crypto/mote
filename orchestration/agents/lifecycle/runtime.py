...
``ExecutionEngine`` stays untouched.
...
from mote.contracts.output import RunOutcome, RunRejected
...
class AgentRuntime(Generic[OutputT]):
    ...
    def __init__(
        ...
        *,
        ...
        self.last_run_result: RunOutcome[OutputT] | None = None
    ...
    # Turn execution
    # ------------------------------------------------------------------
    async def run_one_turn(self, with_message: Message | None = None) -> RunOutcome[OutputT] | None:
        ...
        async with self._lock:
            ...
            try:
                ...
                self.status = AgentStatus.REJECTED if isinstance(rsp, RunRejected) else AgentStatus.COMPLETED
