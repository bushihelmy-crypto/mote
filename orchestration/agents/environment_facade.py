...
This is the bridge between the ``BaseEnvironment`` facade
...
from mote.orchestration.agents.base_environment import BaseEnvironment
...
from mote.runtime.errors import AgentNotFound
...
class AgentEnvironment(BaseEnvironment):
        ...
        if self._control is None:
            self._control = AgentControl(
                store=ResidencyStore(
                    base_dir=str(self.residency_dir),
                    sessions_base_dir=str(self.sessions_dir),
                )
            )
    ...
    def add_role(self, role: Any) -> Any:
        ...
        # Keep the explicit plane reference session-local. Engine Context is
        # shared across sessions and must never carry mutable control state.
        role.bind_agent_control(self.control)
    ...
    async def run_ready_turns(self, max_turns: int = 1) -> int:
        """Run up to ``max_turns`` ready agent turns."""
        return await self.control.run_ready_turns(max_turns)
    ...
    async def ask_user_question(self, questions: Any, sent_from: Optional[Any] = None) -> Any:
        ...
        The production front-end is ``PortHumanChannel`` (mote.product.cli), which
    ...
    async def request_approval(self, request: Any, sent_from: Optional[Any] = None) -> str:
        ...
        Returns the ``"deny"`` :data:`~mote.contracts.interaction.ApprovalChoice`
