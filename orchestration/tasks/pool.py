...
from mote.contracts.schema import CauseBy, MessagePriority, ToolResultLimitConfig
...
if TYPE_CHECKING:
    from mote.contracts.ports import MessageActivity, MessageSink
...
from mote.orchestration.tasks.bggraph.types import GraphPause, PauseReason
from mote.orchestration.tasks.constants import DEFAULT_MAX_CONCURRENCY as _DEFAULT_MAX_CONCURRENCY
from mote.orchestration.tasks.constants import DEFAULT_TASK_TIMEOUT as _DEFAULT_TASK_TIMEOUT
from mote.orchestration.tasks.constants import DEFAULT_WAIT_COMPLETION_TIMEOUT as _DEFAULT_WAIT_COMPLETION_TIMEOUT
from mote.orchestration.tasks.constants import MAX_TASK_OUTPUT_BYTES_DISPLAY as _OUTPUT_CAP_DISPLAY
...
class BackgroundTaskPool:
    ...
    def submit(
        ...
        if progress and self._output_store is not None:
            ...
            coro = self._with_progress(coro, task_id, current_telemetry())
