...
**not** ``AgentFlowEngine`` — drains the mailbox at the turn boundary (between
...
Two drive modes (don't mix them on the same scheduler instance):
  ...
  * :meth:`run` — a bounded "barrier pump" that synchronously advances every
    ready runtime up to ``k`` rounds (the ``env.run(k)`` compat shim).
"""
...
class EventDrivenScheduler:
    ...
    # ------------------------------------------------------------------
    # Bounded barrier pump (env.run(k) compat)
    # ------------------------------------------------------------------
