...
from mote.contracts.policy.spawn import SpawnIntent
...
from mote.orchestration.environment.registry import AgentMetadata, AgentRegistry, next_agent_spawn_depth
from mote.orchestration.environment.residency import Residency, ResidencySlot
from mote.orchestration.environment.runtime import AgentRuntime, AgentStatus, is_final
...
class _ContextualRole(Protocol):
    """The narrow context-provisioning surface required from a spawned role.

    Structural only — keeps the ``environment`` layer from importing the Role;
    the Runtime Role decides how ownership is represented in its wiring value.
    """

    agent_control: object
...
class AgentControl:
    ...
    def _role_cost_tracker(role: object) -> Optional[CostTracker]:
        ...
        wiring = getattr(role, "wiring", None)
        ctx = getattr(wiring, "context", None)
        if ctx is None:
            return None
    ...
    async def spawn_agent(self, spec: SpawnSpec) -> ChildAgentHandle:
        ...
        if agent_id is None:
            slot.rollback()
            raise RuntimeError("spawn completed without an Agent session identity")
    ...
    def _build_spawn_context(self, spec: SpawnSpec, child_path: AgentPath) -> SpawnContext:
        ...
        if parent_rt is not None:
            ...
            parent_wiring = getattr(parent_role, "wiring", None)
            parent_services = getattr(parent_wiring, "services", None)
            parent_ctx = getattr(parent_services, "context", None)
    ...
    def _provision_context(self, role: _ContextualRole, spec: SpawnSpec, spawn_ctx: SpawnContext) -> None:
        ...
        if spec.context_policy is ContextPolicy.SHARE_PARENT:
            ...
            parent_wiring = getattr(parent_rt.role, "wiring", None)
            parent_services = getattr(parent_wiring, "services", None)
            if parent_services is None:
                raise RuntimeError("SHARE_PARENT spawn parent has no provisioned EngineServices.")
            role.bind_services(parent_services, owned=False)
            return
        # FRESH owns a new tracker but inherits the parent's provider capability.
        # Provider availability is a composition decision, never an import-order
        # side effect that a child is expected to rediscover globally.
        parent_rt = self._runtimes.get(spec.parent_id) if spec.parent_id else None
        parent_role = getattr(parent_rt, "role", None)
        parent_wiring = getattr(parent_role, "wiring", None)
        parent_services = getattr(parent_wiring, "services", None)
        parent_context = getattr(parent_services, "context", None)
        provider_factory = getattr(parent_context, "provider_factory", None)
        maintenance_coordinator = getattr(parent_context, "maintenance_coordinator", None)
        service_gateway = getattr(parent_context, "service_gateway", None)
        kwargs = {}
        if spawn_ctx.config is not None:
            kwargs["config"] = spawn_ctx.config
        if provider_factory is not None:
            kwargs["provider_factory"] = provider_factory
        if maintenance_coordinator is not None:
            kwargs["maintenance_coordinator"] = maintenance_coordinator
        if service_gateway is not None:
            kwargs["service_gateway"] = service_gateway
        role.bind_services(
            EngineServices(
                context=Context(**kwargs),
                run_lease_coordinator=getattr(parent_services, "run_lease_coordinator", None),
            ),
            owned=True,
        )
    ...
    def _install_rehydrated(self, agent_id: str, slot: ResidencySlot) -> Optional[AgentRuntime]:
        ...
        if restored is None:
            slot.rollback()
            raise AgentNotFound(agent_id)
        restored.role.agent_control = self
