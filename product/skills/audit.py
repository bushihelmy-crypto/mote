...
:mod:`mote.product.skills.reserved_tokens` — the BODY was never screened. This
...
* ``injection`` — model-reserved prompt control tokens;
...
from mote.product.skills.reserved_tokens import RESERVED_MODEL_TOKEN_PATTERN
...
def _scan_injection(text: str) -> list[Finding]:
    return [
        Finding(
            "injection",
            Severity.CRITICAL,
            f"prompt-injection token {m.group(0)!r}",
            _lineno(text, m.start()),
        )
        for m in RESERVED_MODEL_TOKEN_PATTERN.finditer(text)
    ]
            ...
            if (func.value.id, func.attr) in _PY_DANGER_ATTRS:
                findings.append(
                    Finding(
                        "code",
                        Severity.WARNING,
                        f"Python {func.value.id}.{func.attr}() call",
                        line,
                    )
                )
    return findings
    ...
    for m in _FENCE_RE.finditer(text):
        ...
        if lang in _SHELL_LANGS:
            for severity, patterns in (
                (Severity.CRITICAL, _SHELL_CRITICAL),
                (Severity.WARNING, _SHELL_WARNING),
            ):
