...
from mote.contracts.models.tokenization import count_message_tokens, count_string_tokens, get_max_completion_tokens
...
class OpenAILLM(CredentialBindingMixin, BaseLLM):
    ...
    async def _achat_completion_stream_tool(
        ...
        **chat_configs,
        ...
        try:
            response: AsyncStream[ChatCompletionChunk] = await self.aclient.chat.completions.create(
                **self._cons_kwargs(messages, timeout=self.get_timeout(timeout), **chat_configs),
                stream=True,
            )
            async for chunk in response:
    ...
    def _get_max_tokens(self, messages: list[dict]):
        ...
        return min(
            get_max_completion_tokens(messages, self.model, self.max_completion_token),
            4096,
        )
