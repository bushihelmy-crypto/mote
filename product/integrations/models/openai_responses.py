# -*- coding: utf-8 -*-
"""Product integration for the native OpenAI Responses API.
...
try:
    ...
    AsyncHttpxClientWrapper = None

try:
    from openai.types.responses import ResponseCompletedEvent, ResponseTextDeltaEvent
except ImportError:  # pragma: no cover - optional streaming types
    ResponseCompletedEvent = None
    ResponseTextDeltaEvent = None

try:
    from json_repair import repair_json
except ImportError:  # pragma: no cover - malformed-JSON recovery extra unavailable
    repair_json = None
from mote.contracts.config.llm import LLMConfig
from mote.contracts.models import WebSearchHit
from mote.contracts.models.capabilities import supports_web_search
from mote.contracts.models.constants import USE_CONFIG_TIMEOUT
...
class OpenAIResponsesLLM(CredentialBindingMixin, BaseLLM):
    ...
    def _rebuild_client(self) -> AsyncOpenAIClient:
        if AsyncOpenAI is None:
            raise RuntimeError("OpenAI Responses support requires the 'openai' optional dependency.")
    ...
    def _convert_messages(
        ...
        for msg in messages:
            ...
            if role == "assistant":
                ...
                continue
            # user (default): preserve text, images and PDF documents.
            user_item = self._user_message_item(msg.get("content"))
    ...
    @classmethod
    def _user_message_item(cls, content: Any) -> dict | None:
        if not isinstance(content, list):
            text = cls._stringify(content)
            return cls._message_item("user", text) if text else None
        blocks: list[dict[str, Any]] = []
        for block in content:
            if not isinstance(block, dict):
                if isinstance(block, str) and block:
                    if blocks and blocks[-1].get("type") == "input_text":
                        blocks[-1]["text"] += block
                    else:
    ...
    async def _consume_stream(self, kwargs: dict) -> tuple[list[str], Any]:
        ...
        path.
        """
        if ResponseCompletedEvent is None or ResponseTextDeltaEvent is None:
            raise RuntimeError("OpenAI Responses streaming requires the 'openai' optional dependency.")
