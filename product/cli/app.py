...
from mote.orchestration.environment.scheduling.service import CronService
from mote.product.application import Application
from mote.product.cli import backend
from mote.product.cli.consumers.registry import build_consumers
from mote.product.cli.contracts.base import BaseProjector
from mote.product.cli.driver import SessionDriver
...
def build_engine(
    *,
    ...
    container = ProductContainer.standard(config, cwd=Path(resolved_cwd))
...
def build_app(
    *,
    ...
    active_consumers = (
        consumer_objs
        if consumer_objs
        else build_consumers(
            config,
            active=consumers or ["terminal"],
            registry=eng.container.consumers,
        )
    )
    ...
    return SessionDriver(
        ...
        port=terminal_port,
        projector=projector,
        commands=eng.container.commands,
