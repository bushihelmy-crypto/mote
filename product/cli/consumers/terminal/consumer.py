...
:class:`~mote.product.cli.consumers.transcript.reducer.TranscriptReducer` (the single
event-orchestration state machine, shared with the Textual app) and the
:class:`~mote.product.cli.consumers.terminal.surface.TerminalSurface` (the rich
...
:class:`~mote.product.cli.consumers.transcript.driver.SurfaceDriver`, itself an ordinary
...
from mote.product.cli.consumers.core import register_consumer
from mote.product.cli.consumers.render.builders import file_change_verb
from mote.product.cli.consumers.render.builders import fold_note_str as _fold_note_str
from mote.product.cli.consumers.render.builders import format_usage_line as _format_usage_line
from mote.product.cli.consumers.render.palette import COMPACT, NOTE
from mote.product.cli.contracts.base import BaseConsumer
from mote.product.cli.contracts.view import TERMINAL_CAPS, Capabilities
from mote.product.i18n import keys as K
from mote.product.i18n import t

try:
    from mote.product.cli.consumers.terminal.rich_adapter import build_rich_terminal_consumer
...
class PlainTerminalConsumer(BaseConsumer):
    ...
    def on_media_block(self, ev: Any) -> None:
        ...
        ref = ev.ref or (artifact.readable if artifact is not None else "") or ev.alt or "(no reference)"
        self._print(f"  [{label}] {ref}")

    def on_artifact_block(self, ev: Any) -> None:
        artifact = ev.artifact
        label = artifact.suggested_name or artifact.kind or artifact.representation
        self._print(f"  [artifact] {label}: {artifact.readable}")
    ...
    def on_runtime_durability_status(self, ev: Any) -> None:
        readable = f"{ev.runtime_kind}:{ev.alias}"
        if ev.state == "lagging":
            self._print(
                f"Warning: {readable} checkpoint lagging: "
                f"r{ev.current_revision}, recoverable r{ev.recoverable_revision}"
            )
        else:
            self._print(f"Recovered: {readable} checkpoint at r{ev.recoverable_revision}")
