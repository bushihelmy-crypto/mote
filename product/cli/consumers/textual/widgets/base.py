...
def _is_empty_renderable(renderable: Any) -> bool:
    if isinstance(renderable, str):
        return not renderable.strip()
    if isinstance(renderable, Text):
        return not renderable.plain.strip()
    if isinstance(renderable, Group):
        return all(_is_empty_renderable(item) for item in renderable.renderables)
    return False
...
class SelectableStatic(Static):
    ...
    def __init__(self, content: Any = "", **kwargs: Any) -> None:
        ...
        self.display = not _is_empty_renderable(content)

    def update(self, content: Any = "") -> None:
        """Hide empty renderables so they occupy no selectable transcript row."""
