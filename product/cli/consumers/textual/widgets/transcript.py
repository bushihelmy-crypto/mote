...
SHARED ``mote.product.cli.consumers.render.builders`` — the exact same diff colours,
...
from mote.product.cli.consumers.textual.style import BULLET, NOTE, WARN, Palette
from mote.product.cli.consumers.textual.widgets.base import SelectableStatic
...
class ToolCallWidget(FoldableRow):
    ...
    def _folds(self) -> bool:
        # Once a tool call is a standalone row, ctrl+o treats the invocation and
        # its result as one unit. GROUP tools normally coalesce before they reach
        # here; if one is mounted standalone (for example in a test or orphaned
        # completion path), it still follows the same whole-row folding contract.
        return True
...
class MediaRow(SelectableStatic):
    ...
    class LinkedClicked(Message):
        """A media row was clicked so its producing tool can be selected."""

        def __init__(self, row: "MediaRow") -> None:
            super().__init__()
            self.row = row
    ...
    def set_expanded(self, flag: bool) -> None:
        """Show or hide the media with its producing foldable tool unit."""
        self.expanded = flag
        self.display = flag
    ...
    async def _on_click(self, event: Any) -> None:
        if getattr(event, "ctrl", False) or self.text_selection is not None:
            await super()._on_click(event)
            return
        self.post_message(self.LinkedClicked(self))
