...
:class:`~mote.product.cli.io.textual_io.TextualPort` pushes one of these
...
* :class:`ApprovalScreen` — a gated-action permission round-trip; dismisses with an
  :class:`~mote.product.cli.contracts.view.events.ApprovalDecision` (``y/n/a/d`` keys or
...
from mote.product.cli.consumers.textual.style import PROMPT_SYMBOL, WARN
from mote.product.cli.contracts.view.events import ApprovalDecision
from mote.product.cli.view.approval import approval_action, approval_options, approval_preview, approval_risk
...
class HandoffScreen(ModalScreen[HumanHandoffOutcome]):
    """Runtime Live Surface modal with an optional message and explicit return."""
    ...
    HandoffScreen > Vertical.window-control {
        width: 70%;
        max-width: 100;
        height: auto;
    }
    HandoffScreen .h-title { text-style: bold; color: $accent; }
    HandoffScreen .h-message { color: $text; padding-bottom: 1; }
    HandoffScreen .handoff-surface { height: 1fr; border: round $accent 40%; }
    HandoffScreen #handoff-message { margin-top: 1; border: round $accent; }
    HandoffScreen #handoff-actions { height: auto; margin-top: 1; }
    HandoffScreen #handoff-actions Button { width: 1fr; }
    """
    ...
    def compose(self) -> ComposeResult:
        with Vertical(classes="window-control" if self._window_control else ""):
            yield Label(t(K.HANDOFF_TITLE, runtime=self._request.runtime_ref.readable), classes="h-title")
            if self._request.message:
                yield Static(self._request.message, classes="h-message")
            if self._surface is not None:
                self._surface.add_class("handoff-surface")
                yield self._surface
            elif self._window_control:
                yield Static(t(K.HANDOFF_WINDOW_ACTIVE), classes="h-message")
            else:
                yield Static(
                    f"[{self._handle.surface.kind} surface: {self._handle.surface.ref}]",
                    classes="handoff-surface",
                )
            yield Input(placeholder=t(K.HANDOFF_MESSAGE_PLACEHOLDER), id="handoff-message")
            with Horizontal(id="handoff-actions"):
                yield Button(t(K.HANDOFF_COMPLETE), id="complete", variant="success")
                yield Button(t(K.HANDOFF_CANCEL), id="cancel", variant="error")

    def on_button_pressed(self, event: Button.Pressed) -> None:
        event.stop()
        if event.button.id == "complete":
            self._finish(HandoffStatus.COMPLETED)
        else:
            self._finish(HandoffStatus.CANCELLED)

    def on_input_submitted(self, event: Input.Submitted) -> None:
        event.stop()
        self.query_one("#complete", Button).focus()

    def action_cancel(self) -> None:
        self._finish(HandoffStatus.CANCELLED)

    def _finish(self, status: HandoffStatus) -> None:
        message = self.query_one("#handoff-message", Input).value.strip()
        self.dismiss(HumanHandoffOutcome(status=status, human_message=message))
...
class QuestionScreen(ModalScreen[tuple]):
    ...
    def on_selection_list_selected_changed(self, event: "SelectionList.SelectedChanged") -> None:
        ...
        if not self._multi and selected:
            self._dismiss_once(([self._options[selected[0]]], ""))  # snappy single-select pick
    ...
    def _confirm(self) -> None:
        ...
        if "visible" in answer_input.classes and answer_input.value.strip():
            self._dismiss_once(([], answer_input.value.strip()))
            return
        choices = self.query_one("#choices", SelectionList)
        labels = [self._options[v] for v in choices.selected if 0 <= v < len(self._options)]
        self._dismiss_once((labels, ""))

    def _dismiss_once(self, result: tuple) -> None:
        """Dismiss at most once while queued widget events finish dispatching."""
        if self._dismissed:
            return
    ...
    def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        self._dismiss_once(([], (event.value or "").strip()))
...
class ApprovalScreen(ModalScreen[ApprovalDecision]):
    ...
    def compose(self) -> ComposeResult:
        with Vertical():
            ...
            if self._preview:
                yield Static(Text(self._preview), classes="a-preview")
