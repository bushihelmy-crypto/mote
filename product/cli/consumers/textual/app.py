...
The object graph is assembled separately in :mod:`mote.product.cli.consumers.textual.bootstrap`.
"""
...
from mote.product.cli.consumers.textual.clipboard import detect_wsl_clipboard, native_copy
from mote.product.cli.consumers.textual.messages import ViewEventMessage
from mote.product.cli.consumers.textual.style import THEME_NAME, Palette, mote_theme, textual_css_vars
...
from mote.product.cli.consumers.transcript import TranscriptReducer, apply_ops
...
class MoteApp(App):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        # Media rows are separate widgets (so an image can retain its natural
        # height) but selection is shared with the producing tool/group.
        self._linked_media: dict[FoldableRow, list[MediaRow]] = {}
        self._media_owners: dict[MediaRow, FoldableRow] = {}
    ...
    def on_foldable_row_clicked(self, message: FoldableRow.Clicked) -> None:
        ...
        if self._selected_tool is row:
            self._set_tool_selected(row, False)
            self._selected_tool = None
            return
        if self._selected_tool is not None:
            self._set_tool_selected(self._selected_tool, False)
        self._selected_tool = row
        self._set_tool_selected(row, True)

    def on_media_row_linked_clicked(self, message: MediaRow.LinkedClicked) -> None:
        """Treat an image click as a click on its producing tool unit."""
        owner = self._media_owners.get(message.row)
        if owner is not None:
            self.on_foldable_row_clicked(FoldableRow.Clicked(owner))

    def _set_tool_selected(self, row: FoldableRow, selected: bool) -> None:
        """Synchronize a tool/group selection band with its emitted media."""
        row.selected = selected
        for media in self._linked_media.get(row, []):
    ...
    def action_toggle_tool_details(self) -> None:
        ...
        if self._selected_tool is not None:
            ...
            self._set_tool_expanded(selected, not selected.expanded)
            return
        self._tools_expanded = not self._tools_expanded
        for row in self.query(FoldableRow):
            self._set_tool_expanded(row, self._tools_expanded)

    def _set_tool_expanded(self, row: FoldableRow, expanded: bool) -> None:
        """Fold a tool/group and the image rows it produced as one unit."""
        row.set_expanded(expanded)
        for media in self._linked_media.get(row, []):
            media.set_expanded(expanded)
