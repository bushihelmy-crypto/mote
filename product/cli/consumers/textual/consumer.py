...
:class:`~mote.product.cli.consumers.textual.app.ViewEventMessage` via
``App.post_message`` — which is safe to call from any thread and preserves FIFO
order. A single ``on_view_event_message`` handler on
:class:`~mote.product.cli.consumers.textual.app.MoteApp` then performs the actual
...
from mote.product.cli.contracts.view import TEXTUAL_CAPS, Capabilities
