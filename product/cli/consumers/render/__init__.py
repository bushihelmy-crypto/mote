#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""``mote.product.cli.consumers.render`` — the neutral, host-agnostic rich substrate.

Both rich-based hosts read their look from here so neither depends on the other:

* :mod:`~mote.product.cli.consumers.render.palette` — the single colour/glyph source
  (``Palette``, ``BULLET``/``BRANCH``/…, ``PROMPT_SYMBOL``). Rich-free, ANSI-free.
* :mod:`~mote.product.cli.consumers.render.builders` — console-free ``rich`` builders
  (``render_diff``/``build_table``/``tool_started_text``/…) that turn a
  ``ViewEvent`` into a renderable with no ``Console`` and no side effects.

The scrolling :class:`~mote.product.cli.consumers.terminal.consumer.TerminalConsumer`
and the full-screen :class:`~mote.product.cli.consumers.textual.app.MoteApp` (and, in
