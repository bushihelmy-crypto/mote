...
:class:`~mote.product.cli.driver.SessionDriver`, adding only that multiplexing — it
imports no transport, touches no core engine class (only the ``backend`` seam),
and reuses the exact same construction path (:func:`~mote.product.cli.app.build_engine`).
The two pieces:
...
from mote.product.cli.serving.connection_scope import ConnectionScope
from mote.product.cli.serving.prompt_broker import PromptBroker
from mote.product.cli.serving.session_registry import ResidentSession, SessionRegistry
