...
from mote.product.cli.consumers.render.palette import WARN
from mote.product.cli.consumers.terminal.style import PROMPT, ansi_fg, render_banner
from mote.product.cli.contracts.view.events import ApprovalDecision
from mote.product.cli.io.terminal_menu import (
    _AMBER_RGB,
    _RULE_WIDTH,
    _dim,
    _menu_lines,
...
try:
    from mote.product.cli.io.posix_terminal import enter_raw as _enter_posix_raw
...
class TerminalPort:
    ...
    * ``get_input_reader()`` — test seam: supply a fake reader instead of stdin.
    ...
    async def decide_approval(self, ctx: Any, request: Any) -> Any:
        ...
        :class:`ApprovalRequest` via ``mote.product.cli.view.approval`` — the engine sends
