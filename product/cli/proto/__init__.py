...
* ``proto/projector.py`` — ``AppServerProjector``: the ``ViewProjector`` twin. Same
  telemetry handler registration on the Role runtime, same fold-once
  discipline; only the target contract differs. Mapping table in §6.3
  (``SessionStartEvent`` → ``thread/started``, ``LLMStreamDeltaEvent`` →
  ``item/agentMessage/delta``, ``ToolCallFinishedEvent`` → ``item/completed``, …).
...
The eventual entry is ``python -m mote.product.cli app-server`` (stdio, no TUI).
"""
