...
from mote.product.interfaces.terminal.style import PROMPT, ansi_fg, render_banner
...
from mote.product.presentation.rich_rendering.palette import WARN

try:
    from mote.product.interfaces.terminal.posix import enter_raw as _enter_posix_raw
...
class TerminalPort:
    ...
    * ``get_input_reader()`` — test seam: supply a fake reader instead of stdin.
    ...
    async def decide_approval(self, ctx: Any, request: Any) -> Any:
        ...
        :class:`ApprovalRequest` via ``mote.product.presentation.projection.approval`` — the engine sends
