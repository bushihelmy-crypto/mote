...
class CancelTasks(BaseTool):
    ...
    async def call(
        ...
        *,
        ...
        if not isinstance(pool, AgentBackgroundTasks):
            raise ToolError("Local background-task capability is unavailable")
        ...
        receipt = pool.cancel_current(task_id, reason or "user requested")
