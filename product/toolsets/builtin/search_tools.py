...
The result body itself is deliberately minimal: a "loaded, now callable"
confirmation plus the shared stateful-handoff precondition, NOT an echo of each
revealed tool's schema/description.
...
``tool_reference`` expansion, or the client-side native/XML projection adding
the newly revealed definition). The model only needs to know the load succeeded.

The full prose is also persisted as a ``kind="tool"`` sticky resource so a
post-compaction context rebuild retains the fact that the capability was loaded.
This is bookkeeping after reveal; it is never part of the pre-reveal hint and
does not expose the parameter schema early.
...
_STATEFUL_HANDOFF_RULE = (
    "Stateful tool rule: action=handoff requires an existing runtime. "
    "Initialize the tool with a normal non-handoff call first."
)
...
class SearchTools(BaseTool):
    ...
    # (The revealed *set* still lives durably on RoleState regardless; persisted
    # post-reveal descriptions survive via the sticky resource, not this body.)
    ...
    async def call(self, *, query: str = "", names: Optional[list[str]] = None) -> ToolResult:
        ...
        Before handing off any revealed stateful tool, initialize its runtime with
        a normal non-handoff call; handoff never creates the runtime itself.
        ...
        # Persist the revealed tools' prose as sticky post-reveal context so it
        # survives compaction. This is bookkeeping only: it neither rides the
        # pre-reveal hint nor exposes any parameter schema before SearchTools.
        ...
        # mechanism (server-side ``tool_reference`` expansion, or the client-side
        # native/XML projection adding the newly revealed definition).
        # The confirmation also carries the one shared lifecycle rule that all
        # stateful tools need before the model's next call.
        revealed_names = ", ".join(revealed)
        output = (
            f"Loaded {len(revealed)} tool(s): {revealed_names}. "
            f"They are now available to call.\n{_STATEFUL_HANDOFF_RULE}"
        )
