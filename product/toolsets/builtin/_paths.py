"""Shared relative-path resolution for product file tools.
...
def resolve_permission_path(
    get_cwd: Optional[CwdProvider],
    value: object,
    *,
    default_to_cwd: bool = False,
) -> str:
    """Return one canonical path target for permission and sandbox checks."""
    if not isinstance(value, str):
        return ""
    path = value.strip()
    if not path:
        return os.path.realpath(base_cwd(get_cwd)) if default_to_cwd else ""
    return os.path.realpath(resolve_path(get_cwd, path))
