#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Model-artifact loading + V3 feature runtime — vendored verbatim from opensquilla.

Source: ``squilla_router/models/.../runtime_src/src/router/inference/artifacts.py``.

Heavy deps (``joblib`` / ``lightgbm`` / ``onnxruntime``) are imported lazily
inside ``load_runtime_objects`` so this module stays import-safe.
"""
...
except ImportError:  # pragma: no cover - optional ml-routing extra
    joblib = None
    lgb = None
    ort = None
...
from mote.product.routing.squilla.ml.v4_features import BGEChannelExtractor
...
class InferenceArtifacts:
    ...
    def load_runtime_objects(self, config: dict, *, use_aux_head: bool) -> dict[str, object]:
        if joblib is None or lgb is None or ort is None:
            raise RuntimeError("Squilla inference requires joblib, lightgbm, and onnxruntime.")
