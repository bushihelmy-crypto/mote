#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Phase-3 post-processing pipeline — vendored verbatim from opensquilla.

Source: ``squilla_router/models/.../runtime_src/src/router/inference/postprocess.py``.

This is opensquilla's single, authoritative post-processing pipeline that runs
on the fused (LightGBM ⊕ MLP) probability vector: argmax → margin upgrade →
[aux downgrade | skip if margin-upgraded] → R1 rescue → under-routing safety →
flag overrides → context rules → optional sticky tier → thinking/prompt
derivation → trivial-ack collapse → :class:`FinalDecision`.

When the ML engine is unavailable, the no-model fallback path feeds this same
function a Gaussian-synthesised ``fused_probs`` with ``aux_probs=None``.
"""
...
from mote.product.routing.squilla.ml.features import ContextMetadata
from mote.product.routing.squilla.ml.flags import compute_flags
