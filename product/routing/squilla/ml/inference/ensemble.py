#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""LightGBM ⊕ MLP probability fusion — vendored verbatim from opensquilla.

Source: ``squilla_router/models/.../runtime_src/src/router/inference/ensemble.py``.
"""
...
def fuse_probabilities(p_main: np.ndarray, p_mlp: np.ndarray, alpha: np.ndarray) -> np.ndarray:
    ...
    if not (np.isfinite(p_main).all() and np.isfinite(p_mlp).all() and np.isfinite(alpha).all()):
