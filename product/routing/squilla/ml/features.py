#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Hand-crafted / context / history feature extraction — vendored from opensquilla.

Source: ``squilla_router/models/.../runtime_src/src/router/features.py``.

Only the runtime, model-free pieces are kept: the module-level extractors
(``extract_handcrafted`` 51-dim, ``extract_context_features`` 10-dim,
``extract_hist_features`` 16-dim) and the :class:`ContextMetadata` dataclass.
The training-time ``FeatureExtractor`` class (TF-IDF/SVD fit + sentence-
transformers BGE) is intentionally dropped — Phase-3 online inference uses the
serialized ``tfidf.pkl`` / ``svd.pkl`` artifacts via ``V3FeatureRuntime`` and
the ONNX BGE backend instead. Keeping the class out also keeps this module
importable without sklearn/joblib.
"""
...
from mote.product.routing.squilla.ml.trajectory import Trajectory, classify
