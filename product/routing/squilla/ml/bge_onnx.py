#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""ONNX-INT8 backend for the BGE encoder — vendored verbatim from opensquilla.

Source: ``squilla_router/models/.../runtime_src/src/router/bge_onnx.py``.
...
``onnxruntime`` / ``tokenizers`` are imported lazily inside ``_ensure_loaded``
so this module stays import-safe when those optional deps are absent.
"""
    ...
    def _ensure_loaded(self):
        if self._session is None or self._tokenizer is None:
            if ort is None or Tokenizer is None:
                raise RuntimeError("ONNX routing requires the 'onnxruntime' and 'tokenizers' optional dependencies.")
