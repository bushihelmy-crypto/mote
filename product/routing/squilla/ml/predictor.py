...
Phase-3 inference package (:mod:`mote.product.routing.squilla.ml.inference`) imports these.
...
from mote.product.routing.squilla.ml.flags import RoutingFlags
...
def _apply_margin_upgrade(route_class: str, margin: float, config: Mapping[str, Any]) -> str:
...
def _apply_r1_rescue(route_class: str, probs: np.ndarray, config: Mapping[str, Any]) -> str:
...
def _apply_flag_overrides(route_class: str, flags: RoutingFlags, config: Mapping[str, Any]) -> str:
    idx = _CLASS_TO_IDX[route_class]
    if flags.high_risk:
...
def _select_model(route_class: str, config: Mapping[str, Any]) -> tuple[str, str]:
