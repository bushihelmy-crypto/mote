...
from mote.product.inference.daemon.client_port import AuthenticatingSharedRuntimeClient, SharedRuntimeClient
...
class SharedClientFactory(Protocol):
    def __call__(self, socket_path: Path) -> AuthenticatingSharedRuntimeClient: ...
...
class ReconnectingSharedGrpcClient:
    ...
    def __init__(
        ...
        self._client: SharedRuntimeClient | None = None
    ...
    async def get_readiness(self, *, timeout: float | None = None) -> DaemonReadinessView:
        return await self._require_client().get_readiness(timeout=timeout)
    ...
    async def authorize_wire(
        ...
        *,
        ...
        await self._require_client().authorize_wire(execution_id, permit, generation_id=generation_id, timeout=timeout)
    ...
    async def _connect_locked(self, *, force: bool) -> SharedRuntimeClient:
        if self._closed:
