...
class ToolsConfig(YamlModel):
    ...
    # model endpoint transport proxy instead). Keep it consistent with the
    ...
    # credentials for a future direct-API vendor. Product's hosted-service adapter
    # resolves the active backend from here, so adding a vendor does not change the
    # Tool or Runtime gateway.
