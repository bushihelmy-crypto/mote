...
from mote.product.config.observability import ObservabilityConfig
...
class Config(YamlModel):
    ...
    @classmethod
    def _parse_product_model_syntax(cls, values):
        if isinstance(values, dict) and isinstance(values.get("models"), dict):
            values = dict(values)
            models = dict(values["models"])
            values["models"] = parse_product_models_config(models)
        return values
    ...
    def _validate_semantic_routing_graph(self) -> "Config":
        if not isinstance(self.models, ExplicitModelsConfig):
            ...
            enabled = any(agent.strategy is not None for agent in (self.router.main_agent, self.router.sub_agent))
            if enabled and not self.router.routes:
                raise ValueError("router enables routing with an empty semantic pool")
            if self.router.routes:
                raise ValueError("router semantic routes have no models.routes.semantic binding")
            return self
