...
from mote.product.config.diagnostics import _get_path, _render_value, unknown_key_paths
...
def format_report(
    ...
    *,
    ...
    if unknown:
        lines.append("")
        lines.append("# Unknown keys (configuration load will reject these)")
        for path in unknown:
            lines.append(f"  {path}")
    return "\n".join(lines)
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m mote.product.config.report",
