...
2. **Heading layout is dependency-version-specific** (older Rich versions drew
   h1 in a heavy panel; newer versions center and underline it), so a style token
   alone cannot guarantee one stable product layout.
...
* :class:`BrandHeading` subclasses rich's ``Heading`` to normalize headings to a
  bare left-aligned layout, and is
...
from mote.product.presentation.rich_rendering.mathbox import build_box
from mote.product.presentation.rich_rendering.theme import MARKDOWN_THEME
...
# :mod:`mote.product.presentation.rich_rendering.mathbox`, split by *dimensionality*:
#
...
class MathBlock(MarkdownElement):
    ...
    box engine (:func:`~mote.product.presentation.rich_rendering.mathbox.build_box`) which returns
