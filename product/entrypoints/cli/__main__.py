...
``build_repl`` it assembles the decoupled stack via :func:`mote.product.entrypoints.cli.bootstrap.build_app`
...
from mote.product.entrypoints.cli.bootstrap import build_app, build_engine
from mote.product.entrypoints.cron.cli import main as cron_main
from mote.product.entrypoints.gateway.cli import main as gateway_main
from mote.product.interfaces.textual import _HAS_TEXTUAL

try:
    from mote.product.interfaces.acp.server import serve as serve_acp
except ImportError:  # pragma: no cover - optional acp extra
    serve_acp = None

try:
    from mote.product.interfaces.agui.server import serve as serve_agui
except ImportError:  # pragma: no cover - optional agui extra
    serve_agui = None

try:
    from mote.product.interfaces.textual.bootstrap import run_textual
...
def _parse_args(argv=None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--tools",
        default=None,
        help="Comma-separated tool names (default Read,Edit,Search,Bash).",
    )
    ...
    parser.add_argument(
        "--token",
        default=None,
        help="Bearer auth token for the server (required unless --insecure).",
    )
    parser.add_argument(
        "--insecure",
        action="store_true",
...
def main(argv=None) -> None:
    ...
    if raw and raw[0] == "gateway":
        sys.exit(gateway_main(raw[1:]))
