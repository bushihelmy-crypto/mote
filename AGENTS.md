...
```
common  ←  context / executor / router  ←  loop / think / parser  ←  roles
        ←  environment / session / skills / tasks  ←  cli
```
...
- 典型例子：`context` 要记录会话 → 依赖 `SessionRecorder` Protocol，由 `roles` 注入 `session` 的实现；`executor` 要通知 LSP → 依赖 `LspNotifier` Protocol。
- `environment → session` 可在模块加载期直接 import（无环）；`environment → roles` 必须惰性 import 打破环。
- 写新源（turn_context source）这类低层组件时，若要 import 高层（如 `tasks`），**把实现放到那个高层包里**（如 `BackgroundTaskContextSource` 放在 `tasks/` 而非 `context/turn_context/`），守住分层。
...
- 组件都是 `Role` 上的**惰性初始化属性**（`executor` / `context_manager` / `router` / `think_engine` / `command_channel` / `context_provider` / `turn_context_bus` / ...）。新组件照此模式加一个 `_xxx` 槽 + 惰性属性。
- opt-in 子系统（hook / lsp）默认惰性属性返回 `None`，仅在配置开启时构造；turn_context_bus 例外（总存在，各源自抑制）。
...
- 工具返回 `ToolResult`（output/success/images/pdfs/...）。超大输出由 `tool_result_limit.py` 落盘，别自己截断丢信息。
- 持久会话工具（Terminal/Jupyter）是 per-session 隐式 singleton（无 model-facing id），引擎在 `executor/tools/_terminal.py` / `_kernel.py`。

---

## 4. 命令协议与 LLM

- 协议由 `RoleSchema.command_protocol` 选（`"native"` 默认 / `"xml"`），channel 由 `parser/native_channel.py::make_command_channel` 构造。
- native 的 provider 信封（OpenAI vs Anthropic）**不配置**，由 `infer_native_tool_provider(model)` 按 model 名推断（含 `claude` → anthropic）。
...
- 易变 / per-turn 内容（MEMORY.md、git/token/bg/lsp 提醒）**注入 user prompt 的 `<system-reminder>`**，经 `context/turn_context/` 的 bus。新增一类 per-turn 上下文 = 写一个实现 `EphemeralContextSource` 的源（带 `name`/`priority`/`async render`），挂到 bus，**不要**塞进系统提示词或存储历史。
- 压缩在 `ContextManager.manage_history()`：先 `microcompact`（无 LLM 折叠）再 `autocompact`（LLM 摘要）。`COMPACTABLE_TOOLS` 列表决定哪些工具结果可折叠。

---

## 6. 会话持久化

- rollout.jsonl（`session/`）是历史的崩溃安全真相源。新增事件类型加到 `events.py`（tagged-union），`replay.py` 的未知 type 会被忽略（向后兼容）。
...
| Role 编排 | `roles/role.py`（配置 `roles/role_schema.py`，状态 `roles/role_state.py`） |
| 调 LLM | `think/think_engine.py` + `router/llm/base_llm.py` |
| 提示词组装 | `think/prompt_builder.py` + `common/prompt/` |
...
| per-turn 注入 | `context/turn_context/`（+ `tasks/turn_context_source.py`） |
| 会话持久化 | `session/`（log/replay/listing/fork/snapshot/history） |
| 多 agent | `environment/`（runtime/control/scheduler/residency/store） |
