...
Kept dependency-free (only stdlib typing/dataclasses) so any layer — tools,
the engine, the config schema — can import these without circular imports.
This mirrors how Claude Code isolates ``src/types/permissions.ts`` from the
permission engine, and how this repo isolates ``common/schema/tool_config.py``
from ``executor/tool_result_limit.py``.
...
The runtime decision pipeline (see ``engine.py``) combines them into a single
``PermissionDecision``.
"""
