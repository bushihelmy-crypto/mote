...
Storage is the shared run journal (:class:`~mote.common.ledger.RunJournal`): an
EXTERNAL tool record is simply a :class:`~mote.common.ledger.StepRecord` with
...
from mote.common.ledger import COMPLETED, FAILED, JOURNAL_FILE_NAME, KIND_TOOL, STARTED, RunJournal, StepRecord
...
class EffectRecord:
    ...
    holds the tool's final (post-size-limit) output string for a terminal record,
    so the resume reconciler can heal a dangling call whose result message was
    never flushed — without re-running the tool. ``started`` records leave it
    ``None``.
    """
