...
from mote.common.events import EventBus, PreToolUseEvent, span
    ...
    async def run(self, execution: ToolExecution) -> ToolResult | None:
        ...
        outcome = await self._bus.emit(
            PreToolUseEvent(
                ...
                tool_input=execution.args,
                tool_use_id=execution.result_id,
        ...
        if outcome.updated_args is not None:
            execution.args = outcome.updated_args
        if outcome.behavior != "deny" and not outcome.stop:
            return None
        reason = outcome.system_message or outcome.stop_reason or "blocked before tool use"
        return failed_result(ToolPermissionDeniedError(reason), terminate=outcome.stop)
...
class LedgerStage:
    ...
    def run(self, execution: ToolExecution) -> ToolResult | None:
        execution.ledgered = (
            ...
            and execution.tool.resolve_effect() is not ToolEffect.PURE
        )
...
class ToolExecutionPipeline:
    """Fixed five-stage policy: resolve → authorize → ledger → invoke → settle."""
    ...
    def __init__(
        ...
        *,
        ...
        self._authorize = AuthorizeStage(bus)
