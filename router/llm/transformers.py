#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Default ``MessageTransformer`` implementations for the recovery loop.

These are the concrete handlers for the "repair the outgoing request, then retry
the same provider" recovery family (ported conceptually from hermes-agent's
...
# The default registry wired into ``BaseLLM._message_transformers`` so the three
# "repair then retry" recovery paths are live out of the box.
