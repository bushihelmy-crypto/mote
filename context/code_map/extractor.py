...
:class:`~mote.context.code_map.providers.base.LanguageProvider`; the facade picks
one by extension (:func:`~mote.context.code_map.languages.provider_for`), hands it
the already-read source, and wraps the neutral
:class:`~mote.context.code_map.providers.base.ProviderExtract` it returns into a
:class:`~mote.context.code_map.model.FileExtract` — adding the path, content hash,
language tag, and the one shared call-edge step (derived from the resolved
:class:`~mote.context.code_map.scopes.ScopeGraph`, identical for every language).
...
from mote.context.code_map.languages import provider_for
from mote.context.code_map.model import CallEdge, FileExtract, ImportBinding, ImportRef, Symbol
from mote.context.code_map.scopes import ScopeGraph
...
__all__ = ["CodeMapExtractor", "FileExtract", "Symbol", "CallEdge", "ImportRef", "ImportBinding"]
