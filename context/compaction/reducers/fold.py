...
from mote.common.const import RESOURCE_STICKY, RETENTION, RETENTION_PIN, TOOL_CALL_ID, TOOL_CALLS
...
class FoldReducer:
    ...
    async def reduce(self, transcript: Transcript, request: ReductionRequest) -> ReductionOutcome:
        ...
        # Count gate on tool results: every Edit write also emits a reconstructable
        # result, so writes are already reflected in this count and ride the SAME
        # trigger — no separate write trigger, no double counting. Under trigger →
        # nothing (results or writes) folds yet (the deferral the model asked for:
        # a fresh write stays verbatim while still in the working set).
