...
:class:`~mote.executor.tool_executor.ToolExecutor` at its emit sites — *pull* the
current scope at emit time and stamp it onto the event they fan out, exactly the
way they already pull the ambient :class:`~mote.common.events.bus.EventBus` from
...
def push_scope(ref: ScopeRef) -> Iterator[ScopePath]:
    ...
    Mirrors ``set_bus`` / ``set_progress_writer``: sets the extended path, yields
