#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""LLM-related constants — model capability checks.

The four ``supports_*`` capability checks are now THIN DELEGATES to the single
declarative :mod:`mote.common.model_profile` registry — the former scattered
...
def supports_vision(model: Optional[str]) -> bool:
    ...
    here so the two never drift. Delegates to the profile registry.
    """
