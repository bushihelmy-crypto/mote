...
concrete ``roles/session`` implementation.

Why a Protocol here (not in ``roles/session``): the ``executor`` layer must
never import the ``roles`` layer (the strict downward-only layering rule). The
concrete ``FileSnapshotRecorder`` / blob store live in ``roles/session`` and are
...
class FileSnapshotStore(Protocol):
    ...
    Implemented by ``roles.session.FileSnapshotRecorder`` (production) and any
