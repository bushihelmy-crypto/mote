...
class FileWatcher:
    ...
    async def start_async(self) -> None:
        """Async start: cooperatively snapshot the baseline, then run.

        The scan yields between bounded batches, so a large tree cannot monopolize
        the event loop and lifecycle does not depend on an implicit global thread
        pool whose completion callbacks may outlive the owning loop.
        """
        self._state = await self._snapshot_async()
        self._primed = True
        self._runner.start()
    ...
    async def _snapshot_async(self) -> Dict[str, _Sig]:
        """Cooperative counterpart of :meth:`_snapshot` for runtime polling."""
        state: Dict[str, _Sig] = {}
        visited = 0
        for root in self._roots:
            if os.path.isdir(root):
