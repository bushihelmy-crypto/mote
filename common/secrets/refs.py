...
:class:`~mote.common.secrets.store.SecretStore` already emits when it *masks* a
...
def expand_secret_refs(text: str, *, get_secret: Callable[[str], Optional[str]]) -> str:
    ...
    Raises:
        SecretRefError: A referenced key resolves to nothing (unknown key, empty
            value, or secrets disabled), or a ``<totp:…>`` seed is not valid
            base32. Fail-closed so a bad reference never types placeholder text.
    """
    if not text:
        return text
    ...
    def _replace(match: "re.Match[str]") -> str:
        ...
        if not value:
            raise SecretRefError(
                ...
                f"{key!r} is available (check the vault / secrets_config.json, or that "
                "secrets are enabled)."
            )
        if kind == "totp":
            try:
