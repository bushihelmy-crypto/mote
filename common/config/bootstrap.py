...
# annotated template. Only `models.default` (with a working api_key) is required.
models:
  default:
...
def _packaged_config_example() -> Path:
    """Path to the annotated ``config.example.yaml`` shipped inside the package."""
    return MOTE_PACKAGE_DIR / "config.example.yaml"


def _config_yaml_template() -> str:
    """Return the ``config.yaml`` seed text (packaged example, else the stub)."""
    example = _packaged_config_example()
    try:
...
def ensure_mote_home(root: Optional[Path] = None) -> None:
    ...
    absent. *root*
    overrides the destination (defaults to :data:`paths.CONFIG_ROOT`); it exists
    for tests. Never raises: a hostile / read-only home is logged and ignored.
    """
    base = Path(root) if root is not None else paths.CONFIG_ROOT
    try:
    ...
    _seed_file(base / "config.yaml", _config_yaml_template(), label="config.yaml")
    _seed_file(base / "mcp.json", json.dumps(_MCP_TEMPLATE, indent=2) + "\n", label="mcp.json")
    _seed_file(base / "hooks.json", json.dumps(_HOOKS_TEMPLATE, indent=2) + "\n", label="hooks.json")
    _seed_file(
