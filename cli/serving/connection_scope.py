...
turn) bound to a :class:`~mote.cli.serving.session_registry.ResidentSession`
pulled from the shared registry:

    scope = ConnectionScope(session, consumers=[agui_consumer], port=agui_port)
    async with scope:                       # subscribe projector to role bus
...
capability adapters) subscribed to the session's role bus, so two concurrent
...
from mote.cli.contracts.view import ErrorRaised, MessageBlockCompleted
...
class ConnectionScope:
    ...
    subscribed to the session's role bus, plus an optional input port bound as
    ...
    def open(self) -> None:
        """Subscribe this scope's projector to the role bus + bind the port.
        ...
        """
        bus = backend.role_event_bus(self._role)
        if bus is not None and self._subscribed_bus is None:
            try:
    ...
    async def aclose(self) -> None:
        ...
        if self._subscribed_bus is not None:
            try:
                self._subscribed_bus.unsubscribe(self._projector)
