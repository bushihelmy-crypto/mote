...
:class:`~mote.cli.driver.SessionDriver`, adding only that multiplexing — it
imports no transport, touches no core engine class (only the ``backend`` seam),
and reuses the exact same construction path (:func:`~mote.cli.app.build_engine`).
The two pieces:
...
* :class:`ConnectionScope` — the per-connection / per-turn ``{port, consumer,
  BaseProjector}`` bundle: subscribe to the session's role bus, drive one turn,
...
from mote.cli.serving.connection_scope import ConnectionScope
from mote.cli.serving.prompt_broker import PromptBroker
from mote.cli.serving.session_registry import ResidentSession, SessionRegistry
