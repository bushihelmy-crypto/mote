...
:class:`~metagpt.cli.consumers.render.palette.Palette` (hex tokens) into Textual's
``$variable`` map, which :class:`~metagpt.cli.consumers.textual.app.MetaGPTApp`
...
from metagpt.cli.consumers.render.palette import (  # re-export the shared figure set
    BRANCH,
    BULLET,
    CHECK,
    COMPACT,
...
def textual_css_var_block() -> str:
    ...
    ``App.get_css_variables`` (see :class:`MetaGPTApp`). This helper is kept for
