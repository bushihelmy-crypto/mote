...
This lives in the local ``common`` layer (not a single host) because the human
...
class SystemReminder(ViewEvent):
    ...
    metagpt's turn-context sources (git snapshot, token-pressure, changed-files,
...
class ConversationCompacted(ViewEvent):
    ...
    metagpt's ``CompactionEngine`` rebuilds history when the context fills up,
