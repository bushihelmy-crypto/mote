...
result, and dim-styled live streaming of think tokens.
...
using its existing plain-text path. Everything here is driven by ordinary
sequential ``Console.print`` calls (a turn runs think -> act -> think in order,
never concurrently), so no ``rich.Live`` is needed and the renderer is trivially
testable by injecting ``Console(file=StringIO())``.
"""
...
class ConsoleRenderer:
    ...
    def __init__(self, console: Optional["Console"] = None):
        ...
        # Pre-resolved once: ``stream`` runs per LLM token, so avoid re-parsing
        # the "dim" style string on every call.
        self._stream_style = Style(dim=True)
    ...
    # ------------------------------------------------------------------
    # Token streaming (incremental, no markup parsing)
    # ------------------------------------------------------------------
    def stream(self, token: Any) -> None:
        text = token if isinstance(token, str) else str(token)
        self._console.print(text, end="", markup=False, highlight=False, style=self._stream_style)
