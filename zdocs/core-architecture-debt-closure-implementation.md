...
> 本文件是实施 Agent 的唯一任务入口。无需读取需求稿、评审稿、编号迁移记录或修改历史。第 4 至第 7 节的每个 `R*` 编号都是一个不可拆散验收语义的工作包；第 10 节是唯一实施顺序和状态总账。实施者先在总账更新状态，再按工作包中的当前源码事实、实施任务和验证与签收条件完成垂直切片。源码路径用于重新定位当前事实，行号和本文结论不得替代开工时复核。
...
本计划以 2026-08-01 已批准的五层生产代码审计结论为实施基线。测试和既有文档不属于整改对象，但可作为行为证据与验收门禁。问题不按文件大小或 `Any` 数量判定；只有处于正式边界、形成双真相、破坏分层或导致生产行为断链的项目才纳入。
...
拒绝整个批次不符合已确认产品语义：可以安全消解的展示名称冲突不得阻断合法生成需求。自动改名不能退化为写入时临时试探 `-2/-3`：同一 call 重试必须复用首次 target plan/reservation；并发调用必须通过 canonical reservation 避免 TOCTOU，不能因调度顺序覆盖彼此。若首次 planning 面对调用外已有文件占用，是否允许覆盖仍服从既有 mutation/permission contract；本 ADR 只授权消解同一批次或并发 reservation 产生的名称冲突，不把 overwrite 权限偷偷改成自动改名策略。
...
Orchestration 的旧 environment 兼容 facade 进一步放大了该混合边界：它曾用 `_roles: Dict[str, Any]`、`add_role/get_role/set_addresses` 保存和返回宽 Role，并通过嵌套 `getattr/hasattr` 推断 schema/name/state/set_env；human channel 方法也以 `Any` 传递 question/request/sender。Runtime `Role` 与 capabilities 随后直接假设 `state.env` 具有 `set_addresses/publish_message/ask_user/request_approval/reply_to_user` 等未声明方法（`runtime/agent/role.py:498-532,1116-1120`、`runtime/agent/capabilities.py:293-364`）。`BaseEnvironment` 仍是允许 arbitrary runtime object 的 Pydantic facade，实际生产主要由 `AgentEnvironment/MoteEnv` 继承，未形成可序列化价值。
...
源码事实：`kernel/execution/engine.py::CompletionPolicy.evaluate(ModelTurn) -> CompletionDecision` 是单轮模型输出的完成判断；现有 `contracts/ports/output/run_completion_policy.py::RunCompletionPolicy.process(RunCompletionIntent) -> RunCompletionDecision` 是 Role flow 结束后的延续策略。二者名称相近但生命周期、输入和结果均不同，不得机械合并，也不能让 Kernel 反向借用后者。
...
同时收窄当前 authoritative `kernel/execution/request.py` 中的裸 `list`、裸 `dict`、`tool_specs: Any`、`command_channel: Any` 和 `tool_snapshot: Any`；不得再让 ContextProvider 已知的类型在进入 Engine 前丢失。
...
`product/session_hosting/registry.py` 复制 control 构造逻辑并直接写 `role.agent_control`，`ResidentSession.control/role`、role factory 和 engine 均为 `Any`。旧 Orchestration environment facade 也曾用 `Dict[str, Any]` 保存 Role，相关 handle 的 control/residency slot 实际类型稳定却仍为 `Any`。
...
Agent turn limiter 也必须使用原子 admission：`orchestration/agents/turn_queue/limiter.py:38-50` 将 `has_capacity()` 与 `guard()` 分成两个操作，`guard()` 本身无条件递增；`orchestration/agents/control.py:621-674` 仅在消息投递时用 `has_capacity()` 决定 park，而多个已唤醒 runtime 随后可在 `turn_scheduler.py:200-212` 同时调用无条件 `guard()`。因此并发 turn 数可越过 `max_agents`，检查只影响排队时刻而不构成执行 admission。
...
R2.42 authoritative scope/lifecycle matrix（后续 owner 变更必须与本工作包同片更新）：

| 对象/能力 | scope | construct owner | activate owner | shutdown owner | 共享/继承与 durability | required Port |
| --- | --- | --- | --- | --- | --- | --- |
| validated `Config`、`RuntimePaths` | application declaration | Product source adapter | 不激活 | Application release | session 只读借用；Config 本身非运行状态 | typed config/source provenance |
| `ProductContainer` 与 provider/tool/Agent catalog snapshot | application | `product/composition/bootstrap.py` | Product application activation；catalog 按 approved declaration materialize | Application release | 跨 session 共享 immutable generation；不跨 application | canonical catalog/compiler contracts |
| `RoutingModelRuntime`、Runtime `Context`、model/service gateway | application | Product composition | ordered async application activation | `EngineServices` reverse lifecycle | 跨 session 共享；不得缓存 turn/Agent mutable state | model/service gateway、admission、journal Ports |
| `AtomicApplicationComposition` 与 application lease | application generation | Product composition | Product composition | `EngineServices` registered close | holder 借用固定 generation；不允许 borrower close owner | typed application lease/transfer/release Ports |
| `EngineServices`、`Application` | application | `product/composition/bootstrap.py` 唯一 factory | composition factory | `Application.aclose` → `EngineServices.aclose` | session/Agent 借用；durable fact 由各 canonical store 拥有 | Runtime lifecycle/resource Ports |
| hosted Session/connection binding | session/connection | Product session hosting | host accept | host disconnect/session release | 不跨 session；不能关闭 application resource | session registry、human interaction Ports |
| Role、Agent runtime、BackgroundTask pool | Agent/incarnation | Product Agent factory | Agent runtime owner | Agent release/drain | application services borrowed；incarnation fence 后不可继承 mutable state | Agent construction、runtime lease、task drain Ports |
| turn context、human binding、tool invocation | turn | Runtime/host turn admission | admitted turn | turn settlement | 不跨 turn，不进入 `RoleState` | turn permit、interaction、tool execution Ports |
...
- runtime context turn format 模块仅为 `wrap_system_reminder` 保留的 historical import shim；
- OAuth error 曾存在全局 Runtime facade 与 OAuth-local facade 双入口，现已归并至 OAuth bounded context；
- `runtime/sandbox/__init__.py` 声称公开但实际不存在的模块级 `wrap_command`。
- Runtime interactive package facade 为获取 checkpoint store、host 或 handoff 就 eager import Chromium backend；基础 Agent component 因而被可选浏览器依赖污染；
- Product presentation projection package facade 明示为 existing callers 保留，但生产消费者均从 defining modules 导入，属于已归零的历史 re-export facade。
...
实施任务：所有真实运行时 import 移到模块顶部；标准库/必需依赖直接导入，可选依赖在顶部形成 typed availability slot，平台依赖按顶部平台分支选择。若顶部导入暴露循环，必须拆分 owner、抽取 Contracts Port 或调整初始化顺序，禁止继续以局部 import 回避。对于体积较大的 optional plugin，需要懒加载时使用一个明确的 Product-owned plugin/entrypoint catalog并返回typed factory，不允许在普通业务函数中散布 `import`；plugin loader本身进入composition declaration和hermetic availability gate。
...
- 已确认工作包若在开工复核中被证明仍跨多个独立状态机、store、lifecycle owner 或可独立验收变更，必须停止实施并更新本文件的工作包与依赖台账；不得在代码中暗拆、临时共用状态或改变既有工作包 identity。
- `R0.0` 至 `R3.6` 的当前编号是唯一稳定工作包 identity；实施期间不得改号、复用编号或创建字母后缀工作包。
...
| --- | --- | --- | --- | --- | --- |
| M0 | R0.0 | DONE | Codex | — | 实现: canonical `Config` + lazy Product catalog materialization + optional Terminal/RunGraph activation guard；测试: 4 passed（3 个 container 节点、1 个 ledger gate），Pyright 3 个 authoritative 文件各 0 error；归零: `MoteConfig` 引用为 0，standard construct 前 Agent/Hook/MCP discovery 为 0；复核闭合: 插件声明改为惰性增量，with_plugins 不再物化 Agent catalog，派生 composition 保留 canonical cwd；新增 3 个定向节点验证惰性、cwd 与既有插件隔离。
| M0 | R2.42 | DONE | Codex | R0.0 | 实现: typed `ApplicationBuildRequest`、唯一 async Product root、scope/lifecycle matrix、统一 CLI/AG-UI/ACP/Textual loop 与 reverse settlement；测试: 9 passed（5 composition/Role 节点、2 governance/ledger、2 R0.0 container 节点），7 个改动文件 Pyright 0 error；归零: Product `Application` constructor 仅 1 个、CLI `ProductContainer/EngineServices` 构造与旧 `build_context/build_role/configure_service_gateway/run_app` 入口均为 0；真实 provider startup 重型节点按 WSL 资源约束未运行；复核闭合: telemetry factory 与治理声明统一，门禁只认可显式 __all__ 公共面，canonical AgentRegistry 不再被名称误杀，governance artifact 已重建；11 个定向断言通过。 |
| M1 | R1.7 | DONE | Codex | R1.8、R2.30 | 实现: USER/MANAGED-only structured absolute argv contract、config parse/credential activation分离、canonical executable dev/inode binding、minimal env/restricted cwd fixed runner、30s timeout/64KiB bound/strict UTF-8/单行 secret、digest-only audit；测试: 50 个定向断言通过（source policy/load/report/activation/nonzero/timeout/oversize/decode/architecture），6 个生产文件逐文件 Pyright 0 error；归零: 字符串 helper、`shell=True`、loader-time process、helper secret cache、ENV/CLI/PROFILE/PROGRAMMATIC注入与 stdout/stderr disclosure 均为 0；复核闭合: Runtime canonical process runner 新增 FixedExecutableBinding 与 descriptor-bound verified argv，open/fstat/pass_fds/exec 使用同一 inode；Product helper 不再 stat 后按 path 执行，3 个 fault-injection 节点与生产编译通过。 |
| M1 | R1.8 | DONE | Codex | M0 | 实现: resolved path+device/inode `SourceIdentity`、owner/mode/root trust、least-trust alias dedupe 与 canonical provenance；测试: 17 passed，1 个中间 provenance failure 修复后通过，3 个 authoritative 文件 Pyright 0 error（loader 仅 1 个既有 yaml source warning）；归零: 生产 `source_root`、`ConfigSource.PROJECT`、重复 USER/PROJECT 加载入口均为 0；复核闭合: loader 通过 O_NOFOLLOW descriptor 打开并以 fstat 复核 discovery device/inode 与可信权限后读取，替换或消失均 fail closed；9 个 source/loader 定向断言通过。 |
| M1 | R1.9 | DONE | Codex | R1.8 | 实现: Product `ExtensionSourcePolicy`、kind/path/inode/digest/principal approval snapshot、builtin/user/project scope 与 approved-bytes materialization；测试: 8 passed，2 个中间 adapter变量 failure 修复后通过，10 个直接文件 Pyright 0 error；归零: Agent/Skill/Hook/MCP raw checkout 消费路径为 0，Hook/MCP raw-path adapter 与 malformed best-effort activation 为 0；复核闭合: 删除 MarkdownMetaParser 任意 Path 读取入口，Agent/Skill 仅解析 ExtensionSource approved bytes；malformed/non-mapping frontmatter fail closed，9 个定向断言通过。；复核闭合: approval identity绑定kind/path/device/inode/digest/principal，inspect使用O_NOFOLLOW descriptor read及前后fstat；测试: 9 个extension trust节点通过，Pyright 0 error；归零: path replacement/symlink TOCTOU与同内容新inode继承批准均为0。 |
| M1 | R1.15 | DONE | Codex | R1.8 | 实现: domain-separated SHA-256 credential subject、strict v1 revision/token-generation envelope、0600 unique-temp+flush/fsync+replace+parent-fsync file commit、keyring同 envelope、durable one-time fallback backend selection、login/refresh/delete统一 subject lock/CAS；测试: 37 个定向断言通过（storage/manager/MCP/architecture，含双 manager 并发 refresh），6 个生产文件逐文件 Pyright 0 error；归零: provider 原文拼 durable path、损坏即 absent、原地 O_TRUNC、逐操作 keyring→file fallback、锁外 login/cache commit 均为 0；复核闭合: credential 与 backend-selection decoder 均要求 exact int version，JSON boolean fail closed；3 个定向节点通过。 |
| M1 | R1.16 | DONE | Codex | R1.9、R1.15 | 实现: source/server-located `McpConfigCompilationError`、absent-vs-declared-invalid auth 判别、完整 config candidate fail closed、Runtime全 server auth preflight与typed storage/manager activation failure；测试: 15 个定向断言通过（invalid/null/missing OAuth、zero client/network、anonymous public server、bridge与architecture），3 个生产文件逐文件 Pyright 0 error；归零: invalid OAuth warning+`None`、`loading without auth`、逐 server auth failure后继续 discovery 均为 0；active catalog 的完整旧/新原子 swap由依赖本语义的 R1.14 签收；复核闭合: R1.9 approved extension source 与 R1.15 strict credential storage 前置均已完成，原有 15 个 MCP auth compilation 定向断言证据继续成立。；复核闭合: 唯一联动退回前置已完成，本项原有实现、测试与归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 对未闭合前置的依赖为0。 |
| M1 | R2.30 | DONE | Codex | M0 | 实现: `run_fixed_argv(Sequence[str])` 与 ToolExecutor-generation-bound `run_authorized_shell`、窄 sandbox Protocol、封闭 process disposition/result、timeout/cancellation process-group settlement；迁移: VCS 使用 `("git", *args)`，Bash 校验 active authorization 且 sandbox missing/activation fail closed；测试: 52 个定向断言通过（process/Bash/VCS/sandbox/architecture），5 个生产文件逐文件 Pyright 0 error；归零: 生产 `aexecute`、通用 `shell` bool、`cmd.split()` 与 Bash unsandboxed fallback 均为 0；复核闭合: runtime/media/video 删除第三套 subprocess facade，ffprobe/ffmpeg 全部消费 canonical run_fixed_argv typed result；raw thumbnail 使用受控临时输出文件，32 个纯节点与生产编译通过。 |
| M2 | R0.1 | DONE | Codex | M0 | 实现: 封闭 `ToolPayload`（JSON/有界 inline binary/typed artifact reference）、严格 `tool-result+json@3` codec、durable `payload` 与 process-local `execution_value` 分离并迁移 Tool/BackgroundTask/Workflow/RunGraph producer；测试: 原 84 个定向断言及本轮 result/receipt/limit 55 个断言通过；归零: 生产 `ToolResult(data=...)`、未知对象 `{type, repr}` fallback、legacy receipt 宽松回放与 deferred control object 持久化均为 0；复核闭合: `ToolResult`、`ToolMedia`、`FileChange` 均为 frozen/slots canonical value，所有 collection 在构造边界固化为 tuple，字段、元素与 durable payload variant 在构造时 fail closed；pipeline 通过 immutable `replace` 推进结果，构造后不能绕过不变量，encoder 不再能写出自身 strict decoder 拒绝的结果。；复核闭合: JsonToolPayload公开构造深冻结，ArtifactToolPayload/ToolMedia验证ArtifactRef，inline binary验证bytes/kind；测试: 40 个tool payload节点通过，Pyright 0 error；归零: nested constructor绕过与可变JSON carrier均为0。 |
| M2 | R0.3 | IN_PROGRESS | Codex | R0.1 | 实现: Product deterministic media target planner、稳定rename及Agent通知；permission-facts经FileOps non-reentrant lease原子reservation并有界释放；resolved plan贯穿permission targets、logical operation key、publication transaction与typed settlement，requested/resolved target共同进入operation digest；canonical Hosted Service payload仅拥有远端media spec，不反向拥有Product本地文件计划；下载仅返回bytes，逐资产经`commit_generated_files`返回committed/failed/in_doubt；测试: 原45个target/reservation/publication证据保留，新增 9 个FileOps与ServiceGateway边界断言通过；归零: 直接写盘、无concrete target、静默materialization成功、随机transaction、permission与落盘target分裂及把Product target塞入Contracts service payload均为0。；复核闭合: download/FileOps generic exception不再转为materialization success，非COMMITTED publication保持failed/in-doubt且不生成成功asset；测试: 12 个media governance节点通过；归零: 本地权限/owner/fence拒绝后继续远端success路径为0。；2026-08-02 再次逐条验收缺口: `_generate_kind.generate_one()` 在远端 `invoke_service` 已成功并取得 `MediaGenerationResult` 后，`_download(result)` 的普通异常仍落入最外层 `except Exception`，返回只含 `status=failed/error` 的新对象，丢弃远端结果、provider reference 与 typed publication settlement；外部副作用已发生却被表现为普通未完成失败，download failure 也未稳定进入 FAILED/IN_DOUBT publication 状态。；2026-08-02 关闭复核: 直接反证已修复，定向类型/行为/架构门禁通过，硬前置依赖闭包恢复 DONE。 |
| M2 | R0.6 | DONE | Codex | R1.9、R2.30 | 实现: approved `ExtensionSource` 后严格 stable-id/absolute structured argv Hook config，共享 canonical `PermissionEngine`、OS `SandboxRuntime.wrap_exec`、最小 env、JSON stdin、256KiB output/strict UTF-8/timeout-cancellation process settlement；control/observation 明确分型，PreToolUse timeout/spawn/decode/unknown/callback failure fail closed，Hook permission fact写入绑定当前 `ToolInvocationIdentity` 的policy trace且参数rewrite后重新permission；测试: 29 个定向断言通过（Hook command governance、ToolExecutor hook、approved source layering、Role composition），11 个直接生产文件 Pyright 0 error；归零: Hook `command` shell字符串、相对executable、独立 `create_subprocess_shell`、parent env/session/transcript secret注入、control `failed_open`、无稳定handler identity与未审计Hook command permission均为0；WSL子进程连续事件循环节点出现宿主挂起，process disposition/stdin路径以typed stub fault injection定向验证；复核闭合: 当前 Hook command governance 可解析 canonical Runtime 模块，bounded stdin、sandbox wrap 与 typed timeout 共 7 个节点全部通过；R1.9/R2.30 硬前置已完成，无需生产改动。；复核闭合: 唯一联动退回前置已完成，本项原有实现、测试与归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 对未闭合前置的依赖为0。 |
| M2 | R0.8 | DONE | Codex | M0 | 实现: `AppendOnlyLedger` typed durable receipt/error、append fsync 后发布内存、reap replace 后切换 snapshot、严格 committed-frame decoder/仅无换行 torn tail、RunJournal 单调 lifecycle 与 EXTERNAL unknown fail-closed，append/replace 均提交 parent directory fsync；测试: 77 个定向断言通过（ledger fault injection、RunJournal、Jsonl runner/backend、ToolExecutor durable-started、4 个 process crash 点），5 个生产/测试文件 Pyright 0 error；归零: ledger 写错 warning+吞错、durable commit 前 `_latest` 更新、任意坏行 skip、terminal-without-started 与 committed lifecycle fork 均为 0；多实例 ownership/Workflow effect reconciliation 明确保留给 R0.9；复核闭合: AppendOnlyLedger append 在写盘与发布前调用同一 strict _parse_record 并要求 canonical round-trip 等值；非法 record 不落盘、不进入内存投影，11 个 durability 节点通过。；复核闭合: StepRecord构造与decoder拒绝NaN/Infinity时间；测试: 29 个ledger节点通过，Pyright 0 error；归零: 非有限durable时间进入fold路径为0。 |
| M2 | R0.9 | DONE | Codex | R0.8、R2.46 | 实现: Contracts Runtime最小typed `OperationOwnershipPort`将deployment/backend/holder/expected revision/logical EffectId贯穿canonical lease/fence；四类effect capability分别约束重放，receipt未知进入typed IN_DOUBT；Product application是Temporal唯一client/worker/catalog lifecycle owner，使用stable typed `StepInput`、EffectId Temporal workflow identity和capability-aware retry，outer command与payload effect/step/capability交叉校验；Runtime删除动态backend plugin、`make_temporal_backend`及closure `run_step/run_activity`第二入口，Role不再构造Temporal backend。测试: 14 个Temporal factory/activity/reconciliation/bootstrap小型节点及2个新增唯一owner/identity-binding架构断言通过，相关非optional生产文件Pyright 0 error；归零: 空catalog、closure跨host、Role级第二worker、backend降级、generic retry覆盖non-replayable、effect identity/capability断链与local journal冒充Temporal真相均为0。；2026-08-02 逐条复验联动: 硬前置 `R0.8` 已因 committed ledger reader 接受 NaN/Infinity 时间值退回 `IN_PROGRESS`；R0.9 的 local journal/effect reconciliation 基础尚未严格闭合，不能维持 DONE。；复核闭合: 全部硬前置已恢复DONE且本项无独立实现反证，原实现/测试/归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。 |
| M3 | R0.2 | DONE | Codex | — | 实现: `contracts/model/invocation.py` 唯一严格 `CanonicalToolCall` 并端到端接入 InferenceResult/Runtime/Kernel/journal；topology `EndpointCapabilityDeclaration` 与 failover `ResolvedEndpointCapabilities` 按生命周期分名，Runtime 使用显式单向 projection；测试: 42 个定向断言通过（ownership/native channel/inference journal/topology/failover），5 个 Runtime/Kernel/Product authoritative 文件 Pyright 0 error；归零: 重复 `CanonicalToolCall` 定义、`command_name/args` 字典兼容访问、歧义 `EndpointCapabilities` 类型名及隐式 model-dump capability 投影均为 0；复核闭合: CanonicalToolCall.arguments 改为 Mapping，并在 Pydantic JSON 校验后递归 freeze_json；源 mapping 与 nested list 后续修改不影响 canonical call，内部写入被拒绝；2 个定向节点与生产编译通过。；复核闭合: 删除LLMToolCall，LLMResponse.tool_calls统一immutable CanonicalToolCall tuple，native signature用thaw_json；测试: 91 个native/parser/think节点通过；归零: 生产LLMToolCall引用为0。 |
| M3 | R0.7 | DONE | Codex | — | 实现: `SearchResult` 顶层导入 authoritative `ContentIdentity`，blob encoder 显式投影 wire `str`、strict decoder 显式恢复 `ContentDigest` nominal type；测试: 5 个定向断言通过（type-hint解析、canonical round-trip、3 个非法 digest），2 个 contract 文件 Pyright 0 error；归零: undefined `ContentIdentity`、普通 `str` digest 构造与 nominal digest 无法通过自身 strict codec 回读均为 0；复核闭合: ContentIdentity 构造与 strict wire decoder 统一限制为 unsigned 63-bit size，encoder 不再产生自身 decoder 无法读取的对象；8 个定向断言与生产编译通过。 |
| M3 | R2.2 | DONE | Codex | — | 实现: Contracts consumer-owned窄 `ModelTurnCompletionPolicy.evaluate(ModelTurn)`，迁移 ExecutionEngine、graph assembly、ReAct/review interpret node与typed `TextCompletionPolicy`；保留独立 Role `RunCompletionPolicy.process(RunCompletionIntent)` 生命周期；测试: 8 个定向断言通过（port ownership/graph wiring/text completion），6 个生产文件 Pyright 0 error；归零: Kernel内联 `CompletionPolicy`、completion policy `Any` graph seam与两种 completion lifecycle 同名歧义均为 0；复核闭合: CompletionDecision 构造封闭 kind/index/reason 合法组合，删除无生产消费者 COMPLETE tag；ReAct/Review 不再以 or 0 静默恢复缺失 index，14 个定向断言通过。 |
| M4 | R1.4 | DONE | Codex | R0.1 | 实现: Contracts-owned `DeferredResultKind`/`DeferredToolSettlement` 与窄 `DeferredResultProjector`/factory Port，Product唯一adapter以authoritative BackgroundTask/Workflow类型立即校验并分别提交，Runtime pipeline仅消费typed projection且缺Product projector fail closed；BackgroundTask process-local result与Workflow result/metadata保持不同owner，删除Workflow `BgTaskResult`/`GraphMeta` alias及两个Orchestration result对Runtime ToolResult的上行依赖；测试: 11个定向断言通过（tagged classification/distinct submission、Runtime reflection gate、deferred/atomic ToolExecutor、Product factory wiring），10个直接生产文件Pyright 0 error；归零: 生产 `is_background_result`、`to_tool_result`、Workflow `BgTaskResult`/`GraphMeta` alias、Runtime对Orchestration import与Orchestration result对Runtime import均为0；复核闭合: 唯一硬前置 R0.1 已完成，Deferred settlement 最终投影进入同一个不可变、严格可逆的 canonical ToolResult durable contract。；复核闭合: 唯一联动退回前置已完成，本项原有实现、测试与归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 对未闭合前置的依赖为0。 |
| M4 | R1.5 | DONE | Codex | — | 实现: BackgroundTask `BackgroundTaskStatus` 与 Workflow `WorkflowNodeStatus` 分属 authoritative owner，并以 Contracts strict source-tagged projection 显式互转；新增 nominal `AttemptId`，submit/adopt 从 1 起步、resubmit 保持模型可见 TaskId 并单调推进 attempt，旧 settlement 只读留存，operation/progress/completion/notification 全链绑定当次 attempt 且 stale/unknown fail closed；测试: 11 个定向断言通过（5 status ownership、3 AttemptId/stale attempt、3 Pool submit/adopt/notification），6 个直接 contract/status/model/delivery 文件 Pyright 0 error且4个生产文件py_compile通过；归零: 生产 `BgStatus`、过期 `common/schema/node_status.py` owner、无 AttemptId resubmit、旧 callback覆盖新 attempt及无 attempt notification均为0；复核闭合: authoritative models.__all__ 已包含 AttemptId；stale runtime governance expected surface 同步 AttemptId 与已有真实消费者的 BackgroundWakeReason，9 个 attempt/progress/owner 节点通过。；2026-08-02 最终复核: 直接缺口与硬前置均已闭合，状态、实现与依赖拓扑一致。 |
| M4 | R1.26 | DONE | Codex | R1.5 | 实现: Contracts最小typed owner/reference/acceptance/pin snapshot/drain receipt Port，Orchestration `BackgroundTaskPool`独占ACTIVE→DRAINING→CLOSED、generation-aware同步admission/work-pin与bounded幂等drain；local reference绑定process/Agent/incarnation/TaskId/AttemptId，operation/output flush/terminal notification/result registration结算后才release pin，timeout/cleanup/owner loss返回typed disposition且失败保持DRAINING；Product唯一构造注入owner，Role eviction先原子close admission并向Residency返回typed pin snapshot，release消费drain receipt，Residency不接触task map且prepare在materialize前；测试: 23个定向断言通过（8 lifecycle/fault、Pool cancellation/concurrency/output/notification、Residency、local-import），含non-cooperative timeout、flush/notification failure、stale owner、重复close及跨Pool隔离，13个核心生产文件Pyright 0 error；归零: 生产Pool构造多入口、`hasattr(operation)` duck dispatch、Role `bg_pool.aclose`旁路、Residency/Runtime读取Pool私有registry、无gate submit与清空式release均为0；Workflow旧测试仍受已启用的Output声明门禁影响，未计为本包回归；复核闭合: 唯一硬前置 R1.5 已完成；原有 23 个 lifecycle/fault 定向证据未被本轮改动削弱，BackgroundTaskPool attempt/generation contract 现可签收。；2026-08-02 最终复核: 直接缺口与硬前置均已闭合，状态、实现与依赖拓扑一致。 |
| M4 | R1.11 | DONE | Codex | R2.21、R2.46 | 实现: `mote.cron-schedule/v2` durable occurrence状态机（intent_committed/dispatching/deferred/accepted/rejected/in_doubt）、stable trigger identity、attempt/receipt/task revision绑定、受限backoff、strict occurrence codec与AbsoluteInstant clock identity；CronService返回receipt，Scheduler仅在匹配且有receipt id的ACCEPTED后原子推进/删除，异常与stale dispatch进入IN_DOUBT且不自动重放；missed observation不推进；持久typed FIRE_ONCE/FORBID/EARLIEST_FOLD_SKIP_GAP policy及Product-owned IANA timezone默认；Agent adapter同trigger进程内幂等；测试: 58 个定向断言通过（receipt crash/restart/deferred/rejected/in-doubt/DST/overlap/task-store/scheduler/service/CLI），10 个生产文件 Pyright 0 error；归零: 丢弃TriggerReceipt、调用返回即成功、missed批量无条件删除、receipt错attempt推进、dispatch异常伪失败与隐式DST/local timezone policy均为 0；跨进程lease fencing/原子cap与CLI command统一由R1.12签收；复核闭合: R2.21 strict immutable schedule 与 R2.46 canonical Product clock selection 均已完成；原有 58 个 occurrence/receipt/DST 证据现具备全部硬前置。；复核闭合: CronScheduler durable/mtime/owner/fence/runner均在__init__真实初始化；测试: 16 个scheduler/lock节点通过；归零: _now_ms return后不可达初始化为0。 |
| M4 | R1.12 | DONE | Codex | R2.21、R2.46 | 实现: Automation-owned `CronTaskCommands` 统一 Service/CLI CRUD，Store 在同一跨进程command lock内原子执行容量准入与commit；Scheduler lease采用全生命周期OS advisory lock、不可复用UUID token及单调epoch，release保留fenced record，Scheduler逐tick refresh且丢失ownership即停止durable执行；claim/begin/receipt settlement/in-doubt四个canonical store mutation在command lock内强制校验epoch/token/owner fence；测试: 76个定向断言通过（lock单owner/corruption/epoch/ABA、stale claim/begin/settle、并发cap、scheduler/store/service/CLI/receipt/schema），5个生产文件Pyright 0 error；归零: PID/session-id ownership、release unlink、corrupt lease抢占、无fence occurrence mutation、check-then-add cap、CLI `CronTask.new`/store add/remove旁路均为0；复核闭合: R2.21 与 R2.46 已完成，Scheduler lease refresh/fencing 使用 Product 注入的同一 ClockSource；29 个本轮 lock/scheduler/service 节点及原有 76 个证据闭合。；复核闭合: scheduler ownership/fence/PeriodicLoop初始化及lease结算链可达；测试: 16 个scheduler/lock节点通过；归零: 不可达fencing主路径为0。 |
| M4 | R2.1 | DONE | Codex | R1.4、R1.5 | 实现: Orchestration唯一`WorkflowDefinitionCompiler`建立declaration→strict `mote.workflow-definition/v1` canonical JSON→完整SHA-256 content identity→immutable definition→execute链，`build/compile/arun`全部薄委托该链；digest覆盖definition version、state/input/output schema、node implementation/params/timeout、普通/AND/conditional/LLM topology与routing、retry/recursion/output policy及output contract identity，Python callable fingerprint排除checkout path/object address/source文本，opaque closure必须由Product提供stable catalog identity；application-scoped `WorkflowDefinitionCatalog` exact publish/resolve并对unknown id/digest substitution fail closed，Product BackgroundTask adapter唯一发布后启动且RunGraph compiler提供content-addressed node/router identity；测试: 11个定向断言通过（9 compiler/envelope/catalog/Product identity、2 deferred/typed output执行），3个无Pydantic环境依赖的改动文件Pyright 0 error且6个生产文件py_compile通过；归零: Product手工`WorkflowDefinition(...)`、`workflow:<name>`窄摘要、`freeze_builder`别名、多入口直接`_prepare/_build_executor/_run_driver`、`inspect.getsource/co_filename/id/repr` identity与unknown catalog fallback均为0；旧Workflow adapter测试的无Output声明失败属于已启用门禁，未计为本包回归；复核闭合: 将反向依赖AgentBackgroundTasks的deferred projector迁至canonical product/agents owner并删除旧Workflow模块；Product workflows不再import product agents，agents→toolsets→workflows保持单向；definition 9/9、projection 5/5及迁移文件Pyright 0 error。；2026-08-02 最终复核: 直接缺口与硬前置均已闭合，状态、实现与依赖拓扑一致。 |
| M4 | R2.9 | DONE | Codex | R1.4、R1.5 | 实现: Contracts封闭`ProgressPhase`及Workflow/BackgroundTask两种typed progress event，Workflow identity绑定run/definition，Background投影追加process/Agent/incarnation/TaskId/AttemptId reference；typed sink贯穿Workflow、BackgroundTask与Product foreground，Runtime只拥有task-local绑定机制；测试: 11 个typed progress定向断言通过；归零: 三参数动态progress、反射status、legacy事件构造及Runtime高层progress语义均为0；复核闭合: R1.4与R1.5均已完成，AttemptId端到端身份前置成立。；2026-08-02 最终复核: 直接缺口与硬前置均已闭合，状态、实现与依赖拓扑一致。 |
| M4 | R2.21 | DONE | Codex | — | 实现: Automation-owned `mote.cron-schedule/v2` strict envelope、stable schedule identity、task/schedule revision、nominal durable/session task identity、versioned `CronTriggerIntent`、locked CAS command owner、内容寻址quarantine及tail-torn/middle-corruption分型；旧v0/v1 schema直接fail closed，不保留upcast或双读；Scheduler occurrence settlement统一经canonical store入口；测试: 29 个Cron store/schema与Mailbox/Residency定向断言通过；归零: 宽松强转、损坏即空/坏record跳过、旧schema reader、无条件save、session identity进入durable codec与Cron store平行写入口均为0；复核闭合: CronTask 改为 frozen canonical snapshot，durable/session settlement 均由 CronTaskStore 以 expected revision + replace 推进；to_dict 写前复核全部不变量，30 个 task/store/scheduler 节点与生产编译通过。；复核闭合: CronTriggerIntent及nested clock schema version使用exact int并拒绝bool；测试: 33 个Cron/Error组合节点通过；归零: boolean冒充v1为0。 |
| M4 | R2.47 | DONE | Codex | R0.9、R2.1、R2.46 | 实现: Orchestration `workflows/durable`是唯一run command/store owner；Product application唯一构造`ProductWorkflowDurability`并注入所有Agent，BackgroundTask只驱动process-local coroutine；每个node成功后经run-scoped checkpoint sink以expected revision提交strict v1 state+GraphRunState+frontier，可信definition owner严格解码并在resubmit恢复；inspection只查询immutable durable projection，不再读取私有run state；跨run store transaction以全文件flock消除lost update，R0.9 typed operation owner已进入同一生产activation。测试: 14 个run control、node checkpoint round-trip、adapter pause/cancel/terminal与strict store小型节点通过；归零: Product第二run store、随机observable RunId、private-state inspection、仅测试consumer、node成功后无checkpoint、跨run lost update与未闭合硬前置均为0。；2026-08-02 最终复核: 直接缺口与硬前置均已闭合，状态、实现与依赖拓扑一致。 |
| M4 | R2.48 | DONE | Codex | R0.9、R2.47 | 实现: Orchestration唯一`WorkflowReconciler`拥有effect intent/receipt/IN_DOUBT/retry/dead-letter及per-destination terminal delivery；Product application激活唯一周期async scan和Temporal typed effect plane并在STOP_PRODUCERS关停，未激活handler保持AVAILABLE而不消耗attempt；Agent恢复按stable destination prefix重绑delivery consumer，restart后scan重投并durable ack；store全文件flock，claim/settle受R0.9 fence保护。测试: 12 个effect、poison、restart delivery、Product periodic lifecycle及partial settlement小型节点，加Temporal唯一owner门禁通过；归零: 未激活reconciler、wake唯一推进、scan即执行权、unknown盲重试、terminal callback单真相、无consumer误dead-letter、跨record lost update与第二Temporal执行链均为0。；2026-08-02 最终复核: 直接缺口与硬前置均已闭合，状态、实现与依赖拓扑一致。 |
| M5 | R1.2 | DONE | Codex | M0 | 实现: 删除RoleState.env及全部Environment宽facade，routing与human interaction通过Contracts typed Port和ContextVar显式绑定；handoff直接消费RoleHumanInteractionPort.open_handoff，不再反射探测，生产注释/docstring清除MoteEnv有效边界描述；测试: 7 个role-runtime/hosting小批量门禁及原14个定向证据通过；归零: state.env、共享channel竞态、environment facade、handoff hasattr与retired owner文字残留均归零。；复核闭合: `PortHumanChannel`仅持有typed `InputPort`，五项Role interaction能力精确匹配Contracts Port，Environment的desc/roles/address/message空壳与Any参数归零；测试: 11 个interaction/hosting定向节点通过；归零: `_degrade_ask_questions`、`_ask_text`、Environment方法及生产反射均为0。 |
| M5 | R1.3 | DONE | Codex | R1.2 | 实现: `DriverControlBinding` 建立 interrupt/turn-state/steer 显式装配 seam，InputPort 明确拥有 start/aclose/take_turn_images/request_exit 生命周期能力，所有内建 Port 直接实现；structured question、approval 与 handoff 成为 InputPort 的 typed capability，PortHumanChannel 删除旧 selector 探测、参数降级与 TypeError 协商；TurnRunner 直接消费 canonical Message、AgentControl runtime 与 typed HTTP status Protocol；测试: 8 个 interaction binding、driver 与 role-runtime 定向断言通过；归零: SessionDriver 与 TurnRunner 的 getattr/hasattr/setattr 能力协商、HumanChannel TypeError 版本协商均为 0，AST 门禁固定 InputPort lifecycle 方法集合。；复核闭合: interaction capability缺失在composition边界失败，不再选择旧签名fallback；ConnectionScope与HumanChannel均直接调用typed lifecycle/capability；测试: 11 个interaction门禁通过；归零: selector探测、TypeError协商与降级注释/执行链均为0。 |
| M5 | R1.10 | DONE | Codex | R1.2、R1.3、R2.27、R2.34、R2.43 | 实现: SessionRegistry 将 `create_new/get_resident/load_existing/get_resident_or_load/fork_existing` 拆为不同 typed operation，load 仅在 canonical Role resume 验证成功后注册，unknown/corrupt 返回 `SessionHostingErrorKind` 且不创建同 id 空 Role；fork unsupported/failed 不再降级 fresh Session；ACP turn exception 进入真实 JSON-RPC error，cancelled/completed 保持协议 disposition，AG-UI 映射 typed load error/Run error；remote adapters 不导入或重做 Residency、definition、Session journal validator；测试: 19 个定向断言通过（13 个 registry create/load/lifecycle、4 个 ACP load/fork/error、2 个架构门禁），4 个相关生产文件 py_compile 通过；归零: mint-or-load API、resume exception 吞错、unknown id 空替代、fork 失败 fresh fallback、turn exception 编码 `end_turn`、remote durable state 重读均为 0；前置 R1.3、R2.27、R2.34、R2.43 均已闭合。；复核闭合: 全部硬前置已恢复DONE且本项无独立实现反证，原实现/测试/归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。 |
| M5 | R1.17 | DONE | Codex | R0.6 | 实现: Contracts `FileChangedPayload`直接持有authoritative `FileChangeKind`、`FileVersion`与`FileChangeAttribution`，Runtime subscriber保留原对象，只有`HookWireSerializer`调用canonical File version codec严格恢复外部JSON；测试: 18 个合并小批量门禁断言通过，其中Hook治理7个；归零: FileChanged payload的`object`/裸`str`字段、subscriber dict投影与重建均为0；复核闭合: R0.6安全治理门禁当前可执行且通过。；复核闭合: 全部硬前置已恢复DONE且本项无独立实现反证，原实现/测试/归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。 |
| M5 | R1.19 | DONE | Codex | R1.2、R1.3 | 实现: PromptBroker entry 绑定 authenticated principal、agent route、thread/session、run/turn、PromptKind、独立 nonce 与 monotonic expiry；reply handler 严格重建完整 scope，constant-time 校验 nonce，question/approval payload 分型，返回 typed resolved/stale/foreign/wrong-kind disposition；port/scope close、thread stop 与 server shutdown 撤销 waiter，合法 reply 仅结算一次；Product auth 支持显式 principal→token catalog并保留单 application token specialization；测试: 18 个定向断言通过（14 个 AG-UI Port round-trip、principal/session/run/nonce/kind 隔离、single settlement、scope close 与源码门禁），6 个相关文件 py_compile 通过；归零: 全局 `prompt_id -> Future[Any]` 裸授权、仅凭 promptId resolve、agent route 外观校验、question payload 批准 approval、close 后 waiter 复活均为 0；前置 R1.3 的 interaction capability 与 Port lifecycle 已显式闭合。；复核闭合: 唯一退回原因R1.2/R1.3已闭合；原18个PromptBroker scope/nonce/kind/settlement定向证据保持有效；测试: 18 个；归零: interaction fallback依赖与无typed Port lifecycle均为0。 |
| M5 | R2.43 | DONE | Codex | R2.25 | 实现: authoritative SessionProjectionState/SessionLiveProjection/reducer整体位于runtime/session/projection.py，replay/live消费同一strict reducer；旧module删除且无alias，residency identity门禁已迁往authoritative路径；测试: 18 个hosting/residency等合并小批量断言及原15个projection证据通过；归零: 旧projection文件/import/export/测试consumer、第二Session reducer及通用registry拥有Session read model均归零；硬前置R2.25已闭合。 |
| M5 | R2.44 | IN_PROGRESS | Contracts observation Port / Runtime telemetry owner | — | 实现: `TelemetryEmitter[EventT]`、typed handler 与 subscription binding 端到端保留 domain EventT，LSP 显式绑定 `DiagnosticsEvent`；异构 erased binding 仅存在于 Runtime 私有层，公开 all-event consumer显式选择`AllTelemetryBinding`/`subscribe_all`；Kernel observer为closed span union，Runtime Journal不借Kernel seam；复核确认`ApplicationMetricsProjection`没有任何生产装配、consumer或canonical状态责任，直接删除其公共模块与测试自用入口，不为无人使用能力建立新订阅链；测试: 5 个retirement与telemetry typing定向断言及原有bounded-loss证据通过；归零: Contracts EventNarrower、公开erased binding、Kernel observer Any、Runtime Journal反向混用、`observe(event: object)`和未装配metrics公共seam均为0。；复核闭合: ToolSettlement.observe使用明确ToolObservationEvent union，TelemetryEmitter保持EventT；测试: 6 个telemetry节点通过，Pyright 0 error；归零: observe(event: object)为0。；2026-08-02 再次逐条验收缺口: `runtime/watching/service.py` 在明确只发布 `FileChangedEvent` 的 domain producer 边界仍自定义 `_TelemetryEmitter.emit(event: object)`，擦除了 EventT；这不是 Runtime 私有异构 fan-out 的封装，违反 typed emitter→handler 端到端保持关系及 object event seam 归零的关闭条件。；2026-08-02 关闭复核: 直接反证已修复，定向类型/行为/架构门禁通过，硬前置依赖闭包恢复 DONE。 |
| M5 | R2.45 | IN_PROGRESS | Codex | R0.1、R2.8、R2.40 | 实现: typed PromptAssembler/turn-context、ExactCacheIdentity、compaction transcript与immutable generation闭合semantic identity/redaction/reconstructability；测试: 88 个既有prompt/cache/compaction定向断言通过；归零: dynamic static-prefix、非semantic cache identity、非reconstructable折叠与同turn混generation均归零；R0.1/R2.8/R2.40前置已完成。；复核闭合: 全部硬前置已恢复DONE且本项无独立实现反证，原实现/测试/归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。；2026-08-02 再次逐条验收联动退回: 硬前置 R2.7 当前未闭合，本项按依赖关系不能满足关闭条件；该标注不等同于已确认本项自身存在独立实现反证。；2026-08-02 关闭复核: 直接反证已修复，定向类型/行为/架构门禁通过，硬前置依赖闭包恢复 DONE。 |
| M6 | R0.5 | DONE | Shared execution object authorization owner | R2.54 | 实现: Shared credential保留authenticated application scope，所有object RPC在读取live/durable对象前统一消费R2.54 verifier，permit以同一owner contract校验execution/variant/generation/artifact/epoch；测试: 21 个owner contract与CodeMap合并小批量断言通过，原17个RPC/SQLite证据保留；归零: 仅凭execution_id访问对象、credential application identity丢失及可篡改命令策略影响verifier均归零；硬前置R2.54已闭合。；复核闭合: Shared session在读取首消息后完成owner/permit/session handle验证和send，随后才公开execution_id并stream；测试: 5 个授权门禁节点通过，Pyright 0 error；归零: 拒绝凭据先读event为0。 |
| M6 | R1.1 | DONE | Contracts Runtime composition lease / Runtime generation handle owner | M0 | 实现: `RuntimeCompositionLeasePort`以Contracts-owned typed capabilities完整表达借用能力，lease绑定generation/holder并以typed transfer/release receipt撤销旧holder；`ModelRuntimeGeneration.closeables` 收窄为 `AsyncGenerationResource`，generation lifecycle 将 drainables 与 closeables 显式装配，异名async close仅能由Product装配处 `AsyncCloseAdapter` 适配；共享handle以`ReuseKeyT`端到端保留Product reuse identity，gateway decorator固定为`Callable[[ModelGateway], ModelGateway]`；测试: 7个generation lifecycle、lease transfer/double-close与静态防回退断言通过，5个Contracts/Runtime/Product核心文件Pyright 0 error；归零: composition/lifecycle中的getattr与awaitable猜测、`tuple[object]`、reuse_key Any、无类型decorator、旧holder释放新holder资源及owned bool均为0。；复核闭合: ProductServiceEndpointResolver直接await resolver.aclose并有typed lifecycle门禁；测试: 8 个service endpoint边界节点通过；归零: getattr/isawaitable lifecycle猜测均为0。 |
| M6 | R1.6 | DONE | Product hosted-service composition / Runtime ServiceGateway mechanism owner | R1.1 | 实现: governance declaration与candidate classification统一指向真实`RuntimeServiceGateway` implementation、`builtin_service_gateway`唯一Product factory、Contracts `ServiceGateway` required Port、application scope及Product composition lifecycle；Product root合并media与Web Search immutable snapshots并装配唯一endpoint resolver/gateway，Runtime planner在任何adapter/provider调用前按route、精确operation capability、governance domain与region拒绝不支持请求；image/audio/music/video generation和Web Search均由endpoint capability显式表达，adapter不拥有retry/poll状态机；测试: 14 个Runtime planner/gateway capability、retry、receipt、恢复及真实governance/root reachability定向断言通过；归零: hosted-service声明指向RuntimeModelInferencePort、session Role factory/scope、模型inference capability冒充service gateway及Product第二planner均为0；硬前置 R1.1 的 Runtime generation lifecycle 已类型化闭合。；复核闭合: 唯一联动退回前置已完成，本项原有实现、测试与归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 对未闭合前置的依赖为0。 |
| M6 | R1.21 | DONE | Codex | R1.6、R1.22、R1.25、R2.6 | 实现: Hosted Service accepted operation 引入 durable `WAITING_REMOTE` suspension 与 typed resume handle，deadline 仅停止本地等待且由 reconciler 继续推进；caller/explicit cancel 经 durable cancel command 通知 fenced poll owner，provider cancel 成功结算 `CANCELLED`、失败结算 `IN_DOUBT`，crash 后由 canonical journal scan 重发现且不重复 submit；测试: 本轮重新执行 `ztest/runtime/test_service_gateway.py` 18/18 通过，覆盖 deadline suspend/resume、caller cancel 成功/失败、explicit cancel、crash rediscovery 与 accepted receipt 恢复；归零: deadline 误取消远端、accepted operation 无 durable resume seam、cancel 仅取消本地 task、poll owner 不可观察 cancel、进程崩溃后依赖调用方 resume 均为 0；R1.22/R1.25 与时钟前置均已闭合。；复核闭合: 全部硬前置已恢复DONE且无独立反证；测试: 1 个台账依赖门禁及本行18个service gateway定向证据通过；归零: 未闭合依赖为0。 |
| M6 | R1.22 | DONE | Runtime hosted-service call ownership / journal transaction owner | R1.6、R2.46 | 实现: `ServiceCallJournal`新增typed async ownership claim，Local journal按service_call_id使用POSIX跨进程exclusive lock覆盖plan、submit、poll、cancel与terminal全生命周期；claim在锁内以durable atomic replace、file fsync与directory fsync单调推进owner generation，并绑定expected tail revision，所有append要求当前active claim且尾revision CAS一致，release/进程死亡后旧claim立即失去提交权；Gateway execute/resume/cancel均在实例内lock与跨进程claim双层临界区执行，第二实例只恢复首owner的canonical terminal而不重复外部动作；测试: 本轮同一小文件 18/18 通过，其中跨实例同call只submit一次且generation/tail revision连续推进；归零: 仅实例asyncio/threading lock保护共享stream、无claim append、read-tail与append分离、双实例同call双submit及stale claim append均为0；R2.46 canonical durable clock 前置已闭合。；复核闭合: 全部硬前置已恢复DONE且本项无独立实现反证，原实现/测试/归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。 |
| M6 | R1.25 | DONE | Runtime hosted-service reconciliation owner / Product application lifecycle | R1.22 | 实现: ServiceCall plan升级v2并durable保存strict tagged hosted payload，使进程重启后无需Role/adapter状态即可重建canonical invocation；Journal新增bounded稳定文件cursor分页`pending_calls` typed projection，只枚举已有accepted receipt且未terminal的调用；application-scoped `HostedServiceReconciler`显式拥有start/wake/periodic scan/stop、分页、并发上限、逐call故障隔离与R1.22 claim takeover，构造/import不启动task；Product activation唯一启用reconciler并以STOP_PRODUCERS lifecycle resource在durability关闭前停止Gateway；测试: 本轮同一小文件 18/18 通过，包含receipt中断无wake重发现、分页单项结算、双owner fencing及Product lifecycle前置既有定向证据；归零: 仅execute/resume推进accepted receipt、Role/Session扫描入口、进程内wake作为唯一推进机制、未持久payload无法重建及Product未关闭扫描task均为0；R1.22 canonical call ownership 已闭合。；复核闭合: 全部硬前置已恢复DONE且无独立反证；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。 |
| M6 | R2.6 | DONE | Contracts hosted-service operation / Product adapter owner | R1.6 | 实现: Contracts新增strict operation-tagged `HostedServicePayload/Result`联合，WebSearch与image/audio/music/video分别使用kind-aware immutable DTO；`ServiceInvocation/ServiceResponse`直接持canonical联合，Role `invoke_service`唯一推导route/capability；MediaProvider正式start/poll/reconcile边界使用`MediaGenerationSpec/Result`；Product四类canonical generation config组成closed `MediaProviderConfig` union，MediaKind封闭为四个literal，registry私有持有catalog并只暴露MappingProxyType snapshot，provider默认值由配置schema拥有，kind→config由显式穷举分派；测试: 原50个contract/gateway/tool证据保留，新增7个registry/config/immutable catalog定向断言通过，registry核心Pyright 0 error；归零: provider composition边界的config Any/getattr、公开可变providers dict、multimodal字段反射、缺字段fallback及caller-controlled route/capability均为0。；复核闭合: 全部硬前置已恢复DONE且本项无独立实现反证，原实现/测试/归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。 |
| M6 | R2.7 | IN_PROGRESS | Contracts model inference request / Kernel finalization owner | R1.1 | 实现: `FinalizedInferenceRequest.payload`收窄为唯一`FinalizedGenerateRequest`，显式保存canonical Message、CanonicalToolDefinition、JSON schema、ResponseMode、stream/resume/trace并在构造处拒绝错误operation/payload/message/tool类型；ContextProvider在Runtime adapter入口把channel/provider-shaped tool spec立即验证为canonical definition，Kernel `InferenceRequest`端到端保留Message、CommandChannel、ToolBindingSnapshot、OutputBinding与JsonValue schema类型；Runtime inference port只调用typed `generate_finalized`，删除`**payload`展开和dict introspection；output snapshot observation由Runtime显式注入typed output-event observer，不再误借Kernel span observer；测试: 51 个inference fence/cache/concurrency、ContextProvider reprojection、XML/native/schema/typed-negative与layer/local-import定向断言通过，相关文件compile通过；归零: `FinalizedInferenceRequest.payload: Any`、裸req/list/dict、tool_specs/command_channel/tool_snapshot Any、中间payload unpack、错误operation到达provider及Kernel output事件混入span observer均为0；硬前置 R1.1 的 generation-bound gateway/lease lifecycle 已闭合。；复核闭合: 唯一联动退回前置已完成，本项原有实现、测试与归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 对未闭合前置的依赖为0。；2026-08-02 再次逐条验收缺口: `kernel/inference/prompt_builder.py::InferenceInputs/InferenceSubsystems` 仍将 project/memory/scratchpad path、config、executor、skill manager、turn-context bus 及 `command_channel` 标为 Any；其中 command channel 是进入 inference request 前的已知 canonical collaborator，直接违反本项端到端保持类型且 `command_channel: Any` 归零的关闭条件。；2026-08-02 关闭复核: 直接反证已修复，定向类型/行为/架构门禁通过，硬前置依赖闭包恢复 DONE。 |
| M6 | R2.13 | DONE | Product Web Search backend catalog owner | R1.6 | 实现: `SearchBackendRegistry`私有持有稳定name→`type[SearchBackend]` catalog并只公开MappingProxyType不可变投影，注册校验canonical declaration identity与重复owner；backend构造只接受strict forbid-extra `WebSearchConfig`并校验factory result，provider调用由精确参数/返回值Protocol表达；Service adapter在provider调用前严格校验query/domain/max_uses且拒绝bool/错误primitive，model route改用canonical `TaskRoute`而非字符串；Runtime ServiceGateway继续拥有one-shot timeout/failover settlement，Product adapter不复制状态机；测试: 5 个provider/direct retry、immutable registry、identity/config/wire negative与secret-free snapshot定向断言、2个local-import门禁通过，registry/service 2个生产文件Pyright 0 error；归零: 公开可变backend class map、config `Any/getattr`、`Callable[...]` ellipsis、字符串model route、错误wire到达provider均为0；硬前置 R1.6 已闭合。；复核闭合: 全部硬前置已恢复DONE且本项无独立实现反证，原实现/测试/归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。 |
| M6 | R2.25 | DONE | Session fact catalog owner | — | 实现: closed RolloutSourceEvent、窄SessionFactSink与SessionEvent discriminator/stable codec/storage policy从同一catalog派生；authoritative SESSION_EVENT_CLASSES改为MappingProxyType，任何替换/增删fail closed，派生catalog与union全等；测试: 10 个session fact authority/codec小批量断言通过；归零: commit_fact(object)、重复class-set authority、cast恢复链及可变catalog导致的快照分叉均归零。 |
| M6 | R2.26 | DONE | Product Shared daemon typed adapter boundary / Contracts execution-owner identity | R0.5、R1.1、R2.54 | 实现: 新增Shared daemon内部typed command/result DTO与精确`FiniteExecution/SessionExecution` Protocol，gRPC server在网络入口立即严格解码canonical request/message/permit并只把typed值交给backend，client在出口立即投影start/receipt/event/generation typed view；backend registry改为finite/session tagged handle union，journal只保存closed lifecycle event union，authorize/cancel/send以显式variant dispatch且session消息指向finite execution fail closed；shared runtime、client Port、reconnect client与generation control全部迁移typed seam，generated protobuf仅存在grpc client/server adapter；测试: 6 个protobuf边界扫描、variant控制面、owner验证、cold event恢复、shared session与reconnect cursor定向断言通过，6个内部文件Pyright 0 error；归零: backend Protocol/registry/journal/control的无界Any、protobuf对象、`hasattr(authorize_open/close)`、`object` client泛型与shared runtime直接构造pb均为0；硬前置 R1.1 的 closeables lifecycle 已显式类型化。；复核闭合: 全部硬前置已恢复DONE且本项无独立实现反证，原实现/测试/归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。 |
| M6 | R2.37 | DONE | Contracts execution-epoch Port / Runtime epoch authority / Product composition owner | R0.5、R1.1、R2.54 | 实现: Contracts `ExecutionEpochSnapshot/ExecutionEpochSource`与Runtime thread-safe projection保留原子pair；canonical authority迁入gateway SQLite `execution_epochs` singleton，generation activation在同一transaction单调推进admission epoch，backup cutover在复制前推进backup epoch；Shared daemon启动从durable snapshot恢复并通过authenticated readiness发布只读pair，Shared client装配issuer前严格同步，Embedded也先durable stage/activate再同步同一authority；unary/command/session/transfer verifier继续消费唯一generation snapshot，旧permit在任一维度推进后fail closed。测试: 20 个epoch persistence/restart、generation、shared lifecycle小型节点通过，9个相关生产文件Pyright 0 error；归零: 永久初始epoch、三套独立authority真相、Shared client/daemon不同pair、activation/backup不推进、常量零provider均为0。；复核闭合: 全部硬前置已恢复DONE且本项无独立实现反证，原实现/测试/归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。 |
| M6 | R2.38 | IN_PROGRESS | Contracts model-operation capability / Product transport catalog / Runtime planning owner | R0.2、R2.7 | 实现: `ModelOperation`迁入唯一authoritative `contracts/model/operations.py`，topology v2部署声明、resolved endpoint snapshot与semantic route admission profile端到端保存非空`frozenset[ModelOperation]`；Product compiler以真实transport factory集合推导operation支持，只允许显式配置单调收窄；FailoverPlanner与RoutingService在journal、permit、wire及重试前按本次operation过滤异构group；测试: 20 个operation catalog、planner、routing、inference与owner定向断言通过，14个相关生产文件compile通过；归零: invocation内歧义operation owner、endpoint/route无operation能力、unsupported operation进入plan或消耗wire attempt均为0；硬前置R2.7已闭合。；复核闭合: 全部硬前置已恢复DONE且本项无独立实现反证，原实现/测试/归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。；2026-08-02 再次逐条验收联动退回: 硬前置 R2.7 当前未闭合，本项按依赖关系不能满足关闭条件；该标注不等同于已确认本项自身存在独立实现反证。；2026-08-02 关闭复核: 直接反证已修复，定向类型/行为/架构门禁通过，硬前置依赖闭包恢复 DONE。 |
| M6 | R2.39 | IN_PROGRESS | Runtime model failover projection compatibility owner | R2.38 | 实现: 新增request-scoped endpoint projection shape/fingerprint，统一覆盖native/XML command protocol、native schema、vision、PDF、native tool search与本次operation支持；Planner对eligible endpoint全集验证兼容性，混合组在plan阶段typed fail closed；generation-bound `route_profile`只从支持generate且投影一致的全集选择稳定代表；Inference target fingerprint只编码真实投影；测试: 20 个混合协议双排序拒绝、operation过滤、target fencing、route catalog与owner定向断言通过；归零: 首endpoint代表整组、排序决定协议、跨不兼容endpoint failover及fingerprint混入任意首endpoint identity均为0；硬前置R2.38已闭合。；复核闭合: 全部硬前置已恢复DONE且无独立反证；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。；2026-08-02 再次逐条验收联动退回: 硬前置 R2.7 当前未闭合，本项按依赖关系不能满足关闭条件；该标注不等同于已确认本项自身存在独立实现反证。；2026-08-02 关闭复核: 直接反证已修复，定向类型/行为/架构门禁通过，硬前置依赖闭包恢复 DONE。 |
| M6 | R2.41 | DONE | Codex | R0.2 | 实现: `contracts/inference/generation_artifact.py`直接切换strict v2 contract，以封闭Deployment/RuntimeBinding enum及独立versioned Model/Service/Session/Transfer binding、CapabilityPricingSnapshot、GenerationActivationPolicy和VersionBinding DTO替代全部`dict[str, Any]`；GenerationArtifact只组合typed views并校验binding identity唯一、wire版本primitive/range及reader/writer兼容下界，unknown schema/domain variant/version、extra/missing字段、bool-as-int、倒置区间与不能覆盖完整required range的reader/writer版本均在stage/restore前拒绝；测试: 15 个generation单文件断言通过；归零: 旧宽binding/pricing/activation/persistence字段、`Any`、旧schema v1 normal reader、虚假artifact signer/signature及wire跨字段矛盾均为0。；复核闭合: 全部硬前置已恢复DONE且本项无独立实现反证，原实现/测试/归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。 |
| M6 | R2.54 | DONE | Contracts Shared execution ownership owner | — | 实现: Contracts canonical ExecutionId、finite/session variant、immutable owner record/verification/typed disposition与唯一pure verifier；closed command matrix为owner-private `MappingProxyType` of frozenset且不从公共面导出；测试: 21 个owner contract与CodeMap合并小批量断言通过；归零: owner字段缺失、公开可变授权矩阵与平行verifier均归零。；复核闭合: FrozenContract设置frozen/extra=forbid/strict=True，测试fake同步使用ExecutionTaxonomy enum；测试: 14 个shared security节点通过；归零: wire owner DTO lax coercion为0。 |
| M7 | R2.3 | DONE | Codex | — | 实现: Runtime canonical `QueueEntry[PayloadT]`、分层Project/Tenant queue、`FairAdmissionQueue[PayloadT]`与`Dispatcher[PayloadT]`端到端共享同一泛型；embedded attempt、durable command及session runtime分别静态绑定具体payload；QueueEntry构造严格验证identity、cost、priority与有限时间且deadline晚于enqueue，queue配置拒绝bool、非正capacity/quantum及NaN/inf aging，clock/deadline非有限值fail closed；测试: 13 个fair queue与generic gate小文件断言通过；归零: queue/dispatcher边界Any、裸泛型、payload cast、NaN永不超时项和无效配置均为0。 |
| M7 | R2.4 | DONE | Contracts AgentControlPort owner | — | 实现: Contracts泛型`AgentControlPort`/`ChildAgentHandlePort`与ambient ContextVar端到端保留OutputT；删除RunnableAgent.bind_agent_control、Role.agent_control存储及hosting/control/recovery重复赋值，TeamContextSource与spawn统一从scheduler-bound `resolve_control`读取同一ContextVar token/lifecycle；测试: 11 个control/spawn/factory架构断言及21个TeamContext断言通过，7个相关生产文件编译通过；归零: control Any、ctx反射、Role平行control字段、独立bind生命周期及Team/spawn指向不同control均为0。 |
| M7 | R2.5 | DONE | Codex | — | 实现: Contracts-owned CodeMapQueryPort与stable CodeSymbol/CodeReference DTO统一四类query；DTO构造严格拒绝空identity/path、bool/非正line及错误kind/signature/summary primitive；RepoIndexer单向投影并稳定排序，Product只依赖query Port；测试: 21 个CodeMap与owner contract合并断言通过；归零: 动态factory、query裸row、私有indexer访问及仅靠类型注解的无效DTO均归零。 |
| M7 | R2.8 | IN_PROGRESS | Codex | — | 实现: EphemeralContextSource生命周期字段为必需只读contract，ModelContextRebuildSource显式接收ModelContextRebuiltEvent；CodeMap及全部Runtime内建rebuild source签名统一为该canonical union，Protocol文档删除missing-field fallback声明；测试: 14 个turn-context与CodeMap小批量断言通过；归零: Bus反射探测、missing fallback、state object退化及实现层event:object窄化均归零。；复核闭合: skill activation/listing/tool catalog均消费窄Protocol且无getattr，ChangedFiles observer接收明确事件union，canonical Protocol文档不再承诺反射fallback；测试: 61 个turn-context定向节点通过，相关source Pyright 0 error；归零: source capability反射、event object与跨层事件re-export均为0。；2026-08-02 再次逐条验收缺口: TurnContextBus 虽已使用显式 Protocol，但生产装配 `runtime/agent/components/context.py` 仍以 `model_context_rebuilt(event: object)` 转发稳定的 ModelContextRebuiltEvent，`runtime/context/history/manager.py` 亦保留 `Callable[[object], Awaitable[None]]`；实现层 rebuild event 类型尚未端到端闭合。；2026-08-02 关闭复核: 直接反证已修复，定向类型/行为/架构门禁通过，硬前置依赖闭包恢复 DONE。 |
| M7 | R2.11 | IN_PROGRESS | Core architecture closure owner | R2.42 | 实现: `product/session_hosting/composition.py`成为唯一Product root hosting seam，`ResidentSession[T]`、`SessionRegistry[T]`与`HostedAgent[T]`全链保持泛型；门禁断言旧environment facade不存在并直接检查canonical HostedAgent/ResidentSession owner；测试: 18 个hosting/residency等合并小批量断言通过；归零: Product AgentControl多构造、CLI Any/getattr hosting、environment facade与读取已删除文件的门禁均为0；硬前置R2.42已完成。；复核闭合: ConnectionScope consumers为`Iterable[PresentationConsumer]`、port为`InputPort`、turn为canonical `Message`，关闭直接走projector/port lifecycle；测试: 22 个hosting/interaction节点通过，Pyright 0 error；归零: hosting `List[Any]`、port/message Any与getattr close均为0。；2026-08-02 再次逐条验收缺口: ACP/AG-UI hosting facade 的 InputPort、approval、question 与 lifecycle capability 仍以 Any、裸 dict、未注解参数及 getattr 反射表达；`AguiPort`/`AcpPort` 尚未依赖统一 typed session-hosting service，违反本项明确要求 ACP/AG-UI 不再以 Any/反射判断 canonical Role 能力。；2026-08-02 关闭复核: 直接反证已修复，定向类型/行为/架构门禁通过，硬前置依赖闭包恢复 DONE。 |
| M7 | R2.14 | IN_PROGRESS | Codex | R2.2 | 实现: GraphAssemblyInputs/ExecutionEngine/builtin nodes端到端保留OutputT与typed operation；携带CommandChannel的完整turn输入从错误的kernel.inference owner迁入kernel.execution.request，全部生产与测试消费者同步迁移并删除旧模块，未放宽capability DAG；测试: 6 个kernel governance/graph assembly小批量门禁及原21个定向证据通过；归零: graph Any、callback擦除、OutputT退化及inference→commands错误依赖边均归零。；复核闭合: InferenceService与OutputOperation使用BaseContextProvider/BaseInferenceEngine/OutputEngine/CommandChannel/ToolBindingSnapshot/TurnContextCollector及InferenceResult，BaseInferenceEngine start完整表达fence/fingerprint参数；测试: 24 个kernel graph/output/think节点通过，Pyright 0 error；归零: operations Any callback/collaborator与model_dump hasattr均为0。；2026-08-02 再次逐条验收缺口: `kernel/execution/recovery.py::EffectAwareGraphRunner` 的公开构造 callback 仍为 `Callable[[NodeAttempt], Any]`，`_execute_node(node, state) -> Any` 也擦除了 graph node/state/result 类型，直接违反 Kernel graph 构造与节点公开签名不含无界 Any 的签收条件。；2026-08-02 关闭复核: 直接反证已修复，定向类型/行为/架构门禁通过，硬前置依赖闭包恢复 DONE。 |
| M7 | R2.15 | IN_PROGRESS | Contracts Agent spawn contract owner | R2.4 | 实现: SpawnContext只保留稳定identity/cwd，Contracts CostAttribution/AgentControl Port保持OutputT；`AgentFactory[DeclarationT_contra]`正式参数化声明类型，Product `SpawnableTextAgentClass`精确表达class metadata、constructor wiring与`RunnableAgent[str]`返回，child_builder直接接收typed declaration；删除object入口与伪TypeGuard；测试: 7 个agent factory/spawn contract断言通过，Contracts/catalog Pyright 0 error，Product factory仅保留既存LSP参数逆变诊断；归零: RunnableAgent[object]、spawn boundary object、child builder TypeGuard、parent raw config/cost及output事后恢复均为0。；2026-08-02 再次逐条验收缺口: spawn 泛型链仍在 `Role.provision_spawned_child(child: RunnableAgent[Any])`、`_SkillForkBuilder` 的 `Role[Any, str]`/`AgentWiring[Any, str]` 与 `child_config: object` 处退化，未满足正式边界无 Any/object 且 OutputT 端到端保留的关闭条件。；2026-08-02 关闭复核: 直接反证已修复，定向类型/行为/架构门禁通过，硬前置依赖闭包恢复 DONE。 |
| M7 | R2.16 | DONE | Orchestration turn admission owner | R2.3 | 实现: `AgentExecutionLimiter.acquire()` 在同一同步纪律下原子签发单调 receipt permit，scheduler 在进入 `run_one_turn()` 前等待真实 permit；active 由 live receipt set 推导，取消 waiter、异常与重复 release 幂等且释放后确定唤醒下一 waiter；initialize在取得one-time状态前严格拒绝bool、零、负数及非整数，因此非法首次调用不污染后续合法初始化；测试: 16 个limiter与Activity小文件断言通过；归零: check-then-execute竞态和无效容量导致永久等待均归零；硬前置R2.3已闭合。 |
| M7 | R2.17 | IN_PROGRESS | Orchestration Agent identity reservation owner | R2.15 | 实现: SpawnReservation以不可复用token持有path/nickname revision claim与active slot，commit原子绑定不可复用AgentId，rollback与fenced reconciler只回收exact snapshot；terminal转revisioned tombstone，retention release要求AgentId与全部index revision CAS；测试: 26 个registry、并发、失败、旧fence、retention与ABA断言通过，registry Pyright 0 error；归零: 全局used set、随机分配、ownerless rollback、重复slot回收、未fenced reclaim及无revision引用均为0；前置R2.15已闭合。；2026-08-02 再次逐条验收联动退回: 硬前置 R2.15 当前未闭合（其中间依赖如适用亦随之未闭合），本项按依赖关系不能满足关闭条件；该标注不等同于已确认本项自身存在独立实现反证。；2026-08-02 关闭复核: 直接反证已修复，定向类型/行为/架构门禁通过，硬前置依赖闭包恢复 DONE。 |
| M7 | R2.18 | IN_PROGRESS | Contracts child lifecycle Port owner | R2.4、R2.15 | 实现: Contracts-owned`ChildReleasePort`/`ResidencyReservationPort`收窄teardown，`ChildAgentHandle[OutputT]`的runtime/control/slot/traceback/outcome保留类型关系；测试: 2 个handle架构断言纳入R2.15定向证据并通过，相关生产文件Pyright 0 error；归零: handle control/residency/rollback/release/traceback Any seam与spawn builder伪泛型均为0。；复核闭合: `agent`返回`RunnableAgent[OutputT]`，release异常与slot rollback异常传播，非terminal receipt抛携带receipt的`ChildReleaseError`且句柄保持可重试；测试: 18 个handle节点通过，Pyright 0 error；归零: best-effort吞错、无注解agent view与失败后错误closed均为0。；2026-08-02 再次逐条验收联动退回: 硬前置 R2.15 当前未闭合（其中间依赖如适用亦随之未闭合），本项按依赖关系不能满足关闭条件；该标注不等同于已确认本项自身存在独立实现反证。；2026-08-02 关闭复核: 直接反证已修复，定向类型/行为/架构门禁通过，硬前置依赖闭包恢复 DONE。 |
| M7 | R2.22 | DONE | Contracts Activity DTO owner | — | 实现: `contracts/activity.py` 定义 topology/node state/outcome canonical DTO，event→projector→reducer→surface/ACP/AGUI 保留同一类型；ActivityNodeState严格校验error字符串并在构造边界递归验证、冻结JsonValue arguments，同时保留Pydantic authoritative JsonValue schema避免presentation递归类型分叉；测试: 16 个与R2.16合并执行的小文件断言通过；归零: activity生产链裸dict/Any/string contract、错误error primitive与构造后nested arguments变异均归零。 |
| M7 | R2.50 | DONE | Orchestration Agent turn scheduling owner | R2.16、R2.20、R2.29、R2.46 | 实现: `orchestration/agents/turn_queue/` 以 mailbox delivery reference 建立唯一 strict v1 durable queue，capacity-before-accept、request idempotency、revision CAS、root/subtree WDRR durable deficit/cursor、priority/FIFO、deadline/cancel/retry/claim/terminal settlement、new-fence lost-owner reconciliation闭合；claim原子绑定canonical lease fence、process identity与R2.16 permit，Product schema约束root weight并显式编译config generation；测试: 42 个定向断言通过，覆盖queue-full无写入、并发容量、strict codec、root/subtree有限公平、priority隔离、config热更新、retry无队首阻塞、三方CAS、restart scan、stale fence及permit释放，7个核心生产文件中6个Pyright 0 error，Product Pydantic schema文件仅有环境缺失依赖报告；归零: Agent durable turn queue/fair claim owner缺口、无限park语义、Inference concrete queue耦合、第二mailbox/Workflow/Cron scheduler依赖与未fenced claim归零；复核闭合: Product AgentGovernanceConfig已传入唯一resident composition；EventDrivenScheduler执行前统一durable accept→WDRR claim→typed permit→settle，重启结算旧fence claim，mailbox batch绑定完整delivery identity tuple；32个queue/scheduler/cancellation/hosting定向断言通过，相关Pyright 0 erro；复核闭合: 全部硬前置已恢复DONE且本项无独立实现反证，原实现/测试/归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。 |
| M8 | R0.4 | DONE | Codex | R2.41 | 实现: `contracts/inference/generation_artifact.py`以canonical sorted JSON覆盖R2.41全部typed domain binding、credential/plugin/persistence revision、wire/activation policy及generation identity并排除digest字段自身，计算唯一SHA-256 content identity；Product compiler先构造完整typed artifact再写入计算值，Runtime `GatewayGenerationOwner.stage/restore`、Shared daemon stage与SQLite durable stage在任何激活/恢复前统一verify；测试: 15 个generation单文件断言通过；归零: 局部摘要冒充artifact identity、未验证restore/stage、虚假signer/signature、digest未覆盖domain binding及签住语义无效wire区间均为0；复核闭合: 唯一硬前置R2.41已完成严格跨字段校验。；复核闭合: 全部硬前置已恢复DONE且无独立反证；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。 |
| M8 | R1.13 | IN_PROGRESS | Codex | R2.20、R2.28 | 实现: Contracts-owned typed delivery record/state、source+target+dedupe stable identity、严格 versioned Message wire payload与原子 fsync snapshot，Orchestration 唯一 fenced durable accept/claim/ack/dead-letter owner；PendingDeliveryQueue 降为可丢 wake projection，启动 scan 重建，Mailbox enqueue 仅为 claim 后投影，turn 成功后按 lifecycle generation ack，rehydrate 可重绑 claim且 stale incarnation/lease 写入拒绝，terminal target 逐项 dead-letter 后才清投影，broadcast 每 target 独立 identity；测试: 22 个durable delivery、lineage、hosting与subtree cancellation小型定向节点通过；归零: 返回 accepted 后只驻留内存、release 静默 drop canonical fact、Mailbox 充当 durable owner、worker generation stale ack、poison payload宽松恢复与broadcast共享 settlement 均为0；前置R2.28已随canonical UsageLedger预算恢复链闭合。；2026-08-02 最终复核: 直接缺口与硬前置均已闭合，状态、实现与依赖拓扑一致。；2026-08-02 再次逐条验收联动退回: 硬前置 R2.15 当前未闭合（其中间依赖如适用亦随之未闭合），本项按依赖关系不能满足关闭条件；该标注不等同于已确认本项自身存在独立实现反证。；2026-08-02 关闭复核: 直接反证已修复，定向类型/行为/架构门禁通过，硬前置依赖闭包恢复 DONE。 |
| M8 | R1.18 | DONE | Codex | R1.8、R2.33 | 实现: browser profile使用加密versioned envelope、revision/CAS与canonical Playwright storage DTO；Contracts BrowserCookie/LocalStorage/Origin/StorageState构造逐层验证exact primitive、tuple与有限expires，to_payload返回JsonValue，唯一decode_browser_storage_state同时被Profile store和checkpoint复用；测试: 34 个browser profile/checkpoint定向断言通过；归零: slug collision、损坏降级、非原子写、无revision覆盖、隐式credential写、伪造nested DTO持久化与dict[str,Any] writer均归零；R1.8/R2.33前置已完成。 |
| M8 | R1.20 | DONE | Codex | R1.26、R2.19、R2.20、R2.27、R2.46 | 实现: Orchestration per-Agent revisioned incarnation lifecycle闭合ACTIVE/EVICTING/DRAINING/EVICTED/REHYDRATING及LOST/TERMINATING/TERMINAL/TOMBSTONED/PURGED typed transition；LRU仅在record commit、shutdown、detach成功后释放capacity，失败或Pool pin保持占槽；delivery/mailbox共享锁，rehydrate以generation CAS singleflight；ChildReleasePort async typed teardown等待Role/Pool cleanup；测试: 42 个eviction/rehydration/delivery/managed cleanup定向断言通过；归零: LRU提前释放、无CAS双loader、delivery TOCTOU、mailbox旁路、best-effort release与cleanup前capacity release均为0；全部硬前置现已闭合。；2026-08-02 最终复核: 直接缺口与硬前置均已闭合，状态、实现与依赖拓扑一致。 |
| M8 | R1.23 | DONE | Codex | R1.24、R2.19、R2.29、R2.46 | 实现: workspace-scoped `FileLeaseCoordinator` 是唯一maintenance owner，cleanup与Artifact GC共用acquisition token/monotonic fence；stamp仅作调度hint；ownership release、GC、overflow/session删除前逐阶段assert current；sweep保护全部live RunLease，legal hold先于TTL，collector只消费R1.24 immutable closure。测试: 20 个maintenance、pin、cleanup小型定向节点通过；归零: process-local cleanup owner、并行GC owner、stamp承担ownership、只保护调用session、stale owner删除、live/held session按mtime删除均为0。；复核闭合: 全部硬前置已恢复DONE且本项无独立实现反证，原实现/测试/归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。 |
| M8 | R1.24 | DONE | Codex | R2.24 | 实现: Runtime唯一typed `ArtifactPinRegistry`管理transient reachability及freeze/repin原子性；生产CAS构造入口门禁证明仅有Artifact layout与FileOps transaction两处：DurableArtifactStore将全部未释放committed representation及queued/failed publication outbox representation纳入root scan，FileOps通过公开pin source与全session durable journal roots投影；Workflow/BackgroundTask/tool/model没有第三个CAS写入口，若产生Artifact revision即由前者owner edge覆盖；legal hold在session owner release前阻断删除。测试: 20 个pin、writer-closure、maintenance、cleanup小型定向节点通过；归零: 空pin装配、第三CAS旁路writer、collector读取producer私有map、minimum-age证明unreachable、publication窗口丢root及hold后release均为0。；复核闭合: 全部硬前置已恢复DONE且本项无独立实现反证，原实现/测试/归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。 |
| M8 | R2.19 | DONE | Orchestration Agent Residency owner | — | 实现: strict`mote.agent-residency/v1`绑定完整identity、revision、Session source与materialization fence；Store只支持可信blueprint的canonical materialize/rehydrate/forget，删除pre-v1 migration与Mailbox upcaster；嵌入mailbox snapshot现经R2.20唯一strict Message decoder恢复；测试: 29 个Residency/Mailbox/Cron断言及R2.20新增inner-message负例通过；归零: 双读、migration API、磁盘class选择、ownerless/fenceless恢复、identity断链与宽松嵌套Message均为0。 |
| M8 | R2.20 | DONE | Agent messaging Mailbox owner | — | 实现: strict`mote.agent-mailbox/v1`绑定owner、sequence、delivery/message identity与exact boolean；canonical`Message.from_dict/load`统一执行required/allowed exact fields、primitive、nonempty identity、unique send_to、instruct tagged envelope与metadata JSON validation，非法JSON直接typed failure不返回None；Mailbox、delivery、Session replay共用此入口；测试: 31 个Mailbox/delivery/resource-message合并断言及9个schema负例通过；归零: 裸list、旧shape、错误Message跳过、bool coercion、ownerless构造、extra/wrong primitive与动态非JSON metadata均为0。 |
| M8 | R2.24 | DONE | Contracts output event owner | R0.1 | 实现: `contracts/events/output.py` 的 candidate/accepted/migrated/committed durable value 使用 canonical `JsonValue` 并在构造时 `freeze_json` fail closed，全部 durable output fact 为 frozen dataclass；`contracts/ports/output/evaluation.py` 固定 encoder 返回 JSON，Runtime 写 fact 前完成严格投影；测试: 原 13 个及本轮 10 个 output durable payload 定向断言通过；归零: durable output raw/value 与 `OutputDecoder.encode` 的 `Any` 返回、构造后覆盖 raw/value/validator provenance 绕过 canonical projection 均归零，telemetry-only snapshot 动态值保持隔离。；复核闭合: 唯一联动退回前置已完成，本项原有实现、测试与归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 对未闭合前置的依赖为0。 |
| M8 | R2.27 | DONE | Runtime Session truth + Orchestration Agent Residency commit | — | 实现: `runtime/session/log.py`与`runtime/session/projection.py`在verified read/live projection重申meta首条唯一及envelope/stream/meta identity全等；Residency v1 record显式绑定logical identity并以revisioned durable claim、install fence和fenced forget组成恢复提交，架构门禁直接读取authoritative `runtime/session/projection.py`且不建立旧模块兼容；测试: 18 个hosting/residency/hook/error小批量断言通过；归零: identity split-brain、宽松meta覆盖、校验前注册删除、stale claim删除后继generation、安装失败残留及已删除projection路径的验收消费者均归零。 |
| M8 | R2.28 | IN_PROGRESS | Codex | R1.20、R2.17、R2.51、R2.52 | 实现: Contracts固定stable SpawnRequest、九态durable lifecycle、LineageRecord、typed advance/authorization receipt与revisioned subtree snapshot；`orchestration/agents/lineage/`以strict `mote.agent-lineage/v1`、跨进程文件锁、current lease fence及fsync atomic replace拥有唯一lineage/spawn-saga truth；AgentControl在worker启动前提交logical identity、capacity和完整budget reservation identity，冷启动按lineage IDs从canonical UsageLedger严格恢复active reservation，missing、settled或identity mismatch fail closed，不复制余额；terminal保留tombstone并严格一次结算capacity/budget。测试: 22 个budget/lineage/hosting/cancellation小型节点通过，相关生产文件Pyright 0 error；归零: 空budget IDs、budget=None生产装配、第二budget store、随机logical identity、重复request第二child与冷启动预算丢失均为0。；2026-08-02 最终复核: 直接缺口与硬前置均已闭合，状态、实现与依赖拓扑一致。；2026-08-02 再次逐条验收联动退回: 硬前置 R2.15 当前未闭合（其中间依赖如适用亦随之未闭合），本项按依赖关系不能满足关闭条件；该标注不等同于已确认本项自身存在独立实现反证。；2026-08-02 关闭复核: 直接反证已修复，定向类型/行为/架构门禁通过，硬前置依赖闭包恢复 DONE。 |
| M8 | R2.29 | DONE | Runtime lease persistence owner | R2.46 | 实现: strict `mote.file-lease-coordinator/v1` envelope、重复subject identity、canonical AbsoluteInstant/clock identity与exact primitive decoder；bare mapping fail closed且无migration双读；测试: 88 个既有lease/vault/checkpoint定向断言通过；归零: 裸mapping reader、强转、unknown-version reset、migration与双读均归零；R2.46 Product clock composition已闭合。；复核闭合: RuntimeLease保存canonical expires_at_instant，File codec保留精确纳秒与clock identity；测试: 15 个lease/ownership节点通过，Pyright 0 error；归零: float expiry round-trip为0。 |
| M8 | R2.31 | DONE | Contracts File search codec owner | R0.7 | 实现: `contracts/file/codec.py` 对 row/skipped/summary 执行 exact-shape、exact primitive、enum、非负范围、present-version 与 completion/termination 一致性校验；SearchRow/SearchSummary/SearchSkippedFile 在构造期执行同一不变量，encoder 通过 canonical decoder round-trip 复核后才写出；测试: 13 个 file-search strict codec 定向断言通过；归零: search durable decoder 的 `.get` 默认、`int/str/bool` 强转、未知 reason/termination、额外字段兼容路径及 encoder 写出 reader 拒绝值均归零。 |
| M8 | R2.32 | DONE | Contracts Runtime checkpoint + Runtime interactive codec owner | — | 实现: strict checkpoint identity/evolution与Terminal/Browser/Canvas/Kernel唯一current typed codec；Browser codec复用Contracts canonical storage-state decoder逐项校验nested exact shape和primitive；删除无人消费的异构CheckpointCodec[object] registry/object-return facade，各driver直接持具体typed codec；测试: 15 个checkpoint codec单文件及34个browser合并断言通过；归零: 宽shape、历史decoder、nested错误状态与result类型擦除均归零。；复核闭合: DriverCheckpoint构造验证codec/payload/schema/digest/sensitivity/fidelity，decoder使用exact schema int；测试: 22 个checkpoint节点通过，Pyright 0 error；归零: boolean schema与非法直接构造为0。 |
| M8 | R2.33 | DONE | Runtime Secret vault transaction owner | R1.15 | 实现: strict `mote.secret-vault/v1`、revision与exact sections；canonical writer经vault-scoped跨线程/进程锁、锁内重读、原子提交，坏key/坏vault保留证据并fail closed；测试: 88 个既有lease/SecretStore/vault治理断言通过；归零: unversioned reader、migration、lost update、固定temp、无fsync replace及损坏降级均归零；硬前置R1.15已完成exact int version与boolean拒绝。 |
| M8 | R2.34 | DONE | Product Markdown Agent definition + Runtime recovery identity owner | R1.9 | 实现: strict Markdown definition identity绑定approved source scope/path/device/inode/content digest/principal并生成content-addressed id，Catalog/Session/Residency/incarnation使用同一opaque identity；删除全局Role registry/BaseRole.load平行恢复链；测试: 22 个既有Markdown identity定向断言通过；归零: raw-path parser、best-effort candidate、identity分裂、source replacement fallback及磁盘type选择均归零；硬前置R1.9已完成approved source唯一入口。；复核闭合: 唯一联动退回前置已完成，本项原有实现、测试与归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 对未闭合前置的依赖为0。 |
| M8 | R2.46 | DONE | Codex | — | 实现: Contracts-owned versioned `ClockIdentity`/timezone-aware `AbsoluteInstant` strict codec、process-instance `MonotonicMark`与窄 `ClockSource` Port；Runtime `SystemClock`机制由Product composition显式选择；DST fold/gap只返回typed resolution，不接管Cron/Workflow/lease/retention policy；测试: 13 个deterministic fake-clock断言通过（restart、wall rollback/forward、monotonic、DST、identity/version/shape fail-closed、禁止持久化monotonic），4 个生产文件 Pyright 0 error；归零: durable naive datetime、无clock identity instant、跨process monotonic比较/序列化及万能timer/domain状态机进入clock contract均为 0；各domain字段与rollback/catch-up disposition按台账由其后续工作包迁移；复核闭合: CronService 必需注入唯一 ClockSource 并向 commands/scheduler/lease lock 投影；Orchestration 删除 SystemClock/time.time 默认，CLI 与交互 Product root 显式选择 SystemClock；29 个定向节点通过。；复核闭合: clock schema使用exact int，AbsoluteInstant验证ClockIdentity，nested decoder拒绝boolean schema；测试: 14 个clock节点通过，Pyright 0 error；归零: True冒充v1与错误clock实例为0。 |
| M8 | R2.51 | DONE | Codex | R1.20、R2.16 | 实现: Contracts分别定义logical reservation/settlement、resident incarnation reservation/settlement与turn permit/settlement nominal receipt；`LogicalCapacityProjection`以application/root/subtree/parent scope在单锁内revision-CAS原子准入、严格一次terminal settlement并可从committed facts重建，identity registry删除capacity counter而只拥有不可复用identity/tombstone；ResidencySlot绑定reservation identity与incarnation generation，只有commit/rollback或R1.20 eviction settlement释放resident slot；turn limiter只拥有`max_concurrent_turns`与typed UUID permit，重复release不双减；AgentControl/Product schema一次性拆成logical、resident、turn三个配置值，spawn按logical→resident→identity顺序取得typed authority，terminal、eviction与turn各自只结算所属capacity；测试: 53个capacity并发/revision/rebuild、limiter、registry、Residency、Control定向与local-import断言按小文件或少量节点通过，7个核心contract/Orchestration文件Pyright 0 error；归零: 生产与测试中的`max_agents`、`residency_capacity`、`reserve_spawn_slot`、registry `_total_count`、一个配置投给多类cap及lineage/Residency/limiter共享mutable counter均为0；复核闭合: AgentControl 使用同workspace strict durable capacity fact store恢复active counters与agent→reservation索引，active lineage缺失reservation时fail closed；新增durable reopen/settle/reopen验证，写失败不发布虚假receipt。；2026-08-02 最终复核: 直接缺口与硬前置均已闭合，状态、实现与依赖拓扑一致。 |
| M8 | R2.52 | DONE | Codex | existing UsageLedger extension | 实现: canonical `SQLiteUsageLedger` v2 typed dimension/scope原子约束Agent root/subtree Token、cost-micro-USD、depth与capability；Product application唯一构造governance authority并经EngineServices注入resident Agent composition；spawn先reserve再durable提交reservation IDs，失败回滚以0 settlement，terminal按canonical CostTracker实际Token/微美元settle退款；冷重启按IDs从同一ledger严格查询active receipt；Product schema验证child reservation不超过root budget。测试: 22 个budget/lineage/hosting/cancellation小型节点通过，相关生产文件Pyright 0 error；归零: Agent第二ledger、空budget IDs、budget=None生产装配、内存预算snapshot、宽松v1恢复与重启预留丢失均为0。；复核闭合: UsageLedger命令exact primitive/aware datetime，SQLiteUsageLedger强制注入ClockSource且无datetime.now；测试: 8 个usage/budget节点通过；归零: bool数值与绕过canonical clock为0。 |
| M8 | R2.53 | IN_PROGRESS | Codex | R1.20、R1.26、R2.16、R2.28、R2.50 | 实现: lineage store在spawn admission同一fenced transaction推进subtree cancellation epoch、冻结revisioned target snapshot并原子abort未公开descendants；`SubtreeCancellationCoordinator`逐Agent派发typed idempotent command，AgentControl验证epoch/path后依次结算durable unclaimed/claimed turn、runtime、Role-owned Pool、capacity与budget；聚合保留settled/already_terminal/owner_lost/timeout。测试: 22 个subtree cancellation及其budget/lineage/turn前置小型节点通过，相关生产文件Pyright 0 error；归零: supervisor直接修改Pool map、裸bool settlement、无epoch snapshot、spawn/cancel漏child、重复cancel新epoch与取消后预算泄漏均为0。；2026-08-02 最终复核: 直接缺口与硬前置均已闭合，状态、实现与依赖拓扑一致。；2026-08-02 再次逐条验收联动退回: 硬前置 R2.15 当前未闭合（其中间依赖如适用亦随之未闭合），本项按依赖关系不能满足关闭条件；该标注不等同于已确认本项自身存在独立实现反证。；2026-08-02 关闭复核: 直接反证已修复，定向类型/行为/架构门禁通过，硬前置依赖闭包恢复 DONE。 |
| M9 | R1.14 | DONE | Runtime MCP lifecycle + bound catalog owner | R1.9、R1.16、R2.40 | 实现: MCP candidate完成connect/discovery/definition/capability及namespace预校验后单次替换immutable catalog generation并切换lifecycle owner；测试: 12 个既有MCP reload/alias/generation/local-import断言通过；归零: 先删旧catalog、逐项live注册、silent overwrite与失败半代均归零；R1.9/R1.16/R2.40前置已完成。；复核闭合: 全部硬前置已恢复DONE且本项无独立实现反证，原实现/测试/归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。 |
| M9 | R2.10 | IN_PROGRESS | Codex | R1.2、R1.8、R1.9、R1.17、R2.42 | 实现: AgentDependencies使用authoritative Hook/MCP类型与approved activation identity，Runtime client只消费Contracts RuntimeClientActivationSpec且child只继承approved投影；测试: 3 个既有wiring/activation架构断言通过；归零: Hook/MCP Any、Runtime Product根config与自行批准checkout source均归零；全部硬前置已完成。；复核闭合: 全部硬前置已恢复DONE且无独立反证；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。；2026-08-02 再次逐条验收缺口: `AgentWiring.for_context()` 公开返回 `AgentWiring[Any, Any]`，`AgentDependencies` 仍向 Runtime wiring 传播 primary/user/session/browser/oauth 等路径与任意 routing builder object，组件仍可经宽 wiring/service locator 取得无关 capability，未闭合本项要求的窄 immutable input/Port 边界。；2026-08-02 关闭复核: 直接反证已修复，定向类型/行为/架构门禁通过，硬前置依赖闭包恢复 DONE。 |
| M9 | R2.12 | IN_PROGRESS | Codex | M0 | 实现: 唯一`contracts/config/base.py::ConfigModel`统一`extra=forbid`与frozen配置值语义；Contracts inference/tool、Runtime LSP/Hook/MCP/device、Product overrides/model inputs/WebSearch全部迁移canonical base，删除inference私有`_Config`；diagnostics仅在外部annotation inspector边界识别Pydantic BaseModel，不声明配置；测试: 13 个ownership、forbid-extra、override与LSP构造定向断言通过，10个迁移生产文件编译通过；归零: 正式配置直接继承BaseModel、重复私有base、LSP/Hook宽松extra、旧base import及Runtime接收Product根shape均为0。；复核闭合: sandbox/file-watch/permission/RoleSchema及BrowserClientCert全部迁移canonical ConfigModel，Role name与skill-fork改为immutable model_copy投影，并新增目录外正式配置门禁；测试: 16 个config/loader及12 个Role不可变投影节点通过；归零: 已确认正式部署配置直接BaseModel与RoleSchema写后改均为0。；2026-08-02 再次逐条验收缺口: `contracts/conversation/context.py::ContextManagerConfig` 仍直接继承 Pydantic `BaseModel`，未使用 canonical forbid-extra `contracts.config.base.ConfigModel`；额外字段仍会被默认忽略，和本行所称正式配置直接继承 BaseModel 已归零相矛盾。；2026-08-02 关闭复核: 直接反证已修复，定向类型/行为/架构门禁通过，硬前置依赖闭包恢复 DONE。 |
| M9 | R2.23 | DONE | Runtime artifact bounded-context owners | — | 实现: verified CAS为ContentAddressedArtifactStore，mutation reservation/lifecycle owner为FileMutationArtifactRepository；DurableArtifactStore作为当前artifact metadata/lifecycle store真实存在并被Runtime组件消费，composition governance声明与公开面一致，不是旧名残留；测试: 12 个composition/retired-residue小批量门禁通过；归零: 歧义ArtifactRepository、ContentRepository alias、错误owner声明及不同lifecycle强行合并均归零。 |
| M9 | R2.35 | DONE | Product AgentCatalog compiler owner | R2.34 | 实现: 唯一canonical compiler以versioned JSON摘要name/aliases/description/definition identity并完成全命名空间所有权校验；测试: 26 个既有compiler/lookup/Markdown定向断言通过；归零: inspect identity、多version算法、先到先得lookup与私有definitions外部装配均归零；硬前置R2.34已闭合。；复核闭合: 全部硬前置已恢复DONE且本项无独立实现反证，原实现/测试/归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。 |
| M9 | R2.36 | DONE | Codex | R2.40 | 实现: ToolExecutor唯一mint logical id/attempt，definition/catalog/args/owner/run identity贯穿effect全链并由durable replay复核；测试: 88 个既有identity/codec/presentation定向断言通过；归零: optional id、UI临时id、位置/name fallback及live catalog公共面均归零；硬前置R2.40已闭合。；复核闭合: 全部硬前置已恢复DONE且本项无独立实现反证，原实现/测试/归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。 |
| M9 | R2.40 | DONE | Runtime canonical Tool definition compiler owner | R1.9 | 实现: strict schema-versioned compiler统一protocol/name/aliases/schema/effect/approval及content-addressed Python/MCP source identity，全部catalog/snapshot入口委托同一compiler；测试: 41 个既有definition/catalog/MCP定向断言通过；归零: inspect源码身份、固定版本字符串、多套摘要及未传播identity/source/approval均归零；硬前置R1.9 approved source唯一入口已完成。；复核闭合: 唯一联动退回前置已完成，本项原有实现、测试与归零证据保持有效；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 对未闭合前置的依赖为0。 |
| M9 | R2.49 | DONE | Codex | R1.9、R1.14、R2.35、R2.40、R2.42 | 实现: Product `ApplicationReloadCoordinator`/`AtomicApplicationComposition`是唯一reload trigger、candidate compiler调用与generation pointer owner；candidate在active代外完整build/readiness后以latest request sequence+source revision+ExpectedActive CAS原子swap，stale/failed candidate关闭自身且旧代不变；candidate携带approved capability set与trust revision，能力扩大或extension/source identity变化fail closed并要求重新批准；session/turn通过ApplicationLease固定单代，旧代进入retired只在最后holder release后drain/close，shutdown有界结算；CLI 通过唯一 `product.composition.bootstrap.activate_application` 间接安装 initial composition，不再要求入口复制底层 installer；测试: 本轮 3 个小型架构文件 7/7 通过，覆盖production reachability、唯一Application constructor owner、外层event-loop与hot-reload owner；归零: 原地mutate active config/catalog、失败candidate破坏旧代、stale reload覆盖新代、同turn混代、能力扩大自动激活、陌生checkout变化继承trust、Runtime第二reload owner与CLI复制composition installer均为0；全部硬前置已闭合。；复核闭合: 全部硬前置已恢复DONE且无独立反证；测试: 1 个台账依赖门禁及本行原定向证据通过；归零: 未闭合依赖为0。 |
| M10 | R3.1 | DONE | Error domain owners | — | 实现: Agent错误迁入`runtime/agent/errors.py`，OAuth错误与分类合并到`runtime/models/auth/oauth/errors.py`，provider-call稳定错误迁入Contracts，route/provider catalog错误分别归Runtime/Product model owner，recovery/classification归Runtime resilience，provider logging helper归Product provider；删除Runtime errors目录下全部Python源码并迁移生产与测试消费者，门禁禁止所有`mote.runtime.errors.*` import；测试: 32 个owner/recovery/provider/Role定向断言通过，8个迁移文件编译通过；归零: Runtime errors垃圾包源码、全仓旧子模块import、Contracts error re-export、错误owner doc引用均为0；Pyright仅报告环境未安装anthropic/openai optional dependency。 |
| M10 | R3.2 | DONE | Package facade owners | — | 实现: 删除turn format、interactive eager、OAuth package/registry、Product models和presentation projection兼容facade；Agent/Bootstrap/测试迁到defining module；environment facade与旧Session projection门禁只验证文件不存在，不再读取旧实现；Sandbox package init清除SandboxRuntime/detect_backend/violation eager re-export，仅保留bounded-context声明；测试: 原33个facade/context/OAuth证据保留，新增 11 个facade retirement、hosting与Role边界断言通过；归零: 六个旧facade文件、精确旧入口import、OAuth二次export、基础Agent可选backend加载及Sandbox公共符号re-export均为0。 |
| M10 | R3.3 | DONE | Runtime Tool binding owner | — | 实现: live binding为`ExecutableToolBinding`，snapshot registry持有`PinnedToolInvocation`；ToolExecutor新增只供snapshot manager使用的typed `run_pinned_command`，完整复用authorization/ledger/effect/settlement pipeline但直接注入retained executable binding，identity使用snapshot revision与binding semantic identity；catalog replacement后旧snapshot继续执行原binding，release后才unrecoverable；测试: 8 个snapshot/registry/AST定向断言通过，4个核心生产文件Pyright 0 error；归零: `BoundTool` alias、snapshot按名称回查live catalog、identity guard伪pinned及replacement导致旧snapshot不可执行均为0。 |
| M10 | R3.4 | DONE | Canonical package owners | — | 实现: 删除死模块、错误export与平行catalog，composition声明中的DurableArtifactStore/FileWatchService均由当前authoritative模块真实定义和消费；清除生产注释/docstring对deleted common owners的引用；测试: 12 个composition/retired-residue门禁及原28个compatibility证据通过；归零: 死模块、旧常量、错误export、deleted common owner文字残留与缺失治理符号均归零。；复核闭合: 生产注释/docstring中的common.git_state/common.i18n/common.const owner残留已删除；测试: 1 个归零搜索门禁通过；归零: deleted common owner生产文字引用为0。 |
| M10 | R3.5 | DONE | Import and optional backend governance owner | — | 实现: 全生产import保持module scope；Product-owned immutable Squilla manifest与唯一approved loader经显式activation解析，普通composition import不加载ML/Textual/Chromium；hermetic门禁固定从项目父目录启动独立`python -B`，不依赖调用者PYTHONPATH；测试: 原15个local/dynamic/manifest证据保留，3 个optional backend断言在显式移除PYTHONPATH的子进程环境通过；归零: nested import、未登记loader、composition eager optional backend及依赖测试启动环境的伪hermetic证明均为0。 |
| M10 | R3.6 | DONE | Codex | R3.1 | 实现: Contracts `mote.error-report/v1` strict envelope绑定typed namespace、authoritative ErrorCode/RecoveryAction与JSON-safe context；ToolResult receipt使用唯一`tool-result+json@3`并拒绝v2，BackgroundTask内部持typed report，notification Session wire严格投影恢复；无upcaster/双读；当前R0.1架构门禁已同步明确断言@3；测试: 原有ErrorReport/receipt/BackgroundTask/Session证据保留，新增 10 个ToolResult/ErrorReport架构断言通过；归零: 宽松decoder、旧receipt v2、内部error裸dict、旧detail wire字段、partial shape与@2验收真相均为0。；复核闭合: ErrorReport构造严格验证error/code/message/recovery/cause/retryable/namespace primitive；测试: 33 个ErrorReport/Cron节点通过；归零: 构造器写出decoder拒绝envelope路径为0。 |

#### 10.5.1 2026-08-01 第二轮逐项架构验收记录

本轮对台账全部 96 个工作包逐行复核状态、硬前置、实施证据中的归零声明、当前生产源码 owner/consumer 及现存架构门禁的一致性。结论为 66 个保留 `DONE`、30 个 `IN_PROGRESS`；其中 `R2.43` 与 `R3.1` 是第二轮新增回退，`R2.27` 的回退原因已纠正为“canonical 新 owner 存在，但旧验收 consumer/gate 未迁移”。所有 `IN_PROGRESS` 行均在本表内保留可复核的“复核缺口”，不得用本节摘要替代逐项原因。

本轮未再次执行测试。静态验收只用于发现确定性反证，不能把未执行的运行时、崩溃恢复、并发、跨进程或真实 provider 验证推定为通过；保留 `DONE` 表示本轮未发现新的直接反证，不扩大其原实施证据所证明的范围。此前已完成的一次 `ztest/architecture` 执行存在 18 个失败节点，相关失败已投影到对应工作包；在这些门禁及其硬前置闭合前，不得宣称 96 项整体完成。

#### 10.5.2 2026-08-01 第三轮 `DONE` 专项复核记录

本轮只复核第二轮保留的 66 个 `DONE`，未执行测试。新增发现并回退 `R1.3` 与 `R1.5`：前者的 Product interaction 主路径仍以 `getattr` 协商 InputPort 生命周期/可选能力，后者新增 `AttemptId` 后没有同步 Runtime package-governance 的 authoritative public-surface 门禁。依据硬前置规则，连带回退 `R1.19`、`R1.26` 与 `R2.9`。复核后总账为 61 个 `DONE`、35 个 `IN_PROGRESS`，且不存在 `DONE` 直接依赖 `IN_PROGRESS` 的台账关系。

本轮保留的 61 个 `DONE` 仅表示基于当前静态源码、owner/consumer、归零声明和现存门禁证据未发现新的确定性反证；没有运行的行为、故障注入或真实 backend 验证仍不得推定为通过。

#### 10.5.3 2026-08-01 第四轮 `DONE` 专项复核记录

本轮只复核第三轮保留的 61 个 `DONE`，未执行测试。新增直接回退 `R1.9`、`R2.15`、`R2.44` 与 `R2.8`：扩展来源仍有 raw-path parser 与 malformed candidate best-effort 路径；Product child builder 仍以 `object + TypeGuard` 伪造 `RunnableAgent[str]` 泛型关系；公开 application metrics observation 仍以 `object + type map` 建立未装配的窄化 seam；Code Map rebuild source 仍以 `event: object` 自行窄化且 canonical Protocol 文档保留 `getattr` fallback 承诺。

依据硬前置规则，连带回退 `R1.16`、`R2.34`、`R1.14`、`R2.40`、`R2.35`、`R2.36`、`R2.45`、`R2.17` 与 `R2.18`。复核后总账为 48 个 `DONE`、48 个 `IN_PROGRESS`，且不存在 `DONE` 直接依赖 `IN_PROGRESS` 的台账关系。静态保留的 `DONE` 不代表未执行的行为、故障注入、真实 provider 或跨进程验证已通过。

#### 10.5.4 2026-08-01 第五轮 `DONE` 专项复核记录

本轮只复核第四轮保留的 48 个 `DONE`，未执行测试。新增直接回退 `R2.37` 与 `R2.46`：Execution epoch 在 Embedded、Shared client 与 Shared daemon 中分别由进程内 authority 构造，且全仓没有生产推进 backup/admission epoch 的 consumer；Clock contract 则被 Orchestration Cron lock 通过直接导入并默认构造 `SystemClock` 绕过 Product composition，Cron service/scheduler 仍保留直接 wall-clock 默认。

依据硬前置规则，连带回退 `R1.11`、`R1.12`、`R1.22`、`R2.29`、`R1.21` 与 `R1.25`。复核后总账为 40 个 `DONE`、56 个 `IN_PROGRESS`，且不存在 `DONE` 直接依赖 `IN_PROGRESS` 的台账关系。保留项仍只表示本轮静态检查未发现新的确定性反证。

#### 10.5.5 2026-08-01 第六轮 `DONE` 专项复核记录

本轮只复核第五轮保留的 40 个 `DONE`，未执行测试。新增直接回退 `R1.18`、`R2.6` 与 `R1.1`：Browser profile 的 strict nested DTO 只在 decoder 生效，写入边界可接受伪造 nested primitive；Media Provider composition 仍使用 `config: Any`、`getattr` 与公开可变 registry；Runtime generation closeables 仍为 `tuple[object, ...]` 并通过 `getattr(aclose/close)` 猜测生命周期。

依据硬前置规则，连带回退 `R1.6`、`R2.7`、`R2.13`、`R2.26`、`R2.38` 与 `R2.39`。复核后总账为 31 个 `DONE`、65 个 `IN_PROGRESS`，且不存在 `DONE` 直接依赖 `IN_PROGRESS` 的台账关系。保留项仍只表示静态检查未发现新的确定性反证。

#### 10.5.6 2026-08-01 第七轮 `DONE` 专项复核记录

本轮只复核第六轮保留的 31 个 `DONE`，未执行测试。新增回退 `R2.20`、`R2.19`、`R2.32` 与 `R2.24`：Mailbox 只严格校验外层 envelope，内层 message 仍通过宽松 `Message.load/from_dict` 恢复；Residency rehydrate 直接依赖该 Mailbox snapshot；Browser checkpoint 未校验 cookie/origin/localStorage nested shape；durable output events 仍是可变 dataclass，构造后可覆盖已 freeze 的 JsonValue 字段。

复核后总账为 27 个 `DONE`、69 个 `IN_PROGRESS`，且不存在 `DONE` 直接依赖 `IN_PROGRESS` 的台账关系。保留项仍只表示静态检查未发现新的确定性反证。

#### 10.5.7 2026-08-01 第八轮 `DONE` 专项复核记录

- 本轮在第七轮剩余 27 个 `DONE` 上继续只读静态验收；遵循用户约束，未运行任何测试。
- 直接撤销 7 项：`R2.31`（Search DTO 构造/编码不变量缺失）、`R2.21`（mutable CronTask 可绕过 owner/CAS）、`R0.1`（ToolResult/媒体/文件变更 receipt 写读不对称）、`R0.8`（StepRecord/AppendOnlyLedger 可提交自身 reader 拒绝的 committed frame）、`R2.16`（并发上限缺少正整数不变量）、`R2.22`（Activity DTO 未严格校验 error/JsonValue）、`R2.41`（generation wire range 跨字段不变量未闭合）。
- 按硬前置规则联动撤销 2 项：`R1.4` 依赖 `R0.1`，`R0.4` 依赖 `R2.41`。
- 复核后 96 项状态为：`DONE=18`、`IN_PROGRESS=78`；当前没有 `DONE` 显式依赖 `IN_PROGRESS` 的表内依赖违例。

#### 10.5.8 2026-08-01 第九轮 `DONE` 专项复核记录

- 本轮对第八轮剩余 18 个 `DONE` 继续进行只读静态架构验收；遵循用户约束，未运行任何测试。
- 直接撤销 8 项：`R1.7`（helper inode check 与按路径 exec 间仍有 TOCTOU）、`R2.54`（公开可变命令授权矩阵）、`R0.2`（CanonicalToolCall 内部 arguments 可变）、`R1.2`（typed human Port 仍被 hasattr 探测且残留 MoteEnv 生产语义）、`R0.7`（ContentIdentity size 写读范围不对称）、`R2.25`（可变 session catalog 与导入时 codec 快照可分叉）、`R2.5`（Code Map DTO 无构造不变量）、`R2.30`（Video 保留第三套一次性 fixed-argv subprocess facade）。
- 按硬前置规则联动撤销 1 项：`R0.5` 依赖 `R2.54` 的 immutable authorization contract。
- 复核后 96 项状态为：`DONE=9`、`IN_PROGRESS=87`；当前没有 `DONE` 显式依赖 `IN_PROGRESS` 的表内依赖违例。

#### 10.5.9 2026-08-01 第十轮 `DONE` 专项复核记录

- 本轮对第九轮剩余 9 个 `DONE` 按原始工作包条款继续进行只读静态验收；遵循用户约束，未运行任何测试。
- 直接撤销 1 项：`R1.15`。OAuth credential record 与 fallback backend-selection 的 version decoder 均以普通 `== 1` 判断版本，JSON boolean `true` 会因 Python `True == 1` 被接受为 v1，strict durable envelope 仍未闭合。
- 按硬前置规则联动撤销 1 项：`R2.33` 依赖 `R1.15`。
- 复核后 96 项状态为：`DONE=7`、`IN_PROGRESS=89`；当前没有 `DONE` 显式依赖 `IN_PROGRESS` 的表内依赖违例。

#### 10.5.10 2026-08-01 第十一轮 `DONE` 专项复核记录

- 本轮对第十轮剩余 7 个 `DONE` 继续进行只读静态架构验收；遵循用户约束，未运行任何测试。
- 直接撤销 2 项：`R2.12`（大量正式配置仍直接继承 `BaseModel`，部分未 forbid extra，并保留第二私有 `_Config` base，未实现全仓 canonical base 收敛）、`R1.8`（source descriptor 的 path/dev/inode 未在实际 open/read 后复核，discovery 与 parse 间替换可使 provenance identity 和实际内容分叉）。
- 复核后 96 项状态为：`DONE=5`、`IN_PROGRESS=91`；当前没有 `DONE` 显式依赖 `IN_PROGRESS` 的表内依赖违例。

#### 10.5.11 2026-08-01 第十二轮 `DONE` 专项复核记录

- 本轮按用户要求对最后 5 个 `DONE` 主动查找生产架构缺口；遵循用户约束，未运行任何测试。
- 撤销全部 5 项：`R0.0`（`with_plugins()` 旁路触发 checkout Agent discovery 且丢失 canonical cwd）、`R2.2`（CompletionDecision 非封闭状态且 graph 静默恢复/接受非法 candidate index）、`R2.3`（queue boundary 接受 NaN deadline 等非法运行状态）、`R2.4`（ambient ContextVar 与 Role.agent_control 两条可分叉 control 真相链）、`R3.3`（PinnedToolInvocation 仍通过 live name lookup 执行，未拥有真正 pinned binding 生命周期）。
- 复核后 96 项状态为：`DONE=0`、`IN_PROGRESS=96`；不存在残余 DONE 或 DONE 依赖闭包问题。
...
截至 2026-08-01，本文件的实施基线为：96 个工作包全部 `CONFIRMED`，D1–D5 产品决定全部生效，122 条显式依赖边无环，当前不存在计划级开工阻断。实施 Agent 只按本文件的工作包正文、ADR、依赖表和台账执行，不读取需求演进或评审历史来改变已确认语义。

`CONFIRMED` 只表示实施定义、owner、依赖和验收已经固定，不表示债务已经修复。每个工作包仍须在开工时依据当前源码重建事实链，领取唯一 Owner 后进入 `IN_PROGRESS`；若硬前置尚未完成，依赖链可共同处于 `IN_PROGRESS`，但任何下游包均不得提前签收。只有全部硬前置为 `DONE/REJECTED` 后，工作包才能以代码、测试、门禁和删除旧路径的实际证据签收。
...
3. `DONE` 的全部硬前置均为 `DONE/REJECTED`；`IN_PROGRESS` 可显式暴露正在共同闭合的依赖链，但不得据此签收；`DECISION_REQUIRED` 链接未决 ADR；
