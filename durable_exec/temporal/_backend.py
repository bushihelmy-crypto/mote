...
Satisfies the SAME :class:`~mote.loop.durable.backend.DurableBackend` protocol the
loop drives for Tier 1, so the loop's typed façades
(:class:`~mote.loop.durable.runner.DurableRunner`) are backend-agnostic — only the
...
* **Inside a workflow** (``workflow.in_workflow()``) — dispatch the step as a
  ...
  :class:`~mote.common.schema.ActivityConfig` mapped onto temporalio's
  :class:`RetryPolicy`.
...
from mote.common.ledger import KIND_THINK, KIND_TIMER, RunJournal
