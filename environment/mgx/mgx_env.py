#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""MGXEnv — the human-channel face of the agent control plane.

``MGXEnv`` is the concrete environment that roles run inside. It is the
...
an :class:`MGXEnv`. Everything else (membership, routing, scheduling, residency)
is inherited unchanged from :class:`AgentEnvironment`.

The two human-channel methods mirror the legacy single-bus ``MGXEnv``:
...
from metagpt.common.logs import get_human_input
from metagpt.environment.base_env import AgentEnvironment


class MGXEnv(AgentEnvironment):
