...
class LLMConfig(YamlModel):
    ...
    def check_auth_present(self):
        ...
        Raises ``MissingAPIKeyError`` for the non-OAuth path while allowing an
        empty/placeholder key when OAuth will supply the bearer token.
        """
        if self.oauth is None and not self._api_key_is_valid(self.api_key):
            raise MissingAPIKeyError("Please set your API key in config.yaml")
        return self
