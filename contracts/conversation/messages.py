#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Message classes for the conversation pipeline."""
...
class Message(BaseModel):
    ...
    def check_instruct_content(cls, ic: Any) -> Any:
        if ic and isinstance(ic, dict):
            required_keys = {"type", "version", "value"}
            if set(ic) != required_keys:
                raise ValueError("instruct_content must contain exactly type, version, and value")
            if type(ic["version"]) is not int or ic["version"] != 1:
                raise ValueError("unsupported instruct_content version")
            model = _INSTRUCT_CONTENT_TYPES.get(ic["type"])
            if model is None:
                raise ValueError("unsupported instruct_content discriminator")
            if not isinstance(ic["value"], dict):
                raise ValueError("instruct_content value must be an object")
            ic = model(**ic["value"])
        return ic
    ...
    def ser_instruct_content(self, ic: BaseModel) -> Union[dict, None]:
        if not ic:
            return None
        type_id = next((key for key, model in _INSTRUCT_CONTENT_TYPES.items() if type(ic) is model), None)
        if type_id is None:
            raise ValueError("unsupported instruct_content model")
        return {"type": type_id, "version": 1, "value": ic.model_dump()}
