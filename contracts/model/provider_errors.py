"""Stable provider-call failure contracts.
...
class LLMError(MoteError):
    ...
    Gateway backoff honour the provider's advertised cool-off instead of guessing.
    """
...
class LLMBillingError(LLMError, NonRetryableError):
    ...
    Distinct from the orchestration budget gate. This is the upstream provider refusing on
...
class LLMContentPolicyError(LLMError, NonRetryableError):
    ...
    The default failover policy aborts so changing providers cannot bypass the
    configured governance boundary.
    """

    default_code: ClassVar[ErrorCode] = ErrorCode.LLM_CONTENT_POLICY
    default_recovery: ClassVar[RecoveryAction] = RecoveryAction.ABORT
...
class LLMResourceUnavailableError(LLMError, NonRetryableError):
    ...
    :class:`~mote.runtime.resilience.CircuitBreaker`. Recovery is FALLBACK: fail
...
class LLMUnusableResponseError(LLMError, NonRetryableError):
    ...
    Product adapters raise this only after one wire response has been normalized.
    Runtime may switch endpoint, but the rejection never counts as an availability
    failure.
    """
