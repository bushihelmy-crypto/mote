...
class ExternalCommitState(StrEnum):
...
class QuotaObservation(StrEnum):
    NONE = "none"
    LIMITS = "limits"
...
class HttpCompatibilityClass(StrEnum):
    TRANSPORT = "transport"
...
class FailureDisposition(_FrozenContract):
    ...
    external_commit_state: ExternalCommitState = ExternalCommitState.NOT_COMMITTED
    ...
    safe_message: str = Field(default="", max_length=2048)
    reconcile_strategy: ReconcileStrategy = ReconcileStrategy.NONE
    usage_observation: UsageObservation = UsageObservation.NONE
    http_compatibility_class: HttpCompatibilityClass = HttpCompatibilityClass.INTERNAL

    @model_validator(mode="after")
    def _validate_reconciliation(self) -> "FailureDisposition":
        if self.external_commit_state is ExternalCommitState.UNKNOWN:
            if self.retryability is not Retryability.RECONCILE_ONLY:
                raise ValueError("unknown external commit requires reconcile_only")
            if self.reconcile_strategy is ReconcileStrategy.NONE:
                raise ValueError("unknown external commit requires reconciliation strategy")
        elif self.retryability is Retryability.RECONCILE_ONLY:
            raise ValueError("reconcile_only requires unknown external commit")
        return self
