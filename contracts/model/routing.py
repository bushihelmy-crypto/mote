...
def _semantic_route_from_json(value, info: ValidationInfo):
    if info.mode == "json" and isinstance(value, str):
        route = decode_route_id(value)
        if not isinstance(route, SemanticRoute):
            raise ValueError("semantic routing requires a semantic RouteId")
        return route
    return value
...
class RouteCandidate(_FrozenContract):
    ...
    _decode_route = field_validator("route_id", mode="before")(_semantic_route_from_json)

    @field_serializer("route_id")
    def _serialize_route(self, route_id: SemanticRoute) -> str:
        return encode_route_id(route_id)
...
class RoutingHints(_FrozenContract):
    ...
    candidate_scope: tuple[SemanticRoute, ...] | None = None

    _decode_scope = field_validator("candidate_scope", mode="before")(
        lambda value, info: (
            tuple(_semantic_route_from_json(item, info) for item in value) if value is not None else None
        )
    )

    @field_serializer("candidate_scope")
    def _serialize_scope(self, scope):
        return tuple(encode_route_id(route_id) for route_id in scope) if scope is not None else None
...
class RecentRoutingDecision(_FrozenContract):
    decision_id: str = Field(min_length=1)
    selected_route_id: SemanticRoute
    final_class: str | None = None
    turn_id: int = Field(default=0, ge=0)

    _decode_route = field_validator("selected_route_id", mode="before")(_semantic_route_from_json)

    @field_serializer("selected_route_id")
    def _serialize_route(self, route_id: SemanticRoute) -> str:
        return encode_route_id(route_id)


class RoutingHold(_FrozenContract):
    target_route_id: SemanticRoute
    evidence: str = ""
    turns_remaining: int | None = Field(default=None, ge=1)
    expires_at_utc: AwareDatetime | None = None

    _decode_route = field_validator("target_route_id", mode="before")(_semantic_route_from_json)

    @field_serializer("target_route_id")
    def _serialize_route(self, route_id: SemanticRoute) -> str:
        return encode_route_id(route_id)
