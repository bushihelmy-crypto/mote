...
from mote.contracts.model.invocation import CanonicalModelResponse, ModelUsage
...
class ModelCallPlannedRecord(_FrozenRecord):
    ...
    def _generation_attribution(self) -> "ModelCallPlannedRecord":
        if not self.runtime_generation_id or not self.topology_revision:
            raise ValueError("model plans require generation attribution")
        decode_route_id(self.route_id)
        return self
...
class ModelWireAuthorizedRecord(_FrozenRecord):
    kind: Literal["wire_authorized"] = "wire_authorized"
    schema_version: Literal[1] = 1
    model_call_id: str = Field(min_length=1)
    attempt_id: str = Field(min_length=1)
    ordinal: int = Field(ge=1)
    ...
    permit_digest: str = Field(pattern=r"^sha256:[0-9a-f]{64}$")
    occurred_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
