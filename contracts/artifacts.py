"""Stable contracts for immutable, revisioned artifacts."""
...
def _validate_content_ref(content_ref: str) -> None:
    ...
    has_scheme = bool(separator and re.fullmatch(r"[A-Za-z][A-Za-z0-9+.-]{0,31}", scheme))
    if (
...
class ArtifactContentRef:
    ...
    def __post_init__(self) -> None:
        if not self.content_ref or not self.digest:
            raise ValueError("artifact content_ref and digest must be non-empty")
        _validate_content_ref(self.content_ref)
        _validate_digest(self.digest)
        if type(self.size) is not int or self.size < 0:
            raise ValueError("artifact content size must be a non-negative integer")
